...
-- Customer activation and tier changes share one advisory-lock namespace. The
-- subscription trigger takes the transaction-scoped side before evaluating the
-- pending downgrade ceiling. The reseller tier-change endpoint holds the same
-- key while it changes the remote billing agreement and persists pending_tier_id,
-- so a last-seat activation cannot slip between those two steps.
...
BEGIN
    ...
    PERFORM pg_advisory_xact_lock(
        hashtextextended('captainfin:reseller-capacity:'||reseller_uuid::text,904731)
    );
