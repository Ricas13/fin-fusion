-- Explicit administrator lifetime-entitlement override.
-- This never rewrites provider billing. It pins one existing primary
-- subscription as the customer's service entitlement until an administrator
-- revokes the override. Explicit access holds still block delivery.
...
CREATE TABLE IF NOT EXISTS customer_entitlement_overrides (
    ...
    CONSTRAINT customer_entitlement_overrides_reason_length CHECK (length(reason) <= 500)
);

CREATE INDEX IF NOT EXISTS customer_entitlement_overrides_active_idx
    ON customer_entitlement_overrides(customer_id, subscription_id)
    WHERE permanent_access=TRUE AND revoked_at IS NULL;
...
COMMENT ON TABLE customer_entitlement_overrides IS 'Audited administrator entitlement overrides. Permanent access pins one existing primary subscription without modifying provider billing.';
