...
-- Payment delinquency is subscription-specific. Administrative/safety holds
-- remain customer-wide. This lets an unrelated healthy product continue while
-- a particular recurring provider subscription is delinquent.
...
-- Jellyfin provisioning owns only Jellyfin-capable primary products. A paid or
-- trial Premium/Bundle entitlement overlays the permanent Free Server fallback;
-- once the overlay expires, Free Server becomes effective again automatically.
...
COMMENT ON VIEW effective_customer_entitlements IS 'Effective Jellyfin-capable primary entitlement. Premium/bundle overlays permanent Free Server while independent Stremio access may coexist.';

-- Stremio owns a separate primary lane. Bundles participate in both lanes.
...
COMMENT ON VIEW effective_stremio_entitlements IS 'Effective standalone Stremio/bundle primary entitlement, independent of Jellyfin primary access.';
