...
async function main(){
  ...
  try{
    ...
    await Promise.all([admin.waitForNavigation({waitUntil:'networkidle',timeout:15000}),duplicate.getByRole('button',{name:'Create customer'}).click()]);
    ...
    let response=await customer.goto(activationLink,{waitUntil:'networkidle'});
    ...
    response=await replay.goto(activationLink,{waitUntil:'networkidle'});
