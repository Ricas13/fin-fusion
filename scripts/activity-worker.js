...
process.env.DATABASE_URL=process.env.ACTIVITY_DATABASE_URL;process.env.DATA_ENCRYPTION_KEY=process.env.ACTIVITY_ENCRYPTION_KEY;process.env.ALLOW_LEGACY_DATA_KEY_FOR_JELLYFIN='false';
...
let lastFleetRun=0,shuttingDown=false,sleepTimer=null,sleepResolve=null,lastPolicyReload=0;
...
async function refreshPolicyIfDue(){if(Date.now()-lastPolicyReload<30000)return;lastPolicyReload=Date.now();try{const loaded=await streamPolicy.reloadIntoEnvironment();console.log(`Stream policy mode=${loaded.mode} grace=${loaded.graceSeconds}s confirmations=${loaded.confirmationsRequired}`)}catch(error){console.warn('Stream policy settings unavailable; keeping last safe configuration:',error.message)}}
...
async function run(){heartbeat();while(!shuttingDown){const started=Date.now();try{const guarded=await withMaintenanceSharedLock(async()=>{await refreshPolicyIfDue();try{const result=await activity.runActivityPolicyCycle();if(!result.skipped)console.log(`Activity cycle mode=${result.mode} streams=${result.observedStreams} violations=${result.violations} durationMs=${Date.now()-started}`)}catch(error){console.error('Activity cycle failed:',error.message)}try{await refreshFleetMetricsIfDue()}catch(error){console.error('Fleet metrics refresh failed:',error.message)}return{processed:true};},{skipIfBusy:true});if(guarded?.skipped&&guarded?.reason==='database_maintenance')console.log('Activity cycle skipped during database maintenance.');}catch(error){console.error('Activity maintenance guard failed:',error.message)}heartbeat();if(!shuttingDown)await sleep(intervalSeconds*1000)}heartbeat();try{await getPool().end()}catch(_){}}
