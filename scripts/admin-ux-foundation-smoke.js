...
const librariesGroup=navModel.groupFor('libraries');
assert(librariesGroup.key==='jellyfin'&&librariesGroup.pages.some(page=>page[0]==='libraries')&&navModel.sidebarKey('libraries')==='servers','Libraries must remain a routable Jellyfin Servers workflow without permanent sidebar space');
...
assert(planCreateV2.includes("'concurrent_streams', 'household_network'")&&planCreateV2.includes("jellyfinAccessModel === 'household_network' ? null")&&planCreateV2.includes('data-jellyfin-stream-fields')&&planCreateV2.includes('data-jellyfin-household-fields'),'Jellyfin plan creation must expose stream-count and household-connection enforcement without applying stream counts to household plans');
