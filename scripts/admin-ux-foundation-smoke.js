...
const deliveryGroup=navModel.groups.find(group=>group.key==='servers');
assert(deliveryGroup&&!deliveryGroup.pages.some(page=>page[0]==='libraries'),'Libraries must not render as a standalone left-sidebar Delivery child');
const librariesGroup=navModel.groupFor('libraries');
assert(librariesGroup.key==='servers'&&librariesGroup.pages.some(page=>page[0]==='libraries')&&navModel.sidebarKey('libraries')==='servers','Libraries must remain a routable Jellyfin-server workflow without permanent sidebar space');
