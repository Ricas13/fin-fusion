...
assert(directLifecycle.includes("serviceScope=require('../entitlements/service-scope')")&&directLifecycle.includes("function jellyfinCapable(row){return serviceScope.capabilities(row).has('jellyfin');}")&&directLifecycle.includes('serviceScope.overlaps(sub,plan)&&jellyfinCapable(plan)')&&directLifecycle.includes('!serviceScope.overlaps(sub,target)||!jellyfinCapable(target)'),'single-customer plan changes must only offer and accept service-compatible Jellyfin-capable target plans');
assert(directLifecycle.includes('!(subscriptionState.recurringProvider(sub)&&planExpiry.isFreeTier(plan))')&&directLifecycle.includes('subscriptionState.recurringProvider(sub)&&planExpiry.isFreeTier(target)'),'recurring provider subscriptions must not be manually moved to the free tier while provider billing remains active');
assert(directLifecycle.includes('planChange.currentRecurring(req.params.customerId,target)')&&directLifecycle.includes('planChange.requestChange({customerId:req.params.customerId')&&!directLifecycle.includes('applyManualEntitlementContract'),'recurring Customer 360 plan changes must use the canonical provider-aware plan-change service rather than a local-only entitlement rewrite');
assert(directLifecycle.includes("result.mode==='immediate'")&&directLifecycle.includes('Provider billing changed successfully, but access reconciliation needs retry'),'provider-accepted plan changes must not be reported as failed only because downstream access reconciliation needs retry');
