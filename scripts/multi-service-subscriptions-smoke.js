...
function staticContracts(){
  ...
  assert(dashboard.includes("a.access_lane==='free'?'Free Server':'Premium Jellyfin'"),'Jellyfin server cards must identify the Free and Premium lanes');
  ...
  assert(migration.includes("CHECK (access_lane IN ('primary','free'))")&&migration.includes("p_source='free_claim'"),'migration must establish separate Free/Premium lanes and Free-only inactivity holds');
}
...
(async()=>{
  ...
  try{
    ...
    await state.assertNoOtherLiveRecurring(client,upgradeCustomer.id,null,plans.premiumPaid.id);const earlyPremiumPaid=(await client.query(`INSERT INTO subscriptions(customer_id,plan_id,status,source,starts_at,current_period_end,provider_subscription_id) VALUES($1,$2,'active','stripe',NOW(),NOW()+INTERVAL '30 days','sub_trial_upgrade_jellyfin') RETURNING id`,[upgradeCustomer.id,plans.premiumPaid.id])).rows[0];
    await state.assertNoOtherLiveRecurring(client,upgradeCustomer.id,null,plans.stremioPaid.id);const earlyStremioPaid=(await client.query(`INSERT INTO subscriptions(customer_id,plan_id,status,source,starts_at,current_period_end,provider_subscription_id) VALUES($1,$2,'active','paypal',NOW(),NOW()+INTERVAL '30 days','I-TRIAL-UPGRADE-STREMIO') RETURNING id`,[upgradeCustomer.id,plans.stremioPaid.id])).rows[0];
