...
const ROLE_SPECS = {
    ...
    automation: {
        role: 'steamfusion_automation',
        urlEnv: 'AUTOMATION_DATABASE_URL',
        connectionLimit: 12,
    ...
    activity: {
        role: 'steamfusion_activity',
        urlEnv: 'ACTIVITY_DATABASE_URL',
        connectionLimit: 5,
    ...
    backup: {
        role: 'steamfusion_backup',
        urlEnv: 'BACKUP_DATABASE_URL',
        connectionLimit: 6,
        statementTimeout: '0',
        lockTimeout: '10s',
        idleTimeout: '60s',
    ...
    backupVerify: {
        role: 'steamfusion_backup_verify',
        urlEnv: 'BACKUP_VERIFY_DATABASE_URL',
        connectionLimit: 3,
        statementTimeout: '0',
        lockTimeout: '10s',
        idleTimeout: '60s',
...
function activityCredential({ optional = false } = {}) {
    ...
    if (!password) {
        if (optional) return null;
        throw new Error('ACTIVITY_DATABASE_URL (or legacy ACTIVITY_DB_PASSWORD) is required');
    }
...
function ownerPassword() {
    ...
    try { return decodeURIComponent(new URL(raw).password || ''); }
...
async function ensureRole(client, spec, credential) {
    ...
    await client.query(`DO $password$ BEGIN
        EXECUTE format('ALTER ROLE ${role} PASSWORD %L', current_setting('steamfusion.runtime_role_password'));
    END $password$;`);
...
async function grantActivity(client) {
    ...
    await client.query(`GRANT SELECT(id,customer_id,server_id,jellyfin_user_id,disabled) ON jellyfin_accounts TO ${role}`);
    await client.query(`GRANT SELECT(id,customer_id,plan_id,status,current_period_end,created_at,starts_at,superseded_by,service_extension_days) ON subscriptions TO ${role}`);
    await client.query(`GRANT SELECT(id,code,streams,active) ON plans TO ${role}`);
...
async function grantBackup(client) {
    ...
    await client.query(`GRANT USAGE ON SCHEMA public TO ${role}`);
    // pg_dump requires a consistent read of every production table and sequence.
    // The login remains non-owner/non-DDL; its only writes are backup bookkeeping.
...
async function grantBackupVerify(client) {
    // No schema/table grants are intentional. This login can create temporary
    // databases for restore verification but cannot read the production schema.
...
async function configureRoles({ activityOnly = false } = {}) {
    ...
    try {
        ...
        console.log(activityOnly
            ? 'Configured steamfusion_activity with least-privilege grants'
            : 'Configured isolated app, automation, activity, backup and backup-verify PostgreSQL roles');
    } catch (error) {
...
if (require.main === module) {
    main({ activityOnly: process.argv.includes('--activity-only') }).catch(error => {
