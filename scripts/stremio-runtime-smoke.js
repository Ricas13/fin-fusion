...
const jellyfinSource=read('src/stremio/jellyfin-runtime.js');
...
assert(runtimeSource.includes("require('./source-admission')")&&runtimeSource.includes('sourceAdmission.issue()')&&runtimeSource.includes('sourceAdmission.admit'),'all Stremio playback must enforce a CAPTAiNFiN concurrency lease');
assert(runtimeSource.includes('function attachLease(streams,lease=sourceAdmission.issue())')&&runtimeSource.includes('(?:source|jellyfin)')&&runtimeSource.includes('if(streams.length)streams=attachLease(streams)'),'managed and shared releases from one stream lookup must carry one playback lease');
...
assert(runtimeSource.includes('/stremio/:token/jellyfin/:itemId/:mediaSourceId')&&runtimeSource.includes('sourceAdmission.admit(e,lease,null,req.params.itemId)')&&runtimeSource.includes('jellyfin.openPlayback'),'managed hidden-user playback must stay behind the CAPTAiNFiN playback route and serialized entitlement admission gate');
assert(jellyfinSource.includes('startStreamManager')&&jellyfinSource.includes('/Playing/Stop'),'managed hidden-user playback must retain Jellyfin-session reconciliation as defense in depth');
assert(!jellyfinSource.includes('proxyHeaders'),'managed streams must never expose the hidden Jellyfin access token to Stremio');
assert(sourceAdmission.includes('FOR UPDATE')&&sourceAdmission.includes('stream_limit')&&sourceAdmission.includes('active>=limit'),'concurrency admission must serialize per entitlement and enforce the plan stream limit');
...
assert(managedLeaseMigration.includes('ALTER COLUMN source_id DROP NOT NULL'),'managed playback leases must be representable without inventing a shared source');
...
assert(sourcePool.includes('function directPlaybackEnabled(){return false;}'),'manual Stremio sources must not permit token-visible direct playback');
assert(sourcePool.includes('/stremio/${encodeURIComponent(installToken)}/source/')&&!sourcePool.includes("url.searchParams.set('api_key',client.sourceToken(source))"),'manual source playback must remain behind the CAPTAiNFiN proxy without exposing the source token');
