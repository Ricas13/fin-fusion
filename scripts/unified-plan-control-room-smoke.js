...
assert(paygReminderSource.includes("s.source IN ('stripe','paypal','plisio')") && paygReminderSource.includes("s.source='stripe' AND COALESCE(s.provider_subscription_id,'') ~* '^sub_'") && paygReminderSource.includes("s.source='paypal' AND COALESCE(s.provider_subscription_id,'') ~* '^I-'") && !paygReminderSource.includes('s.provider_subscription_id IS NULL'), 'PAYG reminders must include verified one-time provider IDs and exclude only canonical recurring Stripe/PayPal subscriptions');
...
assert(runtimeRoles.includes('media_account_device_policy') && runtimeRoles.includes('media_account_devices') && runtimeRoles.includes('status,source,provider_subscription_id,current_period_end'), 'least-privilege activity role must be able to reconcile device slots and distinguish one-time-vs-recurring subscription identity');
