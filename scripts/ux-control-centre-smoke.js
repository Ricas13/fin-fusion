...
assert(/Customer control/.test(accessCards),'customer control centre missing');
assert(/ctx\.serviceKind==='stremio'/.test(accessCards)&&/'Jellyfin account'/.test(accessCards)&&/'Not required'/.test(accessCards)&&/Stremio-only plan/.test(accessCards),'Customer Control Centre must explain that Stremio-only plans do not require customer Jellyfin placement');
assert(!/manualAssignment\.candidates/.test(customer),'the manual Jellyfin assignment path must live only in the customer-operator console, not duplicated in Customer 360');
const customerOperatorClient=read('public/js/admin-customer-operator.js');
assert(!/buildOperatorPanel/.test(customerOperatorClient)&&/relocatePortalAndTopActions/.test(customerOperatorClient),'the async client-injected operator panel must be gone in favor of server-rendered Customer control');
assert(/Make permanent/.test(accessCards)&&/Reset expiry to subscription/.test(accessCards)&&/permanentAccess\.enable/.test(customer),'permanent customer access controls missing');
...
for(const action of [['add_plan','Add plan'],['cancel_plan','Cancel Plan'],['plan_change','Move Plan'],['migrate_server','Move Server'],['force_server','Force adding to server'],['portal_delete','Delete user and data from the portal...']])assert(bulk.includes(`['${action[0]}','${action[1]}'`),`bulk customer menu missing ${action[1]}`);
for(const retired of ['Libraries: Add','Libraries: Remove','Libraries: Replace','Libraries: Reset to plan','Manual entitlement edit','Extend entitlement','Set expiry date','End current Jellyfin plan','Reset admin overrides to plan','Enable / reactivate service access','Disable service access','Suspend service access','Ban: registration + service access','Delete Jellyfin account(s)','Reconcile service access','Retry failed provisioning','Force logout / revoke sessions','Sync payment state'])assert(!bulk.includes(`'${retired}'`),`retired bulk action returned: ${retired}`);
assert(/registerHandler\('add_plan'/.test(operatorBulkOperations)&&/manualEntitlement\.createManualGrant/.test(operatorBulkOperations),'bulk Add plan must use the canonical manual entitlement grant path');
assert(/registerHandler\('cancel_plan'/.test(operatorBulkOperations)&&/subscriptionRevoke\.revokeSelected/.test(operatorBulkOperations),'bulk Cancel Plan must use the targeted subscription revoke lifecycle');
assert(/registerHandler\('force_server'/.test(operatorBulkOperations)&&/forceAccess\.forceAccess/.test(operatorBulkOperations)&&/persistentAdminOverride:true/.test(operatorBulkOperations)&&/automationProtected:true/.test(operatorBulkOperations),'bulk Force adding to server must use the persistent break-glass access path');
