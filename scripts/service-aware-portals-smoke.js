...
assert(/Create your private installation link/.test(customerAccessJs)&&/Keep this link private/.test(customerAccessJs)&&/Installation manifest/.test(customerAccessJs),'My Access Stremio surface must explain the private installation delivery model safely');
...
assert(/@media\(min-width:901px\)[\s\S]*\.compactFreePlan\{display:grid!important;[\s\S]*grid-template-areas:"top price description features actions"/.test(customerDashboardCss),'desktop Free Server plan must remain a short horizontal row instead of falling back to the tall generic plan card');
