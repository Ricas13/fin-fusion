...
assert(/CREATE TABLE public\.plan_prices \(/.test(migration),'Migration must create per-currency logical-plan prices');
assert(/CONSTRAINT plan_prices_plan_id_currency_key UNIQUE \(plan_id, currency\)/.test(migration),'A logical plan may have at most one price per currency');
assert(/CREATE TABLE public\.plan_provider_prices \([\s\S]*?plan_price_id uuid NOT NULL,/.test(migration),'Provider mappings must belong to a price variant');
assert(/preferred_currency character\(3\)/.test(migration),'Users must have an independent reporting/display currency preference');
assert(/preferred_currency = ANY \(ARRAY\['GBP'::bpchar, 'USD'::bpchar, 'EUR'::bpchar\]\)/.test(migration),'Only GBP/USD/EUR are supported');
assert(/FOREIGN KEY \(plan_price_id, plan_id\) REFERENCES public\.plan_prices\(id, plan_id\)/.test(integrity),'Provider mapping must not point at another logical plan price');
...
assert(/'customer\.subscription\.requested', false, false, NULL, '[^']*', false, false, 'both', false,/.test(transactional),'Mandatory customer acknowledgement events must stay customer-addressable without becoming optional customer toggles');
