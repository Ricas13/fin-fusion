...
assert(controlCenter.nextAdvertLabel(advertCfg,{channelId:'123456789012345678',lastAdvertSlot:'2026-09-18T00:00'},new Date('2026-09-18T15:00:00+01:00'),{pending:true}).includes('Due now'),'A publishable buffered increase must show the current unprocessed advert slot as due');
assert(controlCenter.nextAdvertLabel(advertCfg,{channelId:'123456789012345678',lastAdvertSlot:'2026-09-18T00:00'},new Date('2026-09-18T15:00:00+01:00'),{pending:false}).includes('00:00 tomorrow'),'An unprocessed scheduler slot with no buffered increase must not masquerade as an advert');
assert(controlCenter.nextAdvertLabel(advertCfg,{channelId:'123456789012345678',lastAdvertSlot:null},new Date('2026-09-18T15:00:00+01:00'),{pending:true}).includes('00:00 tomorrow'),'A fresh or legacy digest baseline must not falsely claim the current slot is due for an advert');
assert(controlCenter.nextAdvertLabel(advertCfg,{channelId:'',lastAdvertSlot:'2026-09-18T00:00'},new Date('2026-09-18T15:00:00+01:00'),{pending:true}).includes('00:00 tomorrow'),'A stored digest with no channel identity must be treated as stale because the worker will reset it');
assert(controlCenter.nextAdvertLabel({...advertCfg,discordFreePlacesChannelId:'999999999999999999'},{channelId:'123456789012345678',lastAdvertSlot:'2026-09-18T00:00'},new Date('2026-09-18T15:00:00+01:00'),{pending:true}).includes('00:00 tomorrow'),'Changing the advert channel must invalidate the old slot baseline rather than advertising immediately');
