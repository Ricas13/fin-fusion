...
assert(/GRANT SELECT\(id,name,code,streams,active,service_type,is_free_tier,is_addon,kick_4k_transcodes,jellyfin_access_model,jellyfin_household_network_limit,jellyfin_household_lease_minutes\) ON plans TO \$\{role\}/.test(roleScript), 'activity role must read lane fields plus the plan name and 4K enforcement switch used by the 4K worker');
