...
assert(/GRANT SELECT\(id,customer_id,plan_id,status,source,billing_mode,provider_subscription_id,current_period_end,created_at,starts_at,superseded_by,service_extension_days,service_type_snapshot,commercial_snapshot\) ON subscriptions TO \$\{role\}/.test(roleScript), 'activity role must read contractual lane fields plus canonical billing mode; provider identity remains transport metadata');
assert(/GRANT SELECT\(id,name,code,streams,active,service_type,is_free_tier,is_addon,jellyfin_access_model,jellyfin_household_network_limit,jellyfin_household_lease_minutes,kick_4k_transcodes\) ON plans TO \$\{role\}/.test(roleScript), 'activity role must read lane, household and 4K enforcement plan fields');
...
assert(roleScript.includes("to_regclass('public.customer_household_overrides') AS household_overrides"), 'activity role bootstrap must require the household override table before granting runtime access');
assert(/GRANT SELECT\(customer_id,service,network_limit\) ON customer_household_overrides TO \$\{role\}/.test(roleScript), 'activity role must read only the household override columns required by the Jellyfin household policy');
assert(!/GRANT (?:SELECT,)?INSERT[^\n]*customer_household_overrides TO \$\{role\}/.test(roleScript) && !/GRANT [^\n]*UPDATE[^\n]*customer_household_overrides TO \$\{role\}/.test(roleScript) && !/GRANT [^\n]*DELETE[^\n]*customer_household_overrides TO \$\{role\}/.test(roleScript), 'activity role must never mutate customer household overrides');
