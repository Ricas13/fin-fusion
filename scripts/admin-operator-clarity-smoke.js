...
assert(stableCustomerNav.includes("['overview','Overview'") && stableCustomerNav.includes("['history','History'"), 'Customer 360 must expose one stable customer-scoped navigation from Overview through History');
assert(!stableCustomerNav.includes("['manage','Manage'"), 'Manage must no longer be a separate persistent nav tab now that /manage redirects into the Access tab');
