...
assert.ok(openPage.includes('href="/account/register?intent=free">Start Free Access signup'));
assert.ok(!openPage.includes('method="post" action="/account/register"'),'anonymous storefront must not POST or create a session just to start Free signup');
assert.ok(!openPage.includes('name="reserveFree" value="1"'),'Free intent mutation belongs on the registration flow, not the storefront GET');
assert.ok(!openPage.includes('name="_csrf"'),'anonymous storefront must not render a CSRF field');
assert.ok(!openPage.includes('csrf-test'),'rendering the anonymous storefront must not consume the supplied CSRF token');
...
assert.ok(captchaPage.includes('https://challenges.cloudflare.com/turnstile/v0/api.js'),'Turnstile script may be preloaded when public registration protection is enabled');
assert.ok(!captchaPage.includes('class="cf-turnstile"'),'storefront must not render the registration Turnstile widget after the Free CTA became a GET link');
assert.ok(!captchaPage.includes('data-sitekey="site-key"'));
assert.ok(!captchaPage.includes('data-action="customer_registration"'));
assert.ok(captchaPage.includes('href="/account/register?intent=free">Start Free Access signup'));
