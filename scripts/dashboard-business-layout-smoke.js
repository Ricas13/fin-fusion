...
assert(dashboard.includes('managed customers / configured user capacity')&&dashboard.includes('controlCenter.automationHeroCard')&&!dashboard.includes('Needs attention'),'Dashboard hero must keep unique capacity and automation signals while leaving Needs Attention to the persistent Alerts header');
...
assert(growthData.includes('GREATEST(0,EXTRACT(EPOCH FROM (')&&growthData.includes('LEAST(r.ended_at,gs+INTERVAL')&&growthData.includes('GREATEST(r.started_at,gs,$1::timestamptz)')&&growthData.includes('avg_concurrent')&&growthData.includes('bucket_seconds'),'stream history must use duration-weighted playback overlap instead of pretending session starts equal concurrency');
