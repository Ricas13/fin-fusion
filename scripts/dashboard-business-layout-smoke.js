...
for(const widget of ['cashFlow','newVsCancelled','serviceMix'])assert(main.includes(`registry.register('main','${widget}'`)&&main.includes(`registry.register('main','${widget}'`)&&main.match(new RegExp(`registry\\.register\\('main','${widget}'[\\s\\S]*?defaultSpan:4`)),`Main dashboard widget ${widget} must default to one third of the 12-column body grid`);
