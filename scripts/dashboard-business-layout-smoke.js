...
assert(growthData.includes("LIKE 'sub\\_%'")&&growthData.includes("LIKE 'I-%'"),'MRR trend must include only verified recurring Stripe/PayPal contract identities');
assert(growthData.includes('LEAST(COALESCE(ph.ended_at,ph.last_seen_at),b.bucket_end)')&&growthData.includes('avg_concurrent'),'stream history must use playback overlap time instead of pretending session starts equal concurrency');
