...
assert(!/marketing\/withdraw|name=\"marketingOptIn\"|Marketing consent|marketingConsentChanged/.test(customer),'Customer 360 must not retain retired Marketing controls.');
...
assert(pageKeys('people').includes('users'),'People navigation must expose Customers.');
assert(nav.hiddenPages['jellyfin-import']?.groupKey==='jellyfin'&&nav.hiddenPages['jellyfin-import']?.parentKey==='jellyfin-customers','Jellyfin import must remain owned by the Jellyfin customer workflow.');
assert(!pageKeys('people').includes('invitations'),'Retired Invitations must not return to People navigation');
assert(nav.hiddenPages['customer-claims']?.groupKey==='people'&&nav.hiddenPages['customer-claims']?.parentKey==='users','Imported-user claims must remain subordinate to Customers.');
assert(pageKeys('jellyfin').includes('servers')&&pageKeys('jellyfin').includes('activity'),'Jellyfin navigation must foreground managed servers and playback health');
assert(pageKeys('stremio').includes('stremio-sources')&&pageKeys('stremio').includes('stremio-playback'),'Stremio navigation must foreground external sources and playback health');
...
assert(nav.hiddenPages['request-plan-limits']?.groupKey==='commerce'&&nav.hiddenPages['request-plan-limits']?.parentKey==='commerce-overview','Request limits must stay with Commerce plan policy.');
assert(pageKeys('commerce').includes('referrals'),'Affiliates must remain directly discoverable from Commerce.');
assert(nav.hiddenPages['fleet-operations']?.groupKey==='jellyfin'&&nav.hiddenPages['fleet-operations']?.parentKey==='fleet-operations'&&!pageKeys('jellyfin').includes('fleet-operations'),'Fleet operations must stay as a hidden Jellyfin workflow page.');
...
assert(/data-operator-alerts/.test(adminShell)&&/topStatusLink/.test(adminShell)&&/operator-business-indicators\.js/.test(adminHtml),'Admin shell must expose top-right status and alerts');
