...
assert(nav.hiddenPages['jellyfin-import']?.groupKey==='people'&&nav.hiddenPages['jellyfin-import']?.parentKey==='users','Jellyfin Import must remain subordinate to the canonical Customers workflow');
assert(nav.hiddenPages['customer-claims']?.groupKey==='people'&&nav.hiddenPages['customer-claims']?.parentKey==='users'&&operatorExperience.includes("['Import Jellyfin users','/admin/jellyfin-import'],['Portal claims','/admin/customer-claims']"),'Imported-user claims must remain routable from Customers with Jellyfin Import workflow cards');
...
assert(nav.hiddenPages['request-service']?.groupKey==='settings'&&nav.hiddenPages['request-service']?.parentKey==='settings-integrations','Request service connection settings must belong to Settings → Connections');
...
assert(!pageKeys('jellyfin').includes('fleet-operations')&&!nav.hiddenPages['fleet-operations']&&nav.activeKey('fleet-operations')==='servers','Legacy Fleet operations context must resolve directly to unified Servers without consuming a separate Jellyfin workflow');
...
assert(/Connections/.test(integrationTabs)&&/Request service/.test(integrationTabs)&&!/Plan limits/.test(integrationTabs),'Connections workflow must own integration settings, not plan-level request policy');
...
assert(pageKeys('settings').includes('settings-commerce')&&group('settings').pages.find(item=>item[0]==='settings-commerce')?.[2]==='/admin/settings?section=commerce','Settings must own a real Commerce section instead of linking out to the Commerce dashboard');
...
assert(/createAdminSettingsCommerceRouter/.test(routeComposition)&&routeComposition.indexOf('createAdminSettingsCommerceRouter()')<routeComposition.indexOf('createAdminOriginalSettingsRouter()'),'Settings Commerce must intercept section=commerce before the legacy settings redirect');
