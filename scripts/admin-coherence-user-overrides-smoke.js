...
assert(accessCards.includes("bulkPreviewForm(token,customerId,'migrate_server','Move server (guided)'"),'controlled server move shortcut missing');
...
assert(/\/admin\/api\/operator-state\/unread/.test(operatorBusiness)&&/signalMenuMarkup\(\{key:'alerts',tone:'alert',label:'Alerts',headLabel:'Operational alerts',primaryHref:'\/admin\/attention'/.test(operatorBusiness)&&/key:'attention',label:'Attention',meta:'Unacknowledged items — acknowledge after review',href:'\/admin\/attention'/.test(operatorBusiness)&&/key:'payments',label:'Payments',meta:'New provider callback issues — clears after review',href:'\/admin\/payments',business:true/.test(operatorBusiness),'Admin shell must surface persistent Needs Attention items while making provider callback notifications reviewable');
...
assert(/Customer control/.test(accessCards)&&/plan_change/.test(accessCards)&&/set_expiry/.test(accessCards),'Customer 360 must expose direct safe customer controls');
