...
assert(nav.hiddenPages['jellyfin-import']?.groupKey==='people'&&nav.hiddenPages['jellyfin-import']?.parentKey==='users','Jellyfin Import must remain subordinate to the canonical Customers → All customers workflow');
assert(nav.hiddenPages['customer-claims']?.groupKey==='people'&&nav.hiddenPages['customer-claims']?.parentKey==='users'&&operatorExperience.includes("['Import Jellyfin users','/admin/jellyfin-import'],['Portal claims','/admin/customer-claims']"),'Imported-user claims must remain routable from Customers with Jellyfin Import workflow tabs');
assert(pageKeys('stremio').includes('stremio-sources')&&pageKeys('jellyfin').includes('activity'),'Product navigation must foreground Stremio sources and Jellyfin playback health');
...
assert(nav.hiddenPages['request-plan-limits']?.groupKey==='commerce'&&nav.hiddenPages['request-plan-limits']?.parentKey==='plans','Request limits must belong to Commerce plan policy, not integration settings');
assert(pageKeys('commerce').includes('referrals'),'Affiliates must stay discoverable in Commerce navigation');
assert(pageKeys('jellyfin').includes('fleet-operations')&&!nav.hiddenPages['fleet-operations'],'Fleet operations must live as a visible Jellyfin workflow, not a hidden Operations page');
assert(!/Request service|Plan limits/.test(provisioningTabs),'Provisioning tabs must not contain Overseerr configuration');
assert(/Integrations/.test(integrationTabs)&&/Request service/.test(integrationTabs)&&!/Plan limits/.test(integrationTabs),'Integrations workflow must own connection settings, not Commerce plan policy');
