...
assert(planControl.includes('.planControlGrid{display:grid;grid-template-columns:1fr!important')&&planControl.includes('@media(min-width:1100px)')&&planControl.includes('repeat(2,minmax(0,1fr))'), 'plan control room must own the one-column default and max-two-column desktop layout');
assert(planControl.includes('.planControlGrid>#access,.planControlGrid>#availability{grid-column:auto!important}')&&planControl.includes('.planConfigCard.span2,.planConfigCard.span3{grid-column:1/-1!important}'), 'plan control room must allow only the Access/Availability peer pair while legacy spans stay full width');
