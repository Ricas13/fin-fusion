...
assert((dashboard.includes('stremioManifestUrl')||access.includes('data-stremio-manifest'))&&access.includes('manifest.json'),'Stremio onboarding must expose the private manifest on the customer account or My Access surface without depending on brittle markup details');
assert(dashboard.includes('Install in Stremio')||access.includes('Install in Stremio'),'Stremio onboarding must preserve the primary install action');
assert(dashboard.includes('Keep this link private.')||access.includes('Keep it private'),'Stremio onboarding must preserve the private-link warning');
...
(async()=>{
  ...
  assert(sent[0].text.includes('3 free places currently available.')&&sent[0].text.split('\n').some(line=>line==='Reserve / Create Free Account: https://portal.example/account/register?intent=free')&&sent[0].text.includes('60 minutes'));
  ...
  assert(edited[0].text.includes('No free places currently available.')&&edited[0].text.includes('60 minutes'));
