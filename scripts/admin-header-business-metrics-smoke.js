...
assert(ledger.includes("const classifier = require('./provider-transaction-classifier');")&&ledger.includes('const kind = classifier.historyKind(row);')&&ledger.includes('if (kind) await visit(historyRecord(row, kind));'),'history rows must enter streaming accounting only through the canonical provider transaction classifier');
