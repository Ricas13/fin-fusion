...
assert(!lifecycle.includes('minimumPlaybackMinutes') && !lifecycle.includes('playbackWindowDays') && !lifecycle.includes('Minimum observation'), 'per-plan Jellyfin lifecycle editor must not restore configurable inactivity usage rules');
assert(lifecycle.includes('Inactivity removal follows the global Free Server policy.'), 'Free Jellyfin plan editor must direct inactivity behavior to the global Free Server policy');
...
assert(operatorBusiness.includes('setTimeout(()=>refresh().catch(()=>{}),80)') && operatorBusiness.includes('setInterval(()=>refresh().catch(()=>{}),15000)'), 'business unread badges must repaint from a fresh server snapshot on load and periodically; read acknowledgement now happens server-side on page render, not via a racy client-side fetch');
