...
assert(operatorBusiness.includes('markCurrentAreaRead(data)') && operatorBusiness.includes('markCurrentAreaReadWithRetry') && operatorBusiness.includes('return await fetchSnapshot()') && operatorBusiness.includes('.then(fresh=>apply(fresh||data))'), 'business unread badges must persist the current-area acknowledgement, retry transient failures and repaint from a fresh server snapshot');
assert(!operatorBusiness.includes('data.counts[areaForCurrentPage]=0'), 'business unread badges must remain server-authoritative rather than being cleared locally');
