...
assert(operatorBusiness.includes('markCurrentAreaRead(data)') && operatorBusiness.includes('.then(()=>fetchSnapshot())') && operatorBusiness.includes('.then(fresh=>apply(fresh||data))'), 'business unread badges must persist the current-area acknowledgement and repaint from a fresh server snapshot');
