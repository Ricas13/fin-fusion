...
assert.match(base,/LEAST\(COALESCE\(ph\.ended_at,ph\.last_seen_at\),NOW\(\)\)/,'rolling playback must clip each session at its observed end/current time');
assert.match(base,/ph\.started_at>=allocation\.allocation_start_at/,'rolling playback must never count a session that began before the current Free allocation');
assert.match(base,/GREATEST\(ph\.started_at,NOW\(\)-/,'rolling playback may clip only the rolling-window boundary after allocation scoping is satisfied');
...
assert.match(scoped,/COALESCE\(ended_at,last_seen_at\) > NOW\(\) - \(\$3::int \* INTERVAL '1 day'\)/,'the last-second usage safety check must include sessions that overlap the rolling window even if they began before it');
assert.match(scoped,/LEAST\(COALESCE\(ended_at,last_seen_at\),NOW\(\)\)/,'the last-second usage safety check must clip overlapping session duration at the current time');
