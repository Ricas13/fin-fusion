...
const server = http.createServer(async (req,res)=>{
    try {
        ...
        if(userRoute&&req.method==='GET'){
            if(remote.beforeUserGet){const hook=remote.beforeUserGet;remote.beforeUserGet=null;await hook(Number(userRoute[1]));}
        ...
        if(userRoute&&req.method==='DELETE'){
            ...
            if(remote.afterDelete){const hook=remote.afterDelete;remote.afterDelete=null;await hook(deleted.id);}
            res.writeHead(204);
            return res.end();
        }
...
(async()=>{
    ...
    const deleteRenewCustomer=await makeCustomer({username:'DeleteRenewRace',email:'delete-renew-race@example.test',serverIds:[firstServer],planId});await requestSync.syncOneCustomer(deleteRenewCustomer);const deleteRenewAccess=await requestSync.requestAccessForCustomer(deleteRenewCustomer),deleteRenewOldId=Number(deleteRenewAccess.external_user_id);await query(`UPDATE subscriptions SET current_period_end=NOW()-INTERVAL '1 minute',status='expired' WHERE customer_id=$1`,[deleteRenewCustomer]);const deleteRenewCreateCount=remote.createCalls,deleteRenewDeleteCount=remote.deleteCalls.length;remote.afterDelete=async id=>{if(Number(id)===deleteRenewOldId)await query(`UPDATE subscriptions SET current_period_end=NOW()+INTERVAL '30 days',status='active' WHERE customer_id=$1`,[deleteRenewCustomer]);};const deleteRenewResult=await requestSync.syncOneCustomer(deleteRenewCustomer);assert.strictEqual(deleteRenewResult.status,'synced','entitlement restored during DELETE must be compensated immediately');assert.strictEqual(deleteRenewResult.recoveredAfterDelete,true,'post-delete compensation must be explicit in the sync result');assert.strictEqual(remote.deleteCalls.length,deleteRenewDeleteCount+1,'the stale account may already have been deleted before renewal becomes visible');assert.strictEqual(remote.createCalls,deleteRenewCreateCount+1,'post-delete renewal must immediately create a fresh Seerr account');assert.strictEqual(userById(deleteRenewOldId),undefined,'the stale remote id must stay deleted after compensation');const deleteRenewState=await requestSync.requestAccessForCustomer(deleteRenewCustomer);assert(deleteRenewState.external_user_id,'post-delete renewal must persist the replacement Seerr binding');assert.notStrictEqual(Number(deleteRenewState.external_user_id),deleteRenewOldId,'post-delete renewal must bind to a fresh Seerr id');assert.strictEqual(deleteRenewState.access_suspended,false,'post-delete renewal must finish with active request access');assert(userById(deleteRenewState.external_user_id),'replacement Seerr account must exist after post-delete compensation');

    const missingRaceCustomer=await makeCustomer({username:'MissingRace',email:'missing-race@example.test',serverIds:[firstServer],planId});await requestSync.syncOneCustomer(missingRaceCustomer);const missingRaceAccess=await requestSync.requestAccessForCustomer(missingRaceCustomer),missingRaceOldId=Number(missingRaceAccess.external_user_id);const missingRaceOldIndex=remote.users.findIndex(user=>Number(user.id)===missingRaceOldId);assert(missingRaceOldIndex>=0);remote.users.splice(missingRaceOldIndex,1);const missingRaceReplacement={id:8801,email:'missing-race-replacement',username:'MissingRaceReplacement',permissions:32,settings:{username:'MissingRaceReplacement',email:'missing-race-replacement',locale:'en',movieQuotaLimit:0,movieQuotaDays:30,tvQuotaLimit:0,tvQuotaDays:30}};remote.users.push(missingRaceReplacement);await query(`UPDATE subscriptions SET current_period_end=NOW()-INTERVAL '1 minute',status='expired' WHERE customer_id=$1`,[missingRaceCustomer]);remote.beforeUserGet=async id=>{if(id===missingRaceOldId)await query(`UPDATE request_user_sync SET external_user_id=$2,external_email=$3,external_username=$4,status='synced',access_suspended=FALSE,updated_at=NOW() WHERE customer_id=$1`,[missingRaceCustomer,missingRaceReplacement.id,missingRaceReplacement.email,missingRaceReplacement.username]);};const missingRaceResult=await requestSync.syncOneCustomer(missingRaceCustomer);assert.strictEqual(missingRaceResult.status,'failed','404 cleanup must fail closed when the binding changes after the candidate snapshot');assert.match(missingRaceResult.error,/binding changed while clearing missing Seerr account/);assert.match(missingRaceResult.error,/newer binding was left untouched/);const missingRaceState=await requestSync.requestAccessForCustomer(missingRaceCustomer);assert.strictEqual(Number(missingRaceState.external_user_id),missingRaceReplacement.id,'404 cleanup must not clear a newer replacement binding');assert.strictEqual(missingRaceState.external_email,missingRaceReplacement.email);assert.strictEqual(missingRaceState.status,'synced','stale cleanup failure must not mark the newer binding failed');assert.strictEqual(userById(missingRaceReplacement.id),missingRaceReplacement,'newer replacement remote must remain untouched');

    const deleteRaceCustomer=await makeCustomer({username:'DeleteRace',email:'delete-race@example.test',serverIds:[firstServer],planId});await requestSync.syncOneCustomer(deleteRaceCustomer);const deleteRaceAccess=await requestSync.requestAccessForCustomer(deleteRaceCustomer),deleteRaceOldId=Number(deleteRaceAccess.external_user_id);const deleteRaceReplacement={id:8802,email:'delete-race-replacement',username:'DeleteRaceReplacement',permissions:32,settings:{username:'DeleteRaceReplacement',email:'delete-race-replacement',locale:'en',movieQuotaLimit:0,movieQuotaDays:30,tvQuotaLimit:0,tvQuotaDays:30}};remote.users.push(deleteRaceReplacement);await query(`UPDATE subscriptions SET current_period_end=NOW()-INTERVAL '1 minute',status='expired' WHERE customer_id=$1`,[deleteRaceCustomer]);remote.afterDelete=async id=>{if(Number(id)===deleteRaceOldId)await query(`UPDATE request_user_sync SET external_user_id=$2,external_email=$3,external_username=$4,status='synced',access_suspended=FALSE,updated_at=NOW() WHERE customer_id=$1`,[deleteRaceCustomer,deleteRaceReplacement.id,deleteRaceReplacement.email,deleteRaceReplacement.username]);};const deleteRaceResult=await requestSync.syncOneCustomer(deleteRaceCustomer);assert.strictEqual(deleteRaceResult.status,'failed','post-delete rebinding must fail local finalization without overwriting the replacement');assert.match(deleteRaceResult.error,/binding changed while deleting Seerr account/);assert.match(deleteRaceResult.error,/newer binding was left untouched/);assert.strictEqual(userById(deleteRaceOldId),undefined,'the old remote may already be deleted before the concurrent rebinding is observed');assert(remote.deleteCalls.includes(deleteRaceOldId));const deleteRaceState=await requestSync.requestAccessForCustomer(deleteRaceCustomer);assert.strictEqual(Number(deleteRaceState.external_user_id),deleteRaceReplacement.id,'cleanup failure must never resurrect the deleted external id over a newer binding');assert.strictEqual(deleteRaceState.external_email,deleteRaceReplacement.email);assert.strictEqual(deleteRaceState.status,'synced','stale cleanup failure must leave the newer binding status untouched');assert.strictEqual(userById(deleteRaceReplacement.id),deleteRaceReplacement,'replacement remote must remain present');
...
})().finally(async()=>{await closeServer().catch(()=>{});await getPool().end();});
