...
})().finally(async()=>{await closeServer().catch(()=>{});await getPool().end();}).catch(error=>{const safe=String(error?.stack||error||'request user sync smoke failed').replace(/[\r\n]+/g,' ');console.error('request user sync smoke failed: %s',safe);process.exitCode=1;});
