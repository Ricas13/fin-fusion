...
assert(migration.includes('schedule_next_attempt_at timestamp with time zone'),'campaign retry watermark missing');
...
assert(campaigns.includes("status='scheduled' AND COALESCE(schedule_next_attempt_at,scheduled_for)<=NOW()"),'worker due query must be database-time authoritative');
assert(campaigns.includes("schedule_next_attempt_at=NOW()+INTERVAL '5 minutes'"),'failed scheduled campaigns must back off before retry');
