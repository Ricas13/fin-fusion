...
assert.deepStrictEqual(pageKeys.jellyfin,['servers','fleet-operations','activity'],'Jellyfin must lead directly to its real operational control rooms; shared Plans and Customers belong elsewhere');
assert.deepStrictEqual(pageKeys.stremio,['stremio-sources','stremio-playback'],'Stremio must lead directly to Sources and IP access without a redundant overview');
...
assert.strictEqual(nav.activeKey('stremio-overview'),'stremio-sources','legacy Stremio overview context must resolve to Sources');
...
assert(productModules.includes("router.get('/admin/stremio',(_req,res)=>res.redirect('/admin/servers/stremio'))"),'Legacy Stremio overview URL must redirect to the canonical Sources control room');
