...
assert(/sessions: activeSessions\.filter\(session => session\.userId === userId\)/.test(laneStream), 'safety snapshot must revalidate the exact Jellyfin user while retaining the server-wide active-session snapshot');
assert(/allSessions: activeSessions/.test(laneStream)&&/verified\.allSessions \|\| verified\.sessions/.test(laneStream), 'device fallback must protect other active identities sharing the device');
