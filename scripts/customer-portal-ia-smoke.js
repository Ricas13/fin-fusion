...
assert(nav.includes('/account/access')&&nav.includes("active==='access'"),'My Access must be a first-class customer navigation destination');
...
assert(!nav.includes('>Service passwords</a>')&&!nav.includes('href="/account/service-passwords"'),'service passwords must not create a separate customer navigation destination');
assert(nav.includes('data-customer-now-playing')&&nav.includes('/js/customer-now-playing.js')&&nav.includes('/js/customer-home-access.js'),'the shared signed-in navigation must own the live-stream strip and Home access-summary enhancement');
...
assert(!dashboard.includes('id="jellyfin-onboarding"')&&!dashboard.includes('id="jellyfin-access"')&&homeAccessClient.includes("location.assign('/account/access')"),'Home must omit duplicate Jellyfin setup/server sections server-side and make active access the My Access shortcut');
...
assert(accessRoute.includes("router.post('/account/access/media/:accountId/username'")&&accessRoute.includes('renameJellyfinAccount(req.session.customerId'),'My Access must expose a CSRF-protected media username rename route.');
...
assert(dashboard.includes('can_rename_jellyfin_username')&&dashboard.includes('Change username')&&dashboard.includes('/account/jellyfin/${esc(a.id)}/username'),'legacy Home markup must retain compatible username controls until the Home service blocks are removed server-side');
...
assert(customerNavigationCss.includes('.customerNowPlaying'),'the shared shell must style the live-session strip');
...
assert(stremioRoute.includes("r.get('/account/stremio',(req,res)=>res.redirect(302,'/account/access#stremio-access'))"),'legacy Stremio setup URL must redirect to the canonical My Access Stremio section');
...
assert(pending.includes('FREE_HOLD_MINUTES=60')&&pending.includes('async function reserveFreeAccess')&&pending.includes('free_access_registration_reservations'),'explicit Free Access reservation must create a real 60-minute expiring capacity hold before registration details are entered');
