...
assert(runtimeLegacy.includes("const librarySelectionLimit = routeRateLimit.middleware({scope:'customer-library-selection',max:20,windowSeconds:300})")&&runtimeLegacy.includes("router.post('/account/libraries', requireCustomer, librarySelectionLimit")&&runtimeLegacy.includes("router.post('/account/libraries/:accountId', requireCustomer, librarySelectionLimit"),'customer library selection mutations must retain route-level throttling');
assert(runtimeLegacy.includes("if (req.path.startsWith('/account/')) return res.redirect('/account?error='") ,'legacy signed-in customer errors must return to the account shell rather than rendering the generic message page');
