...
assert(generatedNav.includes('showAccess:hasServiceAccess')&&generatedNav.includes('liveRequestEntitlement(portal)'),'My Access visibility must follow any current non-addon service entitlement');
...
assert(homeAccessClient.includes("document.getElementById('jellyfin-onboarding')?.remove()")&&homeAccessClient.includes("document.getElementById('jellyfin-access')?.remove()")&&homeAccessClient.includes("location.assign('/account/access')"),'Home must remove duplicate Jellyfin setup/server sections and make active access the My Access shortcut');
...
assert(accessRoute.includes('.filter(customerNav.liveSubscription)')&&accessView.includes('subscriptions.forEach(function(subscription)'),'My Access must show every current subscription rather than one selected Jellyfin lane');
