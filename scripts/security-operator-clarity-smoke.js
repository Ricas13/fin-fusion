...
for(const label of ['General','Security','Connections','System'])assert(labels.includes(label),`Settings navigation is missing permanent destination ${label}.`);
assert(!labels.includes('Commerce'),'Commerce settings must be parent-owned by Commerce → Plans rather than a fifth Settings rail item.');
assert(navModel.hiddenPages['settings-commerce']?.parentKey==='plans'&&navModel.hiddenPages['settings-commerce']?.kind==='setting','Commerce settings must remain routable as a Plans-owned setting.');
...
const operationsGroup=navModel.groups.find(group=>group.key==='operations');
assert(Boolean(operationsGroup),'Operations navigation group must exist.');
assert(operationsGroup.pages.some(page=>page[0]==='backups'&&page[1]==='Backups'&&page[2]==='/admin/backups'),'Backup controls must be discoverable as Operations → Backups.');
...
assert(navModel.hiddenPages['fleet-operations']?.parentKey==='servers'&&navModel.hiddenPages.libraries?.parentKey==='servers','Placement and Libraries must remain owned by Servers → Jellyfin rather than becoming permanent rail destinations.');
assert(navModel.aliases.operations==='servers'&&!navModel.aliases['fleet-operations']&&!navModel.aliases.libraries,'Legacy Operations may resolve to Jellyfin, while durable Placement and Libraries pages keep their own contextual identity.');
assert(serversGroup.pages.some(page=>page[0]==='servers'&&page[1]==='Jellyfin'&&page[2]==='/admin/servers'),'Managed Jellyfin servers must be discoverable under Servers.');
assert(serversGroup.pages.some(page=>page[0]==='stremio-sources'&&page[1]==='Stremio'&&page[2]==='/admin/servers/stremio'),'External Jellyfin sources must remain discoverable through Servers → Stremio.');
