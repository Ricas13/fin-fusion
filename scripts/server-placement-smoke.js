...
assert(acquisition.includes("NOT (p.service_type='jellyfin' AND p.server_class IN('premium','free'))")&&acquisition.includes('fleetConfigured'),'fleet Jellyfin acquisition must fail closed instead of falling back to a manual plan limit when no server capacity is configured');
