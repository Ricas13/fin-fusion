...
function validateForPlan(plan,input){
 ...
 if(value.enabled&&(Number(plan?.price_minor||0)!==0||String(plan?.billing_interval||'')==='trial'))throw new Error('Usage-based inactivity removal is limited to free non-trial Jellyfin/bundle plans. Paid and trial access is removed when its entitlement lapses.');
 return value;
}
