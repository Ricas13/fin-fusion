...
function usageTriggered(assessment,policy){const checks=[];if(policy?.noPlaybackDays!=null)checks.push(Boolean(assessment?.noPlaybackEligible));if(policy?.minimumPlaybackMinutes!=null)checks.push(Boolean(assessment?.usageEligible));return checks.length>0&&checks.every(Boolean);}
...
module.exports={DEFAULTS,normalize,effectiveForFreePlan,hasUsageTrigger,usageTriggered,validateForPlan,getPlan,save};