...
async function liveFreeJellyfinSubscription(customerId,{client=null,includeBlocked=false}={}){
 ...
 SELECT s.*,p.*,s.id AS subscription_id,p.id AS plan_id,
        ...
        COALESCE(s.duration_days_snapshot,p.duration_days) AS contract_duration_days,
        CASE WHEN (o.permanent_access=TRUE AND o.revoked_at IS NULL AND o.subscription_id=s.id)
                  OR COALESCE(ctl.mode='admin_present',FALSE)
        ...
        COALESCE(ctl.mode='admin_present',FALSE) AS admin_present,
        COALESCE(ctl.mode='admin_removed',FALSE) AS admin_removed
 FROM subscriptions s
 JOIN plans p ON p.id=s.plan_id
 LEFT JOIN customer_entitlement_overrides o ON o.customer_id=s.customer_id AND o.subscription_id=s.id
 LEFT JOIN customer_service_admin_control ctl ON ctl.customer_id=s.customer_id AND ctl.service='jellyfin'
 WHERE s.customer_id=$1
 ...
 // Free Server has lane-specific inactivity/cleanup holds in addition to the
 // generic subscription blocker. Explicit admin-present/permanent authority
 // can still override those through the canonical admin semantics below; a
 // server pin remains placement-only and therefore preserves row.blocked.
