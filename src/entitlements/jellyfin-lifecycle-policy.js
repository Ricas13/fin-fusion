...
const DEFAULTS=Object.freeze({enabled:true,dryRun:false,freeNoPlaybackDays:7,freeDeleteAfterDisableDays:7,trialDeleteAfterDisableDays:30,paidDeleteAfterDisableDays:30});
...
function normalize(value={}){return{enabled:bool(value.enabled,DEFAULTS.enabled),dryRun:bool(value.dryRun,DEFAULTS.dryRun),freeNoPlaybackDays:days(value.freeNoPlaybackDays,DEFAULTS.freeNoPlaybackDays),freeDeleteAfterDisableDays:days(value.freeDeleteAfterDisableDays,DEFAULTS.freeDeleteAfterDisableDays),trialDeleteAfterDisableDays:days(value.trialDeleteAfterDisableDays,DEFAULTS.trialDeleteAfterDisableDays),paidDeleteAfterDisableDays:days(value.paidDeleteAfterDisableDays,DEFAULTS.paidDeleteAfterDisableDays)};}
...
function categoryFor({serverClass=null,billingInterval=null,priceMinor=0,source=null}={}){if(String(serverClass||'').toLowerCase()==='free')return'free';if(String(billingInterval||'').toLowerCase()==='trial')return'trial';return Number(priceMinor||0)>0?'paid':'free';}
function deleteDays(cfg,category,plan=null){const override=planOverride(plan);if(override!=null)return{days:override,source:'plan'};const key={free:'freeDeleteAfterDisableDays',trial:'trialDeleteAfterDisableDays',paid:'paidDeleteAfterDisableDays'}[category]||'paidDeleteAfterDisableDays';return{days:cfg[key],source:'global'};}
