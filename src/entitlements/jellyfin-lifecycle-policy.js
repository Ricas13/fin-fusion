...
function minutes(v,fallback,min=1,max=1000000){const n=Number.parseInt(v,10);return Number.isInteger(n)&&n>=min&&n<=max?n:fallback;}
function normalize(value={}){return{enabled:bool(value.enabled,DEFAULTS.enabled),dryRun:bool(value.dryRun,DEFAULTS.dryRun),freeFirstPlaybackGraceDays:days(value.freeFirstPlaybackGraceDays,DEFAULTS.freeFirstPlaybackGraceDays),freeNoPlaybackDays:days(value.freeNoPlaybackDays,DEFAULTS.freeNoPlaybackDays),freeMinimumPlaybackMinutes:minutes(value.freeMinimumPlaybackMinutes,DEFAULTS.freeMinimumPlaybackMinutes),freePlaybackWindowDays:days(value.freePlaybackWindowDays,DEFAULTS.freePlaybackWindowDays,1,365)};}
