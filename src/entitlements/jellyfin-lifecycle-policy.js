...
async function get(){
  ...
  // Missing lifecycle configuration must never silently become destructive.
  // A fresh/partially migrated deployment remains paused + dry-run until an
  // operator has explicitly saved the platform lifecycle setting.
  if(!r.rowCount)return{...normalize(DEFAULTS),...SAFE_UNCONFIGURED,configurationMissing:true};
  return{...normalize({...DEFAULTS,...(r.rows[0]?.setting_value||{})}),configurationMissing:false};
}
...
module.exports={KEY,DEFAULTS,SAFE_UNCONFIGURED,normalize,get,save,categoryFor,freeNoPlaybackDays,noPlaybackOverride};
