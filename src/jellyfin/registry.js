...
function operationError(server,method,url,timeoutMs,error){
    ...
    const wrapped=new Error(timedOut
        ? `Jellyfin ${server.name} ${verb} ${path} timed out after ${Math.round(Number(timeoutMs||10000)/1000)}s`
        : `Jellyfin ${server.name} ${verb} ${path} request failed: ${message}`);
    wrapped.code=timedOut?'JELLYFIN_TIMEOUT':'JELLYFIN_REQUEST_FAILED';
    wrapped.retryable=timedOut;
    wrapped.operation={method:verb,path,timeoutMs:Number(timeoutMs||10000)};
    wrapped.cause=error;
    return wrapped;
}

async function request(serverId,endpoint,{method='GET',body=null,timeoutMs=10000}={}){
    if(typeof endpoint!=='string'||!endpoint.startsWith('/')||endpoint.startsWith('//'))throw new Error('Invalid Jellyfin API endpoint.');
    ...
    if(url.origin!==new URL(server.base_url).origin)throw new Error('Jellyfin API endpoint escaped the configured server origin.');
    let response;
    try{
        response=await outbound.safeFetch(url,{purpose:`Jellyfin server ${server.name}`,method,timeoutMs,headers:authHeaders(server.apiKey,{jsonBody:Boolean(body)}),...(body?{body:JSON.stringify(body)}:{})});
    }catch(error){
    ...
    if(!response.ok){
        const verb=String(method||'GET').toUpperCase(),path=url.pathname||'/';
        const err=new Error(`Jellyfin ${server.name} ${verb} ${path} returned HTTP ${response.status}`);
        err.status=response.status;err.response=parsed;err.operation={method:verb,path,timeoutMs:Number(timeoutMs||10000)};
...
module.exports={normalizeBaseUrl,authHeaders,listServers,getServerSecret,request,healthcheckServer,decryptJellyfinKey,operationError};
