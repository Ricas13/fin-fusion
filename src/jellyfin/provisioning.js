...
async function expireSubscriptionsAndReconcile(){const expired=await transaction(async client=>{const rows=await client.query(`WITH expired AS (UPDATE subscriptions s SET status='expired',service_extension_days=0,updated_at=NOW() FROM customers c WHERE c.id=s.customer_id AND COALESCE(c.permanent_access,FALSE)=FALSE AND s.superseded_by IS NULL AND ((s.status IN('active','trialing','past_due','paused','cancelled') AND s.current_period_end+(COALESCE(s.service_extension_days,0)||' days')::interval<=NOW()) OR (s.status='expired' AND COALESCE(s.service_extension_days,0)>0 AND s.current_period_end+(s.service_extension_days||' days')::interval<=NOW())) RETURNING s.customer_id,s.plan_id,s.source) SELECT DISTINCT e.customer_id,BOOL_OR(p.price_minor>0) AS had_paid_expiry FROM expired e JOIN plans p ON p.id=e.plan_id GROUP BY e.customer_id`);return rows.rows});for(const row of expired){const customerId=row.customer_id;let downgraded=null;if(row.had_paid_expiry)downgraded=await maybeAutoDowngrade(customerId);if(downgraded)continue;try{await reconcileCustomer(customerId)}catch(error){console.error('Entitlement reconcile failed.',{customerId:safeLog(customerId,100),error:safeLog(error?.message||error)})}}return expired.length}
