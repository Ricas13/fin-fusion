...
async function reconcileCustomer(customerId){
  ...
  const outcome=await engine.reconcileCustomer(customerId);
...
async function reconcileAccount(accountId){const account=await query('SELECT customer_id FROM jellyfin_accounts WHERE id=$1',[accountId]);if(account.rowCount)await syncAccess(account.rows[0].customer_id);return engine.reconcileAccount(accountId)}
async function createJellyfinAccount(customerId,server,effective,options={}){const account=await engine.createJellyfinAccount(customerId,server,effective,options);if(options.passwordSetupRequired!==false){await markPasswordSetupRequired(account.id);account.password_setup_required=true;account.password_reset_required=true;}return account;}
async function setJellyfinPassword(customerId,accountId,newPassword){const result=await engine.setJellyfinPassword(customerId,accountId,newPassword);await query(`UPDATE jellyfin_accounts SET password_setup_required=FALSE,password_reset_required=FALSE,updated_at=NOW() WHERE id=$1 AND customer_id=$2`,[accountId,customerId]);return result;}
...
module.exports={...engine,currentEntitlement,reconcileCustomer,reconcileAccount,createJellyfinAccount,setJellyfinPassword,holdAccess,releaseAccess,expireSubscriptionsAndReconcile,notifyNewJellyfinAccess};
