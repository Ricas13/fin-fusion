...
async function activeSessions() {
  const result = await query(
    `SELECT aps.server_id,aps.jellyfin_session_id,aps.customer_id,aps.jellyfin_account_id,
            ...
            aps.item_name,aps.client_name,aps.device_name
     ...
     ORDER BY aps.customer_id,aps.first_seen_at,aps.server_id,aps.jellyfin_session_id`
  );
  return result.rows;
}
...
async function stopDeniedSession(candidate, household, cfg) {
  ...
  try {
    ...
    return { stopped: false, reason: 'jellyfin_stop_failed', error: error.message };
  }
}
...
async function runHouseholdNetworkCycle(options = {}) {
  ...
  for (const row of rows) {
    ...
    groups.get(row.customer_id).push(row);
  }

  const summary = { skipped: false, customers: 0, observedSessions: 0, denied: 0, stopped: 0, safetySkipped: 0, unknownNetworks: 0 };
  for (const [customerId, sessions] of groups) {
    const household = await effectiveHousehold(customerId);
    ...
    for (const session of sessions) {
      ...
      try {
        ...
        console.warn(`Jellyfin household lease failed for customer ${customerId}:`, error.message);
        continue;
      }
      ...
      summary.denied += 1;
      if (options.pollsReliable === false || cfg.effectiveMode !== 'enforce') {
        summary.safetySkipped += 1;
        continue;
      }
...
module.exports = { activeSessions, safeRemoteAddress, effectiveHousehold, claimSession, freshSession, stopDeniedSession, runHouseholdNetworkCycle };
