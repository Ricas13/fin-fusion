...
async function reconcileCustomerUnlocked(customerId){
    // Canonicalize typed/legacy hold state before entitlement selection. In
    // particular, a stale Free Server inactivity hold must not suppress a newly
    // purchased paid Jellyfin entitlement during the very reconciliation that
    // is supposed to activate it.
    await inactivityHoldReconciliation.releaseObsoleteForCustomer(customerId);
    await accessHolds.syncLegacySummary(customerId);
    const[effectiveJellyfin,freeEntitlement,stremioEntitlement,embyEntitlement]=await Promise.all([base.currentEntitlement(customerId),subscriptionState.liveFreeJellyfinSubscription(customerId,{includeBlocked:true}),require('../stremio/entitlements').entitledSubscription(customerId),subscriptionState.effectiveEmbySubscription(customerId,{includeBlocked:true})]),primaryEntitlement=effectiveJellyfin&&!effectiveJellyfin.is_free_tier?effectiveJellyfin:null,controlEntitlement=primaryEntitlement||freeEntitlement||stremioEntitlement||embyEntitlement||null,activePlanIds=[primaryEntitlement&&!primaryEntitlement.blocked?primaryEntitlement.plan_id:null,freeEntitlement&&!freeEntitlement.blocked?freeEntitlement.plan_id:null,stremioEntitlement?.plan_id||null,embyEntitlement&&!embyEntitlement.blocked?embyEntitlement.plan_id:null].filter(Boolean);
    ...
    try{
        const outcome=await recordRun(customerId,controlEntitlement?.subscription_id||null,async()=>{
            ...
            const result={active:Boolean(primary.active||free.active||stremioOutcome?.status==='active'||emby.active),account,primary,free,stremio:stremioOutcome,emby,discord};
...
module.exports={...base,reconcileCustomer,reconcileDiscordRoles,activeDiscordPlanIds,reconcileAccount,holdAccess,releaseAccess,setJellyfinPassword,renameJellyfinAccount,expireSubscriptionsAndReconcile,normalAccounts,applyPolicyIfChanged,reconcileLane,adoptExistingFreeAccount,control,libraryPolicyForAccount,setLibrarySelectionForAccount,reconciliationLock,assertDiscordSyncResult,assertLanePostcondition};
