...
function stateDetail(primaryEntitlement,outcome){const account=outcome?.account||outcome?.emby?.account||null,discordStatus=outcome?.discord?.skipped?`skipped:${outcome.discord.skipped}`:outcome?.discord?'synced':null;return{subscriptionId:primaryEntitlement?.subscription_id||outcome?.free?.entitlement?.subscription_id||outcome?.stremio?.subscriptionId||outcome?.emby?.entitlement?.subscription_id||null,planId:primaryEntitlement?.plan_id||outcome?.free?.entitlement?.plan_id||outcome?.emby?.entitlement?.plan_id||null,accountId:account?.id||null,serverId:account?.server_id||outcome?.stremio?.serverId||null,serviceType:serviceType(primaryEntitlement||outcome?.emby?.entitlement),result:{active:Boolean(outcome?.active),status:outcome?.status||null,blockers:outcome?.blockers||[],planCode:primaryEntitlement?.contract_plan_code||primaryEntitlement?.code||outcome?.emby?.entitlement?.contract_plan_code||outcome?.emby?.entitlement?.code||null,jellyfinAccountId:outcome?.account?.id||null,embyAccountId:outcome?.emby?.account?.id||null,serverId:account?.server_id||outcome?.stremio?.serverId||null,primary:laneState(outcome?.primary),free:laneState(outcome?.free),emby:laneState(outcome?.emby),stremioStatus:outcome?.stremio?.status||null,discordStatus,reconciledAt:new Date().toISOString()}};}
...
function assertLanePostcondition(name,entitlement,result){if(!entitlement||entitlement.blocked)return;if(result?.active&&result?.account&&!result.account.disabled)return;const error=new Error(`${name} Jellyfin reconciliation did not converge to an enabled account.`);error.code='RECONCILIATION_POSTCONDITION_FAILED';error.lane=name.toLowerCase();throw error;}
function blockerState(holds){return(holds||[]).map(hold=>({id:hold.id,type:hold.hold_type,sourceKey:hold.source_key||null,reason:hold.reason||null,createdAt:hold.created_at||null}));}
async function currentEntitlementTruth(customerId){return subscriptionState.effectiveSubscription(customerId,{includeBlocked:true});}

async function normalAccounts(customerId){const rows=await query(`SELECT ja.*,js.enabled AS server_enabled,js.server_class,js.name AS server_name,js.public_url FROM jellyfin_accounts ja JOIN jellyfin_servers js ON js.id=ja.server_id WHERE ja.customer_id=$1 AND ja.account_purpose='jellyfin' AND COALESCE(js.media_server_type,'jellyfin')='jellyfin' ORDER BY ja.is_primary DESC,ja.disabled ASC,ja.created_at ASC`,[customerId]);return rows.rows;}
...
async function libraryPolicyForAccount(customerId,accountOrId,entitlementOverride=null){
    ...
    if(!account||typeof account!=='object'){
        const found=await query(`SELECT ja.*,js.enabled AS server_enabled,js.server_class,js.name AS server_name,js.public_url FROM jellyfin_accounts ja JOIN jellyfin_servers js ON js.id=ja.server_id WHERE ja.id=$1 AND ja.customer_id=$2 AND ja.account_purpose='jellyfin' AND COALESCE(js.media_server_type,'jellyfin')='jellyfin' LIMIT 1`,[accountOrId,customerId]);
...
async function activeDiscordPlanIds(customerId){const[effectiveJellyfin,freeEntitlement,stremioEntitlement,embyEntitlement]=await Promise.all([currentEntitlementTruth(customerId),subscriptionState.liveFreeJellyfinSubscription(customerId,{includeBlocked:true}),require('../stremio/entitlements').entitledSubscription(customerId),subscriptionState.effectiveEmbySubscription(customerId,{includeBlocked:true})]),primaryEntitlement=effectiveJellyfin&&!effectiveJellyfin.is_free_tier?effectiveJellyfin:null;return[primaryEntitlement&&!primaryEntitlement.blocked?primaryEntitlement.plan_id:null,freeEntitlement&&!freeEntitlement.blocked?freeEntitlement.plan_id:null,stremioEntitlement?.plan_id||null,embyEntitlement&&!embyEntitlement.blocked?embyEntitlement.plan_id:null].filter(Boolean);}
async function reconcileDiscordRoles(customerId,{discordExtraManagedRoleIds=[]}={}){return reconciliationLock.withCustomerReconciliationLock(customerId,async()=>{const activePlanIds=await activeDiscordPlanIds(customerId);return assertDiscordSyncResult(await discordRoles.syncRoleForCustomer(customerId,activePlanIds,{extraManagedRoleIds:discordExtraManagedRoleIds}));});}
...
async function reconcileCustomerUnlocked(customerId){
    ...
    const[effectiveJellyfin,freeEntitlement,stremioEntitlement,embyEntitlement,holds]=await Promise.all([currentEntitlementTruth(customerId),subscriptionState.liveFreeJellyfinSubscription(customerId,{includeBlocked:true}),require('../stremio/entitlements').entitledSubscription(customerId),subscriptionState.effectiveEmbySubscription(customerId,{includeBlocked:true}),accessHolds.activeHolds(customerId)]),primaryEntitlement=effectiveJellyfin&&!effectiveJellyfin.is_free_tier?effectiveJellyfin:null,controlEntitlement=primaryEntitlement||freeEntitlement||stremioEntitlement||embyEntitlement||null,activePlanIds=[primaryEntitlement&&!primaryEntitlement.blocked?primaryEntitlement.plan_id:null,freeEntitlement&&!freeEntitlement.blocked?freeEntitlement.plan_id:null,stremioEntitlement?.plan_id||null,embyEntitlement&&!embyEntitlement.blocked?embyEntitlement.plan_id:null].filter(Boolean),blockers=blockerState(holds);
    ...
    try{
        const outcome=await recordRun(customerId,controlEntitlement?.subscription_id||null,async()=>{
            ...
            const primary=await reconcileLane(customerId,primaryEntitlement,'primary',accounts,{makePrimary:Boolean(primaryEntitlement)}),free=await reconcileLane(customerId,freeEntitlement,'free',accounts,{makePrimary:!primaryEntitlement&&Boolean(freeEntitlement&&!freeEntitlement.blocked)});
            if(!primaryEntitlement&&!free.active)await disableAccounts(accounts);
            const stremio=require('../stremio/entitlements');let stremioOutcome=null;
            if(stremioEntitlement)stremioOutcome=await stremio.reconcileForCustomer(customerId,stremioEntitlement);else await stremio.suspend(customerId,'No current Stremio subscription.');
            ...
            const result={status:blockers.length?'blocked':'converged',blockers,active:Boolean(primary.active||free.active||stremioOutcome?.status==='active'||emby.active),account,primary,free,stremio:stremioOutcome,emby,discord};
        ...
        return outcome;
    }catch(error){const classified=control.classifyError(error);await control.markCustomerProblem(customerId,classified.status,error,stateDetail(controlEntitlement,null));throw error;}
}
...
async function reconcileAccount(accountId){const result=await query('SELECT customer_id FROM jellyfin_accounts WHERE id=$1',[accountId]);if(!result.rowCount)throw new Error('Media server account not found');return reconcileCustomer(result.rows[0].customer_id);}
...
async function expireSubscriptionsAndReconcile(){return subscriptionExpiry.expireAndReconcile({reconcileCustomer,autoDowngrade:maybeAutoDowngrade,onReconcileError:(customerId,error)=>console.error(`Entitlement reconcile failed for ${customerId}:`,error.message),detail:true});}
module.exports={...base,currentEntitlementTruth,reconcileCustomer,reconcileDiscordRoles,activeDiscordPlanIds,reconcileAccount,holdAccess,releaseAccess,setJellyfinPassword,renameJellyfinAccount,expireSubscriptionsAndReconcile,normalAccounts,applyPolicyIfChanged,reconcileLane,adoptExistingFreeAccount,control,libraryPolicyForAccount,setLibrarySelectionForAccount,reconciliationLock,assertDiscordSyncResult,assertLanePostcondition};