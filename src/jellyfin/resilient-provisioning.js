...
function stateDetail(primaryEntitlement,outcome){const account=outcome?.account||null;return{subscriptionId:primaryEntitlement?.subscription_id||outcome?.free?.entitlement?.subscription_id||outcome?.stremio?.subscriptionId||null,planId:primaryEntitlement?.plan_id||outcome?.free?.entitlement?.plan_id||null,accountId:account?.id||null,serverId:account?.server_id||outcome?.stremio?.serverId||null,serviceType:serviceType(primaryEntitlement),result:{active:Boolean(outcome?.active),planCode:primaryEntitlement?.contract_plan_code||primaryEntitlement?.code||null,jellyfinAccountId:account?.id||null,serverId:account?.server_id||outcome?.stremio?.serverId||null,primary:laneState(outcome?.primary),free:laneState(outcome?.free),stremioStatus:outcome?.stremio?.status||null,reconciledAt:new Date().toISOString()}};}
...
async function reconcileCustomer(customerId){const[effectiveJellyfin,freeEntitlement,stremioEntitlement]=await Promise.all([base.currentEntitlement(customerId),subscriptionState.liveFreeJellyfinSubscription(customerId,{includeBlocked:true}),require('../stremio/entitlements').entitledSubscription(customerId)]),primaryEntitlement=effectiveJellyfin&&!effectiveJellyfin.is_free_tier?effectiveJellyfin:null,controlEntitlement=primaryEntitlement||freeEntitlement||stremioEntitlement||null;require('../integrations/discord-roles').syncRoleForCustomer(customerId,[primaryEntitlement?.plan_id,freeEntitlement?.plan_id,stremioEntitlement?.plan_id].filter(Boolean)).catch(error=>console.warn('Discord role sync failed.',{customerId,error:error.message}));await control.markCustomerRunning(customerId,controlEntitlement);try{const outcome=await recordRun(customerId,controlEntitlement?.subscription_id||null,async()=>{let accounts=await normalAccounts(customerId);accounts=await adoptExistingFreeAccount(customerId,accounts,freeEntitlement,primaryEntitlement);const primary=await reconcileLane(customerId,primaryEntitlement,'primary',accounts,{makePrimary:Boolean(primaryEntitlement)}),free=await reconcileLane(customerId,freeEntitlement,'free',accounts,{makePrimary:!primaryEntitlement&&Boolean(freeEntitlement&&!freeEntitlement.blocked)});if(!primaryEntitlement&&!free.active)await disableAccounts(accounts);const stremio=require('../stremio/entitlements');let stremioOutcome=null;if(stremioEntitlement)stremioOutcome=await stremio.reconcileForCustomer(customerId,stremioEntitlement);else await stremio.suspend(customerId,'No current Stremio subscription.');const account=primary.account||free.account||null;return{active:Boolean(primary.active||free.active||stremioOutcome?.status==='active'),account,primary,free,stremio:stremioOutcome};});await control.markCustomerHealthy(customerId,stateDetail(controlEntitlement,outcome));return outcome;}catch(error){const classified=control.classifyError(error);await control.markCustomerProblem(customerId,classified.status,error,stateDetail(controlEntitlement,null));throw error;}}
...
module.exports={...base,reconcileCustomer,reconcileAccount,holdAccess,releaseAccess,setJellyfinPassword,renameJellyfinAccount,expireSubscriptionsAndReconcile,normalAccounts,reconcileLane,adoptExistingFreeAccount,control,libraryPolicyForAccount,setLibrarySelectionForAccount};
