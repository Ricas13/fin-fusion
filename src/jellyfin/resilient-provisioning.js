...
async function libraryPolicyForAccount(customerId,accountOrId,entitlementOverride=null){
    ...
    if(!account||typeof account!=='object'){
        const found=await query(`SELECT ja.*,js.enabled AS server_enabled,js.server_class,js.name AS server_name,js.public_url FROM jellyfin_accounts ja JOIN jellyfin_servers js ON js.id=ja.server_id WHERE ja.id=$1 AND ja.customer_id=$2 AND ja.account_purpose='jellyfin' LIMIT 1`,[accountOrId,customerId]);
...
async function activeDiscordPlanIds(customerId){const[effectiveJellyfin,freeEntitlement,stremioEntitlement]=await Promise.all([base.currentEntitlement(customerId),subscriptionState.liveFreeJellyfinSubscription(customerId,{includeBlocked:true}),require('../stremio/entitlements').entitledSubscription(customerId)]),primaryEntitlement=effectiveJellyfin&&!effectiveJellyfin.is_free_tier?effectiveJellyfin:null;return[primaryEntitlement&&!primaryEntitlement.blocked?primaryEntitlement.plan_id:null,freeEntitlement&&!freeEntitlement.blocked?freeEntitlement.plan_id:null,stremioEntitlement?.plan_id||null].filter(Boolean);}
