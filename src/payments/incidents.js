...
async function reopen(id,actorUserId){const r=await query(`UPDATE payment_incidents SET incident_status='open',resolved_at=NULL,resolved_by=NULL,resolution_note=NULL,updated_at=NOW() WHERE id=$1 RETURNING *`,[id]);if(!r.rowCount)throw new Error('Payment incident not found.');await query(`INSERT INTO audit_log(actor_user_id,action,entity_type,entity_id,metadata) VALUES($1,'admin.payment_incident.reopen','payment_incident',$2,'{}'::jsonb)`,[actorUserId,id]);return r.rows[0]}
