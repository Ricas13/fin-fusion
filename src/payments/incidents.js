...
async function reopen(id,actorUserId){const r=await query(`UPDATE payment_incidents SET incident_status='open',resolved_at=NULL,resolved_by=NULL,resolution_note=NULL,updated_at=NOW() WHERE id=$1 RETURNING *`,[id]);if(!r.rowCount)throw new Error('Payment incident not found.');const incident=r.rows[0];let affected=0;if(incident.access_action==='suspend'&&incident.provider_case_id&&incident.scope==='direct'&&incident.customer_id){affected=await applyHold({scope:'direct',customerId:incident.customer_id},incident.provider,incident.provider_case_id,`${incident.provider} ${incident.incident_type} is under review`);}await query(`INSERT INTO audit_log(actor_user_id,action,entity_type,entity_id,metadata) VALUES($1,'admin.payment_incident.reopen','payment_incident',$2,$3::jsonb)`,[actorUserId,id,JSON.stringify({affected,holdRestored:affected>0})]);return incident}
