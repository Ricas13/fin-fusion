...
const planChangeResolution = require('./plan-change-resolution');
...
function paypalHttpError(response,payload,requestId,prefix='PayPal HTTP'){const detail=typeof payload==='object'?payload?.message||payload?.name||payload?.error_description:payload;const error=providerHttp.responseError('paypal',response,payload,requestId,`${prefix} ${response.status}: ${detail||'request failed'}`);error.message=`${prefix} ${response.status}: ${detail||'request failed'}`;return error;}
async function accessToken(){const config=await providerSettings.get('paypal');if(!config.clientId||!config.clientSecret)throw new Error('PayPal is not configured');const credentialKey=`${config.environment||'sandbox'}:${config.clientId}:${config.clientSecret}`;if(cachedCredentialKey!==credentialKey){cachedToken=null;cachedUntil=0;cachedCredentialKey=credentialKey;}if(cachedToken&&Date.now()<cachedUntil-60000)return{token:cachedToken,config};const basic=Buffer.from(`${config.clientId}:${config.clientSecret}`).toString('base64'),{response,data:payload,requestId}=await providerHttp.fetchJson('paypal',`${baseUrl(config)}/v1/oauth2/token`,{method:'POST',headers:{Authorization:`Basic ${basic}`,'Content-Type':'application/x-www-form-urlencoded',Accept:'application/json'},body:'grant_type=client_credentials'});if(!response.ok||!payload.access_token)throw paypalHttpError(response,payload,requestId,'PayPal OAuth failed:');cachedToken=payload.access_token;cachedUntil=Date.now()+Number(payload.expires_in||300)*1000;return{token:cachedToken,config};}
async function api(path,{method='GET',body=null,requestId=null}={}){const{token,config}=await accessToken(),result=await providerHttp.fetchJson('paypal',`${baseUrl(config)}${path}`,{method,headers:{Authorization:`Bearer ${token}`,Accept:'application/json',...(body?{'Content-Type':'application/json'}:{}),...(requestId?{'PayPal-Request-Id':requestId}:{})},...(body?{body:JSON.stringify(body)}:{})}),payload=result.data;if(!result.response.ok)throw paypalHttpError(result.response,payload,result.requestId);return payload||{};}
...
async function resolveRecordedPlanChange(mapping,providerSubscriptionId){return planChangeResolution.resolveAwaitingCheckout({customerId:mapping.customerId,planId:mapping.planId,provider:'paypal',providerSubscriptionId});}
async function getCapture(captureId){return api(`/v2/payments/captures/${encodeURIComponent(captureId)}`);}
async function recordCompletedCapture(capture,{customerId,providerCustomerId=null,eventId=null,reconciliation=false}={}){return livePaypalHistory.recordCapture(capture,{customerId,providerCustomerId,eventId,reconciliation,fetchCapture:getCapture});}
async function activateCompletedOrder(order){if(!order?.id||order.status!=='COMPLETED')throw new Error(`PayPal order is ${order?.status||'not completed'}`);const unit=order.purchase_units?.[0],mapping=parseCustomId(unit?.custom_id);if(!mapping)throw new Error('PayPal order is missing internal purchase metadata');const capture=unit?.payments?.captures?.[0];if(!capture?.id)throw new Error(`PayPal order ${order.id} is completed but does not contain a capture ID.`);const providerId=capture.id,payerId=order.payer?.payer_id||null,amount=capture.amount||unit?.amount||{};const contract=await checkoutIntents.verifiedProviderContract({provider:'paypal',providerCheckoutId:order.id,scope:'customer',ownerId:mapping.customerId,planId:mapping.planId,checkoutMode:'payment',amountMinor:paypalMinor(amount),currency:amount.currency_code});if(mapping.discountCodeId&&contract.snapshot.discountCodeId&&String(mapping.discountCodeId)!==String(contract.snapshot.discountCodeId))throw new Error('PayPal discount metadata does not match the checkout contract.');await livePaypalHistory.assertCaptureOwner(providerId,mapping.customerId);const existingCapture=await query(`SELECT customer_id FROM subscriptions WHERE source='paypal' AND provider_subscription_id=$1 LIMIT 1`,[providerId]);const activationPayerId=existingCapture.rowCount?null:payerId;const activated=await lifecycle.activatePurchase({customerId:mapping.customerId,planId:mapping.planId,provider:'paypal',providerCustomerId:activationPayerId,providerSubscriptionId:providerId,providerStatus:'active',commercialSnapshot:contract.snapshot});await checkoutIntents.completeVerifiedProvider('paypal',order.id,'completed');await resolveRecordedPlanChange(mapping,providerId);await recordCompletedCapture(capture,{customerId:mapping.customerId,providerCustomerId:payerId});return activated;}
async function captureOrder(orderId){let order=await api(`/v2/checkout/orders/${encodeURIComponent(orderId)}`);if(order.status!=='COMPLETED'){const intent=await checkoutIntents.findProviderIntent('paypal',orderId);if(!intent||intent.state!=='open')throw new Error(`PayPal capture refused for ${orderId}: local checkout intent is ${intent?.state||'missing'}.`);await api(`/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`,{method:'POST',requestId:`capture-${orderId}`,body:{}});order=await api(`/v2/checkout/orders/${encodeURIComponent(orderId)}`);}return activateCompletedOrder(order);}
...
async function resumeCheckout(subscriptionId){
  const id=String(subscriptionId||'').trim();
  if(!/^I-/i.test(id))return null;

  const subscription=await getSubscription(id),status=paypalStatus(subscription.status);

  if(status==='APPROVAL_PENDING'){
    const url=approvalUrl(subscription);
    if(!url)throw new Error('PayPal checkout is awaiting approval but no approval URL is available.');
    return{id:subscription.id,url,mode:'subscription',resumed:true,status};
  }

  if(status==='ACTIVE'){
    await activateSubscription(subscription.id);
    return{internal:true,message:'Payment confirmed. Your access has been updated.'};
  }

  if(paypalTerminal(status)){
    await checkoutIntents.completeVerifiedProvider('paypal',subscription.id,'cancelled');
    return null;
  }
...
async function paypalRefundContext(resource){
  ...
  const cumulativeMoney=resource?.total_refunded_amount||resource?.seller_payable_breakdown?.total_refunded_amount||resource?.amount||null;
  const cumulative=paypalAmount({amount:cumulativeMoney});
  const original=sale?paypalAmount(sale):{minor:null,currency:null};
  const fullRefund=Number.isInteger(original.minor)&&original.minor>0&&Number.isInteger(cumulative.minor)&&cumulative.minor>=original.minor;
  const providerId=resource?.billing_agreement_id||sale?.billing_agreement_id||saleId||resource?.id||null;
  return{saleId,providerId,amount,cumulativeMinor:cumulative.minor,originalMinor:original.minor,fullRefund,transactionOccurredAt:sale?.create_time||sale?.create_time||null};
}
function captureRefundAmounts(refundResource,capture,{reversed=false}={}){
  const amount=paypalAmount(refundResource);
  const original=paypalAmount(capture||{});
  const cumulativeMoney=capture?.seller_receivable_breakdown?.total_refunded_amount||capture?.total_refunded_amount||null;
  const cumulative=cumulativeMoney?paypalAmount({amount:cumulativeMoney}):amount;
  const fullRefund=Boolean(reversed)||(Number.isInteger(original.minor)&&original.minor>0&&Number.isInteger(cumulative.minor)&&cumulative.minor>=original.minor);
  return{amount,cumulativeMinor:cumulative.minor,originalMinor:original.minor,fullRefund};
}
async function paypalCaptureRefundContext(resource,{reversed=false}={}){
  const related=resource?.supplementary_data?.related_ids||resource?.related_ids||{};
  const captureId=related.capture_id||resource?.capture_id||(reversed?resource?.id:null);
  if(!captureId)throw new Error('PayPal capture refund/reversal is missing the related capture ID.');
  const capture=await api(`/v2/payments/captures/${encodeURIComponent(captureId)}`);
  const orderId=captureOrderId(capture)||related.order_id||null;
  const amounts=captureRefundAmounts(resource,capture,{reversed});
  let identity=await incidents.identityFromProviderSubscription('paypal',captureId);
  let mapping=null;
  if(identity.scope==='unresolved'&&orderId){
    const order=await api(`/v2/checkout/orders/${encodeURIComponent(orderId)}`);
    mapping=parseCustomId(order?.purchase_units?.[0]?.custom_id);
    if(mapping?.customerId)identity={scope:'direct',customerId:mapping.customerId};
  }
  return{captureId,orderId,capture,identity,mapping,...amounts,transactionOccurredAt:capture?.create_time||null};
}
...
async function recordSaleReversal(event,resource){
  const providerId=resource?.billing_agreement_id||resource?.supplementary_data?.related_ids?.subscription_id||resource?.id||null;
  if(!providerId)throw new Error('PayPal sale reversal is missing its subscription or sale identity.');
  const identity=await incidents.identityFromProviderSubscription('paypal',providerId);
  const amount=paypalAmount(resource);
  const recorded=await incidents.record({
    provider:'paypal',
    eventId:event.id,
    caseId:resource?.id||providerId,
    kind:'refund',
    status:'recorded',
    identity,
...
function billingFailureSubscriptionId(resource){
  const candidates=[
    resource?.id,
...
async function settleTerminalCapture(resource,state='failed'){
  ...
  return checkoutIntents.completeVerifiedProvider('paypal',orderId,state);
}
async function settleReversedApproval(resource){
  const orderId=resource?.id||resource?.order_id||resource?.supplementary_data?.related_ids?.order_id||null;
  if(!orderId)return null;
  return checkoutIntents.completeVerifiedProvider('paypal',orderId,'cancelled');
}
