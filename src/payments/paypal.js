...
async function recordSubscriptionPaymentSuccess(event,resource){
  ...
  if(synced.row&&paypalHealthy(synced.providerStatus)){
    await failedRenewals.resolveOpen({
      provider:'paypal',
      providerSubscriptionId:subscriptionId,
      note:'PayPal subscription payment succeeded and the provider subscription is active.'
    });
  }else if(paypalTerminal(synced.providerStatus)){
    await failedRenewals.resolveOpen({
      provider:'paypal',
      providerSubscriptionId:subscriptionId,
      note:'PayPal subscription is already terminal; delayed payment-success event cannot restore it.'
    });
  }
  return synced;
}
