...
async function syncCurrentSubscription(subscriptionId,{activateMissing=true}={}){
  ...
  const effectiveStatus=paypalHealthy(providerStatus)&&paypalOutstandingRenewal(subscription)
...
async function recordSubscriptionPaymentSuccess(event,resource){
  ...
  if(synced.row&&paypalHealthy(synced.providerStatus)&&!paypalOutstandingRenewal(synced.subscription)){
