...
async function syncCurrentSubscription(subscriptionId,{activateMissing=true}={}){
  ...
  // PayPal can remain ACTIVE while a failed renewal is being retried. Never
  // release the existing payment-delinquency hold just because the agreement
  // itself is active while provider billing_info still reports money owed.
  const existingDelinquent=local.rows[0]?.status==='past_due';
  const effectiveStatus=paypalHealthy(providerStatus)
    && (paypalOutstandingRenewal(subscription)
      || (existingDelinquent&&!paypalBillingCleared(subscription)))
    ? 'PAST_DUE' : update.providerStatus;
  const row=await lifecycle.updateProviderSubscription({provider:'paypal',providerSubscriptionId:subscription.id,providerStatus:effectiveStatus,periodEnd:update.periodEnd,cancelAtPeriodEnd:update.cancelAtPeriodEnd});
...
function paypalOutstandingRenewal(subscription){
  const billing=subscription?.billing_info||{};
  const count=Number(billing.failed_payments_count||0);
  const amount=paypalMinor(billing.outstanding_balance);
  return (Number.isFinite(count)&&count>0)||(Number.isFinite(amount)&&amount>0);
}
function paypalBillingCleared(subscription){
  const billing=subscription?.billing_info;
  if(!billing||typeof billing!=='object')return false;
  const count=Number(billing.failed_payments_count);
  const balance=paypalMinor(billing.outstanding_balance);
  return Number.isFinite(count)&&count===0&&balance===0;
}
function paypalPaymentFailureCurrent(event,subscription){
  ...
  const laterSuccessfulPayment=Number.isFinite(eventAt)&&eventAt>0
    ...
    && failedCount===0
    && outstandingMinor===0
    && paypalBillingCleared(subscription);
...
async function recordSubscriptionPaymentSuccess(event,resource){
  ...
  if(synced.row&&paypalHealthy(synced.providerStatus)&&paypalBillingCleared(synced.subscription)){
    await failedRenewals.resolveOpen({
      ...
      note:'PayPal confirms the subscription is active with no failed payments or outstanding balance.'
    });
