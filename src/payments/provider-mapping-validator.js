...
async function persist(table,id,expected,result){
    ...
    return transaction(async client=>{
        ...
        await client.query(`UPDATE ${table} SET
            ...
            active=CASE WHEN $9='verified' THEN TRUE ELSE active END,updated_at=NOW()
            WHERE id=$1`,[
...
async function validateDirect(id){
    ...
    const row=found.rows[0],result=await verifier.verify(row.provider,row.external_id,{
...
module.exports={validateDirect,validateReseller};
