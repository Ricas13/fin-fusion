...
function eligible(plan){
  ...
  return ['jellyfin','bundle'].includes(service)&&['direct','both'].includes(audience)&&!plan?.is_free_tier&&String(plan?.billing_interval||'')!=='trial'&&number(plan?.price_minor)>0;
}
...
async function rowsForPlans(planIds,currency){
  ...
  const result=await query(`SELECT v.*,vp.id mapping_id,vp.provider,vp.checkout_mode,vp.external_id,vp.active mapping_active,vp.verification_status
    FROM plan_stream_variants v
    LEFT JOIN plan_stream_variant_provider_prices vp ON vp.stream_variant_id=v.id AND vp.active=TRUE
    WHERE v.plan_id=ANY($1::uuid[]) AND v.currency=$2 AND v.active=TRUE
    ORDER BY v.plan_id,v.streams,vp.provider,vp.checkout_mode`,[ids,String(currency||'GBP').toUpperCase()]);
  return result.rows;
}
...
async function decoratePlans(plans,currency){
  ...
  for(const row of rows){
    ...
    if(!variants.has(variantKey))variants.set(variantKey,{id:row.id,stream_variant_id:row.id,streams:number(row.streams,1),price_minor:number(row.price_minor),currency:String(row.currency).toUpperCase(),active:Boolean(row.active),payment_options:[]});
  ...
  return list.map(plan=>{
    ...
    const base={id:null,stream_variant_id:null,streams:number(plan.streams,1),price_minor:number(plan.price_minor),currency:String(plan.currency||currency||'GBP').toUpperCase(),active:true,payment_options:Array.isArray(plan.payment_options)?plan.payment_options:[]};
    const extras=Array.from(grouped.get(String(plan.id))?.values()||[]).filter(v=>v.streams!==base.streams);
    ...
    const variants=[base,...extras].sort((a,b)=>a.streams-b.streams);
    return{...plan,stream_variants:variants,has_stream_variants:variants.length>1};
  });
}
...
async function adminState(planId,currency){
  const result=await query(`SELECT v.*,vp.id mapping_id,vp.provider,vp.checkout_mode,vp.external_id,vp.active mapping_active,vp.verified_at,vp.verification_status,vp.verification_error,vp.remote_amount_minor,vp.remote_currency,vp.remote_interval,vp.remote_active
    FROM plan_stream_variants v
    LEFT JOIN plan_stream_variant_provider_prices vp ON vp.stream_variant_id=v.id
    WHERE v.plan_id=$1 AND v.currency=$2
    ORDER BY v.streams,vp.provider,vp.checkout_mode`,[planId,String(currency||'GBP').toUpperCase()]);
  const variants=new Map();
  for(const row of result.rows){const key=String(row.id);if(!variants.has(key))variants.set(key,{id:row.id,plan_id:row.plan_id,streams:number(row.streams,1),currency:String(row.currency).toUpperCase(),price_minor:number(row.price_minor),active:Boolean(row.active),mappings:new Map()});if(row.mapping_id)variants.get(key).mappings.set(`${row.provider}:${row.checkout_mode}`,row);}
  return Array.from(variants.values()).sort((a,b)=>a.streams-b.streams);
}

async function resolve(planCode,provider,currency,streams,checkoutMode=null){
  const requested=Number(streams);if(!Number.isInteger(requested)||requested<1)return[];
  const mode=checkoutMode&&['payment','subscription'].includes(checkoutMode)?checkoutMode:null;
  if(provider==='coingate'){
    const result=await query(`SELECT p.*,pr.id plan_price_id,v.id stream_variant_id,v.price_minor,v.currency,pr.is_default,
  ...
  const result=await query(`SELECT p.*,pr.id plan_price_id,v.id stream_variant_id,v.price_minor,v.currency,pr.is_default,
      NULL::uuid provider_mapping_id,vp.external_id,vp.checkout_mode,'{"streamVariant":true}'::jsonb provider_metadata
    FROM plans p JOIN plan_prices pr ON pr.plan_id=p.id AND pr.active=TRUE
    JOIN plan_stream_variants v ON v.plan_id=p.id AND v.active=TRUE AND v.currency=pr.currency
...
async function byExternalId(provider,externalId){
  const result=await query(`SELECT p.*,pr.id plan_price_id,v.id stream_variant_id,v.price_minor,v.currency,pr.is_default,
      NULL::uuid provider_mapping_id,vp.external_id,vp.checkout_mode,'{"streamVariant":true}'::jsonb provider_metadata,vp.active mapping_active
    FROM plan_stream_variant_provider_prices vp
    JOIN plan_stream_variants v ON v.id=vp.stream_variant_id
