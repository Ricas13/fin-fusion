...
// service-core retains the historical login-only compatibility shortcut. At
...
// pending so the core cannot auto-pass merely because the same user already
...
async function verifySecondFactor(userId,token,req){if(!req?.session)return core.verifySecondFactor(userId,token,req);const prior=req.session.pendingStaffAuth;req.session.pendingStaffAuth=prior||{stepUp:true,userId:String(userId)};try{return await core.verifySecondFactor(userId,token,req)}finally{if(prior===undefined)delete req.session.pendingStaffAuth;else req.session.pendingStaffAuth=prior}}
module.exports={...core,requiresTwoFactor,beginTotpEnrollment,registerSession,verifySecondFactor};
