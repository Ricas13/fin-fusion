...
async function salesAnalytics(resellerId, { from, to }) {
    ...
    const [totals,daily,newCustomers,playback,topCustomers] = await Promise.all([
        ...
        query(`SELECT date_trunc('day',created_at)::date AS sale_day,currency,SUM(${revenueExpr})::bigint amount_minor FROM reseller_sales rs WHERE reseller_id=$1 AND created_at>=$2 AND created_at<$3 AND voided_at IS NULL GROUP BY 1,2 ORDER BY 1,2`,[resellerId,start,end]),
