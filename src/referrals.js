...
async function ensureReferralCode(customerId){
  ...
  for(let attempt=0;attempt<5;attempt++){
    ...
    try{return(await query('INSERT INTO referral_codes(customer_id,code) VALUES($1,$2) RETURNING code',[customerId,code])).rows[0].code;}
    catch(error){if(error.code!=='23505')throw error;const found=await query('SELECT code FROM referral_codes WHERE customer_id=$1',[customerId]);if(found.rowCount)return found.rows[0].code;}
  }
  throw new Error('Could not generate a referral code');
}
async function sameIdentity(referrerId,referredId,client=null){
  const db=client||{query};
  const people=await db.query(`SELECT id,LOWER(TRIM(COALESCE(email,''))) email FROM customers WHERE id=ANY($1::uuid[])`,[[referrerId,referredId]]);
  if(people.rowCount<2)return true;
  const emails=new Map(people.rows.map(r=>[String(r.id),r.email]));
  if(emails.get(String(referrerId))&&emails.get(String(referrerId))===emails.get(String(referredId)))return true;
...
async function rewardIfQualifying(referredCustomerId){
  ...
  return transaction(async client=>{
    const pending=await client.query(`SELECT rr.id,rr.referral_code_id,rr.created_at,rc.customer_id AS referrer_customer_id
    ...
    if(await sameIdentity(redemption.referrer_customer_id,referredCustomerId,client)){
      await client.query(`UPDATE referral_redemptions SET status='unfulfilled',reward_note='Affiliate referral rejected: accounts share an email/payment identity.' WHERE id=$1`,[redemption.id]);
      return{rewarded:false,reason:'same_identity'};
    }
    const paid=await client.query(`SELECT s.id,s.source,s.starts_at,s.current_period_end,s.provider_subscription_id,
      ...
      WHERE s.customer_id=$1 AND s.source IN('stripe','paypal','plisio') AND s.superseded_by IS NULL AND s.status='active'
    ...
    const adverse=await client.query(`SELECT incident_type,incident_status FROM payment_incidents WHERE customer_id=$1 AND provider=$2 AND created_at>=$3 AND (provider_subscription_id=$4 OR $4 IS NULL) AND incident_type IN('refund','chargeback','dispute') ORDER BY created_at DESC`,[referredCustomerId,qualifying.source,qualifying.starts_at,qualifying.provider_subscription_id]);
    const hardLoss=adverse.rows.find(row=>row.incident_type==='refund'||row.incident_type==='chargeback'||String(row.incident_status||'').toLowerCase()==='lost');
    if(hardLoss){await client.query(`UPDATE referral_redemptions SET status='unfulfilled',reward_note=$2 WHERE id=$1`,[redemption.id,`Affiliate credit rejected after ${hardLoss.incident_type} evidence.`]);return{rewarded:false,reason:'payment_reversed'};}
    const unresolved=adverse.rows.some(row=>row.incident_type==='dispute'&&!['won','resolved'].includes(String(row.incident_status||'').toLowerCase()));
    ...
    const ledger=await client.query(`INSERT INTO affiliate_credit_ledger(customer_id,currency,amount_minor,entry_type,state,referral_redemption_id,referred_customer_id,qualifying_subscription_id,available_at,reference_id,note,metadata)
      VALUES($1,$2,$3,'earned',$4,$5,$6,$7,$8,$9,$10,$11::jsonb)
      ON CONFLICT(entry_type,reference_id) DO NOTHING RETURNING id`,[
        redemption.referrer_customer_id,String(qualifying.currency||'GBP').toUpperCase(),reward,eligibleAt<=Date.now()?'available':'pending',redemption.id,referredCustomerId,qualifying.id,eligibleAt,`affiliate:${redemption.id}`,
        `Affiliate reward: ${settings.rewardPercent}% of qualifying paid service`,JSON.stringify({paidMinor:Number(qualifying.paid_minor),rewardPercent:settings.rewardPercent,provider:qualifying.source})
      ]);
    await client.query(`UPDATE referral_redemptions SET status='rewarded',rewarded_at=NOW(),reward_note=$2 WHERE id=$1`,[redemption.id,`${reward} ${String(qualifying.currency||'GBP').toUpperCase()} minor units of affiliate service credit recorded${eligibleAt>Date.now()?` (available ${eligibleAt.toISOString()})`:''}.`]);
    await client.query(`INSERT INTO audit_log(action,entity_type,entity_id,metadata) VALUES('affiliate.reward','customer',$1,$2::jsonb)`,[redemption.referrer_customer_id,JSON.stringify({redemptionId:redemption.id,referredCustomerId,qualifyingSubscriptionId:qualifying.id,paidMinor:Number(qualifying.paid_minor),rewardMinor:reward,currency:String(qualifying.currency||'GBP').toUpperCase(),rewardPercent:settings.rewardPercent,ledgerCreated:Boolean(ledger.rowCount)})]);
    return{rewarded:true,amountMinor:reward,currency:String(qualifying.currency||'GBP').toUpperCase(),availableAt:eligibleAt};
  });
}

async function revisitRewardAfterAdversePayment({referredCustomerId,incidentId=null,reason='payment_reversal',amountMinor=null,fullLoss=false}={}){
  if(!referredCustomerId)return{reversed:false,reason:'no_customer'};
  const r=await query(`SELECT rr.id FROM referral_redemptions rr WHERE rr.referred_customer_id=$1 AND rr.status='rewarded' ORDER BY rr.rewarded_at DESC LIMIT 1`,[referredCustomerId]);
  if(!r.rowCount)return{reversed:false,reason:'no_rewarded_referral'};
  const out=await affiliateCredits.reverseReward({redemptionId:r.rows[0].id,paymentIncidentId:incidentId,reason});
  if(out.reversed)await query(`UPDATE referral_redemptions SET status='reversed',reward_note=$2 WHERE id=$1`,[r.rows[0].id,`Qualifying payment reversed. Removed ${out.amountMinor||0} ${out.currency||''} minor units of unspent affiliate credit; already-delivered service was preserved.`]);
  return out;
}

function unusedExtensionDays(){return 0;}
async function processDueRewards({limit=100}={}){
  await affiliateCredits.matureDueCredits();
  const rows=await query(`SELECT referred_customer_id FROM referral_redemptions WHERE status='pending' ORDER BY created_at LIMIT $1`,[Math.max(1,Math.min(500,Number(limit)||100))]);
  let rewarded=0,pending=0,failed=0;
  for(const row of rows.rows){try{const out=await rewardIfQualifying(row.referred_customer_id);if(out?.rewarded)rewarded++;else pending++;}catch(error){failed++;console.warn('Affiliate qualification retry failed:',error.message);}}
