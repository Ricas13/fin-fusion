...
function safeClientField(value,fallback,max=120){const text=String(value||fallback||'').trim();if(!text||/[\r\n"\\]/.test(text))throw new Error('Media-server client identity contains invalid characters.');return text.slice(0,max);}
...
function clientAuthorization(type,{userId='',client='CAPTAiNFiN Stremio',device='CAPTAiNFiN',deviceId='captainfin-stremio',version='2.0'}={}) { const provider=normalizeType(type),prefix=provider==='emby'?'Emby':'MediaBrowser',id=String(userId||'').trim(),fields=[id?`UserId="${safeClientField(id,'',120)}"`:null,`Client="${safeClientField(client,'CAPTAiNFiN Stremio')}"`,`Device="${safeClientField(device,'CAPTAiNFiN')}"`,`DeviceId="${safeClientField(deviceId,'captainfin-stremio')}"`,`Version="${safeClientField(version,'2.0',40)}"`].filter(Boolean);return`${prefix} ${fields.join(', ')}`; }
...
module.exports={TYPES,normalizeType,label,validToken,safeClientField,authHeaders,userTokenHeaders,clientAuthorization,apiPath,apiUrl,healthEndpoint,credentialProbeEndpoint,canonicalPath,userPolicyOverrides,userPolicy,requestBody,needsPostCreatePassword,responseBody};