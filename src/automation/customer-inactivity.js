...
async function runPlanRules({actorUserId=null,forceDryRun=null}={}){const globalCfg=await lifecyclePolicy.get(),released=await releaseObsoletePlanHolds(actorUserId,globalCfg);if(!globalCfg.enabled)return{processed:0,eligible:0,enforced:0,wouldDisable:0,released,dryRun:true,skipped:'lifecycle_disabled'};const telemetry=await telemetryReady();if(!telemetry.ready)return{processed:0,eligible:0,enforced:0,wouldDisable:0,released,dryRun:true,skipped:'telemetry_not_trustworthy',telemetry};const rows=await candidates(globalCfg),eligible=rows.filter(x=>x.eligible);let enforced=0,wouldDisable=0;for(const row of eligible){const dryRun=forceDryRun===null?row.policy.dryRun:Boolean(forceDryRun),evidence={planId:row.plan_id,planCode:row.plan_code,accessLane:'free',accountId:row.account_id,serverId:row.server_id,lastPlaybackAt:row.last_playback_at||null,inactiveReferenceAt:row.inactive_reference_at,observationStartedAt:row.observation_started_at,playbackMinutes:Math.round(row.playback_seconds/60),triggers:row.triggers,dryRun,policyInherited:row.policy.inherited,portalAccountPreserved:true};await query(`INSERT INTO audit_log(actor_user_id,action,entity_type,entity_id,metadata) VALUES($1,$2,'customer',$3,$4::jsonb)`,[actorUserId,dryRun?'customer.inactivity.would_disable_jellyfin':'customer.inactivity.disable_jellyfin',row.customer_id,JSON.stringify(evidence)]);if(dryRun){wouldDisable++;continue;}await accessHolds.addHold({customerId:row.customer_id,type:HOLD_TYPE,sourceKey:`plan:${row.plan_id}`,reason:`Free-plan Jellyfin usage rule: ${row.triggers.join('; ')}`,actorUserId,metadata:evidence});await provisioning.reconcileCustomer(row.customer_id);enforced++;}return{processed:rows.length,eligible:eligible.length,enforced,wouldDisable,released,dryRun:eligible.every(x=>forceDryRun===true||x.policy.dryRun),telemetry,examples:eligible.slice(0,25).map(x=>({customerId:x.customer_id,name:x.customer_name,plan:x.plan_code,server:x.server_name,triggers:x.triggers,lastPlaybackAt:x.last_playback_at,playbackMinutes:Math.round(x.playback_seconds/60)}))};}
...
async function runCleanup({actorUserId=null,forceDryRun=null}={}){const cfg=await getCleanup(),telemetry=await telemetryReady();if(!cfg.enabled)return{processed:0,eligible:0,deleted:0,dryRun:true,skipped:'cleanup_disabled',telemetry};if(!telemetry.ready)return{processed:0,eligible:0,deleted:0,dryRun:true,skipped:'telemetry_not_trustworthy',telemetry};const dryRun=forceDryRun===null?cfg.dryRun:Boolean(forceDryRun),rows=await cleanupCandidates(cfg),eligible=rows.filter(x=>x.eligible);let deleted=0,failed=0;for(const row of eligible){try{if(await deleteDormantAccount(row,{actorUserId,dryRun,deleteAfterDays:cfg.deleteAfterDays}))deleted++;}catch(error){failed++;console.error('Dormant Jellyfin cleanup failed:',{accountId:row.account_id,error:String(error?.message||error).slice(0,500)});}}return{processed:rows.length,eligible:eligible.length,deleted,failed,dryRun,telemetry,examples:eligible.slice(0,25).map(x=>({customerId:x.customer_id,name:x.customer_name,server:x.server_name,accessLane:x.access_lane,jellyfinUsername:x.jellyfin_username,activityReferenceAt:x.activity_reference_at}))};}
