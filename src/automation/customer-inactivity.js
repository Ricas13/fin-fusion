...
async function reconcileCustomer(customerId){return require('../jellyfin/resilient-provisioning').reconcileCustomer(customerId);}
...
async function candidates(){
  const result=await query(`
    WITH current_access AS (
      ...
      WHERE s.superseded_by IS NULL AND s.status IN('active','trialing','past_due','paused')
        ...
        AND p.is_free_tier=TRUE AND COALESCE(p.service_type,'jellyfin') IN('jellyfin','bundle')
      ...
      SELECT customer_id,MIN(created_at) first_account_at,
             COUNT(*) FILTER(WHERE disabled=FALSE)::int active_accounts
      FROM jellyfin_accounts
      WHERE account_purpose='jellyfin' AND access_lane='free'
    ...
    SELECT ca.*,COALESCE(c.display_name,u.username,c.email,'Customer') customer_name,COALESCE(c.email,u.email) email,c.automation_protected,
      ...
      EXISTS(
        ...
        WHERE aps.customer_id=ca.customer_id AND ja.account_purpose='jellyfin' AND ja.access_lane='free'
      ) currently_playing,
    ...
    LEFT JOIN LATERAL (
      ...
      JOIN jellyfin_accounts ja ON ja.id=ph.jellyfin_account_id
      WHERE ph.customer_id=ca.customer_id AND ja.account_purpose='jellyfin' AND ja.access_lane='free'
    ) us ON TRUE
  ...
  return result.rows.map(row=>{
    ...
    return{...row,policy,playback_seconds:seconds,inactive_reference_at:referenceAt,eligible,triggers,reasons:eligible?triggers:[row.automation_protected?'admin protected':null,row.currentlyPlaying?'currently playing':null,row.already_held?'already held':null,!noPlaybackEligible&&!usageEligible?'usage requirements currently satisfied':null].filter(Boolean)};
  });
}
...
async function releaseObsoletePlanHolds(actorUserId=null){
  const rows=await query(`
    ...
    WHERE h.hold_type=$1 AND h.released_at IS NULL
      AND NOT EXISTS(
        ...
        WHERE s.customer_id=h.customer_id AND h.source_key=('plan:'||p.id::text)
          AND p.is_free_tier=TRUE AND COALESCE(p.service_type,'jellyfin') IN('jellyfin','bundle')
          AND COALESCE((p.inactivity_policy->>'enabled')::boolean,FALSE)=TRUE
  ...
  `,[HOLD_TYPE]);
  for(const row of rows.rows){await accessHolds.releaseHold({customerId:row.customer_id,type:HOLD_TYPE,sourceKey:row.source_key,actorUserId});await reconcileCustomer(row.customer_id).catch(()=>{});}
...
async function runPlanRules({actorUserId=null,forceDryRun=null}={}){
  ...
  for(const row of eligible){
    const dryRun=forceDryRun===null?row.policy.dryRun:Boolean(forceDryRun),evidence={planId:row.plan_id,planCode:row.plan_code,lastPlaybackAt:row.last_playback_at||null,inactiveReferenceAt:row.inactive_reference_at,playbackMinutes:Math.round(row.playback_seconds/60),triggers:row.triggers,dryRun,portalAccountPreserved:true,accessLane:'free'};
    ...
    await reconcileCustomer(row.customer_id);enforced++;
  }
...
async function cleanupCandidates(cfg=null){
  ...
  const r=await query(`
    ...
    LEFT JOIN LATERAL (SELECT MAX(COALESCE(ended_at,last_seen_at,started_at)) last_playback_at FROM playback_history WHERE jellyfin_account_id=ja.id) ph ON TRUE
    WHERE ja.account_purpose='jellyfin' AND ja.access_lane='free' AND ja.created_at<=$2
...
async function deleteDormantAccount(row,{actorUserId=null,dryRun=true,deleteAfterDays=30}={}){
  const evidence={accountId:row.account_id,serverId:row.server_id,serverName:row.server_name,jellyfinUsername:row.jellyfin_username,activityReferenceAt:row.activity_reference_at,deleteAfterDays,dryRun,portalAccountPreserved:true,accessLane:'free'};
  ...
  await accessHolds.addHold({customerId:row.customer_id,type:CLEANUP_HOLD_TYPE,sourceKey,reason:`Dormant Free Server user removed after ${deleteAfterDays} inactive day(s)`,actorUserId,metadata:evidence});
  try{
...
async function runCleanup({actorUserId=null,forceDryRun=null}={}){
  ...
  for(const row of eligible){try{if(await deleteDormantAccount(row,{actorUserId,dryRun,deleteAfterDays:cfg.deleteAfterDays}))deleted++;}catch(error){failed++;console.error('Dormant Free Jellyfin cleanup failed:',{accountId:row.account_id,error:String(error?.message||error).slice(0,500)});}}
...
async function restoreReturningCustomer(customerId){
  ...
  const [primary,free]=await Promise.all([subscriptionState.effectiveSubscription(customerId,{includeBlocked:true}),subscriptionState.liveFreeJellyfinSubscription(customerId,{includeBlocked:true})]);
  if(!free)return{restored:false,reason:'no_free_entitlement'};
  if(primary&&!primary.is_free_tier)return{restored:false,reason:'premium_active'};
  ...
  await query(`INSERT INTO audit_log(action,entity_type,entity_id,metadata) VALUES('jellyfin.cleanup.restore_on_portal_return','customer',$1,$2::jsonb)`,[customerId,JSON.stringify({releasedCleanupHolds:holds.rowCount,portalReturn:true,accessLane:'free'})]);
