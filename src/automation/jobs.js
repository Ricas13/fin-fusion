...
const jobs={
 ...
 async stremio_media_index(){let managed={total:0,processed:0,failed:0};try{managed=await stremioMediaIndex.indexAll();}catch(error){managed={total:0,processed:0,failed:1};console.error('Managed Stremio media index failed:',error.message);}const external=await stremioSourceIndex.indexDueSources(),expiredPlaybackLeases=await stremioSourceAdmission.cleanup(5000);return{total:Number(managed.total||0)+Number(external.total||0),processed:Number(managed.processed||0)+Number(external.processed||0)+Number(expiredPlaybackLeases||0),failed:Number(managed.failed||0)+Number(external.failed||0),managed,external,expiredPlaybackLeases}}
};
