...
async function runSingleton(jobKey, fn, { force = false } = {}) {
    ...
    try {
        ...
        await client.query(`INSERT INTO automation_job_state(job_key,enabled,interval_seconds,last_started_at,updated_at)
            VALUES($1,TRUE,300,NOW(),NOW()) ON CONFLICT(job_key) DO UPDATE SET last_started_at=NOW(),updated_at=NOW()`, [jobKey]);
        try {
