...
async function selectServer(plan){
  ...
  if(!servers.length)throw new Error('No healthy Stremio-enabled media server with a public URL is available for this plan.');return servers[0];
}
async function authenticateRestrictedUser(serverId,username,password){
  ...
  const type=registry.mediaProvider.normalizeType(server.media_server_type),providerLabel=registry.mediaProvider.label(type),path=registry.mediaProvider.apiPath(type,'/Users/AuthenticateByName');
  const url=new URL(path,`${server.base_url}/`),response=await outbound.safeFetch(url,{purpose:`Stremio restricted authentication on ${server.name}`,method:'POST',timeoutMs:10000,headers:{Authorization:clientAuthorization(type),Accept:'application/json','Content-Type':'application/json'},body:JSON.stringify({Username:username,Pw:password})});
  const text=await response.text();let body={};try{body=text?JSON.parse(text):{};}catch{}
  if(!response.ok||!body.AccessToken||!body.User?.Id)throw new Error(`Restricted ${providerLabel} authentication failed (${response.status}).`);return{accessToken:String(body.AccessToken),userId:String(body.User.Id),mediaServerType:type};
}
async function logoutRestrictedToken(server,encryptedToken){
  if(!encryptedToken)return false;
  try{
    const type=registry.mediaProvider.normalizeType(server?.media_server_type),token=decryptWithEnv(encryptedToken,TOKEN_ENV,TOKEN_PREFIX),path=registry.mediaProvider.apiPath(type,'/Sessions/Logout'),url=new URL(path,`${server.base_url}/`),response=await outbound.safeFetch(url,{purpose:`Stremio restricted logout on ${server.name}`,method:'POST',timeoutMs:8000,headers:restrictedTokenHeaders(type,token)});return response.ok;
  }catch(_error){console.warn('Stremio restricted media-server logout failed.');return false;}
}
...
async function suspend(customerId,reason='No active Stremio entitlement'){
  const rows=await query(`SELECT e.id,e.jellyfin_account_id,e.jellyfin_access_token_encrypted,js.id server_id,js.name server_name,js.base_url,js.media_server_type FROM stremio_entitlements e LEFT JOIN jellyfin_servers js ON js.id=e.server_id WHERE e.customer_id=$1`,[customerId]);
...
module.exports={TOKEN_ENV,TOKEN_PREFIX,serviceType,streamLimit,jellyfinAuthHeader,clientAuthorization,restrictedTokenHeaders,entitledSubscription,explicitSourceCount,usesSharedSources,selectServer,authenticateRestrictedUser,logoutRestrictedToken,refreshRestrictedAccess,reconcileForCustomer,reconcileSharedForCustomer,issueInstallation,revoke,current,findByInstallToken,accessToken,markUse,suspend};
