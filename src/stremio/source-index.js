...
async function dueSources(){const r=await query(`SELECT s.id FROM stremio_sources s JOIN stremio_source_index_state i ON i.source_id=s.id WHERE s.enabled=TRUE AND s.auth_state='connected' AND EXISTS(SELECT 1 FROM stremio_source_libraries l WHERE l.source_id=s.id AND l.selected=TRUE AND l.available=TRUE) AND (i.status IN ('never','queued','failed') OR i.next_incremental_at<=NOW()) ORDER BY i.force_full DESC,COALESCE(i.next_incremental_at,'1970-01-01'::timestamptz),s.priority,s.name`);return r.rows;}
...
module.exports={PAGE_SIZE,PAGE_DELAY_MS,INCREMENTAL_HOURS,FULL_RECONCILE_DAYS,normalizeImdb,selectedLibraries,state,queue,indexSource,indexDueSources,lookup,states};
