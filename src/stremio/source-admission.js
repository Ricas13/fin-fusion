...
const MANAGED_LEASE_SECONDS=20;
...
async function admit(entitlement,rawLease,sourceId,itemId,metadata={}){
  const leaseHash=hash(rawLease),entitlementId=entitlement?.id,customerId=entitlement?.customer_id,meta=cleanMetadata(metadata),leaseSeconds=meta.managedMappingId?MANAGED_LEASE_SECONDS:LEASE_SECONDS;
  ...
  return transaction(async db=>{
    ...
    if(existing.rowCount){
      await db.query(`UPDATE stremio_source_playback_leases SET source_id=$3,item_id=$4,last_seen_at=NOW(),expires_at=NOW()+($5||' seconds')::interval,
        ...
        WHERE lease_hash=$1 AND entitlement_id=$2`,[leaseHash,entitlementId,sourceId,String(itemId),String(leaseSeconds),meta.managedMappingId,meta.serverId,meta.jellyfinUserId,meta.deviceId,meta.playSessionId,meta.mediaSourceId,meta.playMethod]);
    ...
    await db.query(`INSERT INTO stremio_source_playback_leases(lease_hash,entitlement_id,customer_id,source_id,item_id,first_seen_at,last_seen_at,expires_at,managed_mapping_id,server_id,jellyfin_user_id,device_id,play_session_id,media_source_id,play_method,lifecycle_started_at,lifecycle_last_seen_at)
      VALUES($1,$2,$3,$4,$5,NOW(),NOW(),NOW()+($6||' seconds')::interval,$7,$8,$9,$10,$11,$12,$13,CASE WHEN $7::uuid IS NULL THEN NULL ELSE NOW() END,CASE WHEN $7::uuid IS NULL THEN NULL ELSE NOW() END)`,[leaseHash,entitlementId,customerId,sourceId,String(itemId),String(leaseSeconds),meta.managedMappingId,meta.serverId,meta.jellyfinUserId,meta.deviceId,meta.playSessionId,meta.mediaSourceId,meta.playMethod]);
...
async function touchHash(leaseHash,{seconds=LEASE_SECONDS}={}){const value=String(leaseHash||'');if(!/^[a-f0-9]{64}$/.test(value))return false;const r=await query(`UPDATE stremio_source_playback_leases SET last_seen_at=NOW(),lifecycle_last_seen_at=NOW(),expires_at=NOW()+($2||' seconds')::interval WHERE lease_hash=$1`,[value,String(Math.max(5,Math.min(600,Number(seconds)||LEASE_SECONDS)))]);return r.rowCount>0;}
...
module.exports={LEASE_SECONDS,MANAGED_LEASE_SECONDS,issue,hash,playMethod,cleanMetadata,admit,touch,touchHash,release,releaseHash,active,cleanup};