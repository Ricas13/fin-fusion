...
const SESSION_ACTIVE_SECONDS=240;
...
async function start(mapping,rawLease,{itemId,mediaSourceId,playSessionId}){
  ...
  try{await restrictedPost(mapping,token,'/Sessions/Playing',body,id);}catch(error){await logoutToken(mapping,token,id).catch(()=>{});throw error;}
  ...
  return{deviceId:id,jellyfinSessionId:session?.Id?String(session.Id):null,accessToken:token};
}
...
async function activeManagedLeases(limit=2000){
  const result=await query(`SELECT l.*,js.name server_name,js.base_url,ja.jellyfin_user_id
...
async function stopLease(row,reason='stale'){
  ...
  if(row.playback_token_encrypted){try{const token=decryptWithEnv(row.playback_token_encrypted,entitlements.TOKEN_ENV,PLAYBACK_TOKEN_PREFIX);await logoutToken(row,token,row.device_id);}catch(error){console.warn(`Unable to revoke managed Stremio playback token: ${error.message}`);}}
...
async function reconcile(){
  ...
  for(const row of rows){const sessions=snapshot.byServer.get(String(row.server_id))||[],session=matchingSession(row,sessions),started=Date.parse(row.lifecycle_started_at||row.first_seen_at||'')||0;
    if(sessionFresh(session,now)){
      active+=1;await sourceAdmission.touchHash(row.lease_hash);const position=Number(session?.PlayState?.PositionTicks||0)||0;
...
module.exports={SESSION_ACTIVE_SECONDS,START_GRACE_SECONDS,PLAYBACK_TOKEN_PREFIX,deviceId,loginAuthorization,authorization,authenticatePlayback,playbackBody,resolveSession,start,logoutToken,activeManagedLeases,snapshotServers,matchingSession,sessionFresh,stopLease,reconcile,startManager};
