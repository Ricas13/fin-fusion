...
async function validateForManagedStremio(serverId){
  ...
  const type=registry.mediaProvider.normalizeType(server.media_server_type),label=registry.mediaProvider.label(type);
  if(!server.enabled)throw new Error(`Enable the ${label} server before enabling it for Stremio.`);
  if(!server.public_url)throw new Error(`A public ${label} URL is required for direct Stremio playback.`);
  if(!server.api_configured)throw new Error(`A ${label} API key is required before enabling this managed Stremio source.`);
  await registry.request(serverId,registry.mediaProvider.credentialProbeEndpoint(type),{timeoutMs:8000});
  return server;
}
...
async function enabled(){
  const result=await query(`SELECT id,name,slug,server_class,media_server_type,base_url,public_url,priority,stremio_priority
    ...
    WHERE enabled=TRUE AND stremio_enabled=TRUE
