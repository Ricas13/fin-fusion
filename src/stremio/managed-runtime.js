...
async function itemDetails(mapping,runtime,item){
  ...
  const qs=new URLSearchParams({Fields:'Path,MediaSources,MediaStreams',EnableImages:'false',EnableUserData:'false'});
  const detailed=await jellyfin.restrictedRequest(runtime,`/Users/${encodeURIComponent(String(mapping.jellyfin_user_id))}/Items/${encodeURIComponent(id)}?${qs.toString()}`);
  if(!detailed?.Id)return item;
  return{...item,...detailed,id:String(detailed.Id),name:detailed.Name||item.name||item.Name,path:detailed.Path||item.path||item.Path||null};
}
...
async function resolveManagedItems(mapping,runtime,args){
  ...
  const settled=await Promise.allSettled(indexedRows.map(async indexed=>{
    const endpoint=episodeResolution.userItemsPath({userId:mapping.jellyfin_user_id,seriesId:indexed.item_id,season:args.season,episode:args.episode,fields:'Path,MediaSources,MediaStreams'});
...
async function streamsFromMapping(mapping,args,type,videoId){
  ...
  const settled=await Promise.allSettled(items.map(async original=>{
    const item=await itemDetails(mapping,runtime,original),sources=mediaSources(item);
    return sources.map(source=>{
      const filename=jellyfin.sourceFilename(item,source),quality=jellyfin.sourceQuality(source,filename),display=foundation.streamDisplayFromFilename(filename);
...
async function streamsFor(entitlement,type,videoId){
  ...
  for(let index=0;index<settled.length;index+=1){const result=settled[index],mapping=mappings[index];if(result.status==='fulfilled'){output.push(...result.value.map(row=>row.stream));continue;}await query(`UPDATE stremio_managed_accounts SET last_error=$2,updated_at=NOW() WHERE id=$1`,[mapping.id,String(result.reason?.message||result.reason).slice(0,1000)]).catch(()=>{});console.warn(`Managed Stremio raw stream resolution failed on ${mapping.server_name}:`,result.reason?.message||result.reason);}
  return output;
}
