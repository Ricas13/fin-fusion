...
async function disableMapping(row,reason='Managed Stremio access suspended'){
  ...
  await entitlements.logoutRestrictedToken({id:row.server_id,name:row.server_name,base_url:row.base_url,media_server_type:row.media_server_type},row.access_token_encrypted).catch(()=>{});
  await provisioning.disableJellyfinAccount(account).catch(error=>console.warn(`Unable to disable managed Stremio media-server account ${row.jellyfin_account_id}:`,error.message));
