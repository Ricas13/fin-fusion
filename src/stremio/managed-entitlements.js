...
async function ensureSource(entitlement,source,effective){return operationLock.withLock(`managed-account:${entitlement.customer_id}:${source.id}`,async()=>{const current=(await currentMappings(entitlement.id,[source.id]))[0]||null;if(mappingReady(current))return current;const server=await serverFor(source.id),account=await internalAccount(entitlement.customer_id,source.id);if(!account)return createMapping(entitlement,server,effective);try{await registry.request(server.id,`/Users/${encodeURIComponent(account.jellyfin_user_id)}`,{method:'GET',timeoutMs:5000});}catch(error){if(!remoteMissing(error))throw error;return recreateMissingManagedAccount(entitlement,server,account,effective);}return recoverMapping(entitlement,server,account,effective,current);});}
