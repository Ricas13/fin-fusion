...
async function disableMapping(row,reason='Managed Stremio access suspended'){
  const account={id:row.jellyfin_account_id,server_id:row.server_id,jellyfin_user_id:row.jellyfin_user_id,jellyfin_username:row.jellyfin_username,account_purpose:'stremio_internal'};
...
async function revokeInactiveMappings(){
  const rows=(await query(`WITH effective AS (
    ...
    SELECT sma.id mapping_id,sma.server_id,sma.jellyfin_account_id,sma.access_token_encrypted,
      ja.jellyfin_user_id,ja.jellyfin_username,js.name server_name,js.base_url
    ...
    WHERE sma.status='active' AND (
      e.status<>'active' OR js.enabled=FALSE OR js.stremio_enabled=FALSE OR
      ee.subscription_id IS NULL OR ee.blocked=TRUE OR ee.access_expires_at<=NOW()
    )`)).rows;
  for(const row of rows)await disableMapping(row,'Managed Stremio entitlement is no longer active.');
  return rows.length;
}
async function ensure(entitlement){
  ...
  for(const source of sources){const existing=(await query(`SELECT sma.*,ja.jellyfin_user_id,ja.jellyfin_username,ja.disabled account_disabled FROM stremio_managed_accounts sma JOIN jellyfin_accounts ja ON ja.id=sma.jellyfin_account_id WHERE sma.entitlement_id=$1 AND sma.server_id=$2`,[entitlement.id,source.id])).rows[0]||null;if(existing&&existing.status==='active'&&!existing.account_disabled&&existing.access_token_encrypted){ready.push(existing);continue;}const server=await serverFor(source.id);try{const account=await internalAccount(entitlement.customer_id,source.id),mapped=account?await recoverMapping(entitlement,server,account,plan,effective,limit,existing):await createMapping(entitlement,server,plan,effective,limit);ready.push(mapped);}catch(error){await query(`UPDATE stremio_managed_accounts SET status='error',last_error=$3,updated_at=NOW() WHERE entitlement_id=$1 AND server_id=$2`,[entitlement.id,source.id,String(error.message||error).slice(0,1000)]).catch(()=>{});console.warn(`Managed Stremio identity provisioning failed on ${source.name}:`,error.message);}}
  return ready;
}
async function mappings(entitlement){await ensure(entitlement);return managedSources.accountsForEntitlement(entitlement.id);}

module.exports={hiddenUsername,planFor,serverFor,internalAccount,applyPolicy,ensure,mappings,disableStale,disableMapping,revokeInactiveMappings};
