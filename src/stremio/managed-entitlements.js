...
async function applyPolicy(account,plan,effective,limit,disabled=false){
  ...
  const body={...provisioning.policyBody(effective.technical,disabled,access),MaxActiveSessions:disabled?0:limit,EnableContentDownloading:false,EnableSyncTranscoding:false,EnableMediaConversion:false,EnableLiveTvManagement:false,EnableUserPreferenceAccess:false};
  await registry.request(account.server_id,`/Users/${account.jellyfin_user_id}/Policy`,{method:'POST',body});return access;
}
async function createMapping(entitlement,server,plan,effective,limit){
  ...
  const auth=await entitlements.authenticateRestrictedUser(server.id,account.jellyfin_username,bootstrap);if(String(auth.userId)!==String(account.jellyfin_user_id))throw new Error('Restricted Jellyfin authentication returned the wrong user identity.');
...
async function recoverMapping(entitlement,server,account,plan,effective,limit,prior=null){
  await applyPolicy(account,plan,effective,limit,false);await query(`UPDATE jellyfin_accounts SET disabled=FALSE,is_primary=FALSE,password_setup_required=FALSE,updated_at=NOW() WHERE id=$1`,[account.id]);
...
async function disableStale(entitlement,allowedIds){
  const rows=(await query(`SELECT sma.id mapping_id,ja.* FROM stremio_managed_accounts sma JOIN jellyfin_accounts ja ON ja.id=sma.jellyfin_account_id WHERE sma.entitlement_id=$1 AND NOT (sma.server_id=ANY($2::uuid[])) AND sma.status='active'`,[entitlement.id,allowedIds])).rows;
  for(const account of rows){await provisioning.disableJellyfinAccount(account).catch(()=>{});await query(`UPDATE stremio_managed_accounts SET status='suspended',updated_at=NOW() WHERE id=$1`,[account.mapping_id]).catch(()=>{});}
}
async function ensure(entitlement){
  ...
  const plan=await planFor(entitlement),limit=entitlements.streamLimit(entitlement),effective=await provisioning.effectivePolicyForCustomer(entitlement.customer_id,plan),ready=[];
  for(const source of sources){
    const existing=(await query(`SELECT sma.*,ja.jellyfin_user_id,ja.jellyfin_username,ja.disabled account_disabled FROM stremio_managed_accounts sma JOIN jellyfin_accounts ja ON ja.id=sma.jellyfin_account_id WHERE sma.entitlement_id=$1 AND sma.server_id=$2`,[entitlement.id,source.id])).rows[0]||null;
    ...
    try{const account=await internalAccount(entitlement.customer_id,source.id),mapped=account?await recoverMapping(entitlement,server,account,plan,effective,limit,existing):await createMapping(entitlement,server,plan,effective,limit);ready.push(mapped);}catch(error){await query(`UPDATE stremio_managed_accounts SET status='error',last_error=$3,updated_at=NOW() WHERE entitlement_id=$1 AND server_id=$2`,[entitlement.id,source.id,String(error.message||error).slice(0,1000)]).catch(()=>{});console.warn(`Managed Stremio identity provisioning failed on ${source.name}:`,error.message);}
  }
  return ready;
}
async function mappings(entitlement){await ensure(entitlement);return managedSources.accountsForEntitlement(entitlement.id);}

module.exports={hiddenUsername,planFor,serverFor,internalAccount,applyPolicy,ensure,mappings,disableStale};
