...
function directUrl(source,itemId,mediaSourceId=null){
  ...
  url.searchParams.set('Static','true');if(mediaSourceId)url.searchParams.set('MediaSourceId',String(mediaSourceId));url.searchParams.set('api_key',client.sourceToken(source));return url.toString();
}
...
async function items(source,args){
  ...
  const settled=await Promise.allSettled(indexed.map(async row=>{
    const qs=new URLSearchParams({UserId:String(source.jellyfin_user_id),Season:String(args.season),Fields:'Path,MediaSources,MediaStreams',StartIndex:'0',Limit:'500',EnableImages:'false'}),payload=await client.request(source,`/Shows/${encodeURIComponent(row.item_id)}/Episodes?${qs.toString()}`);
...
module.exports={parseVideoId,filename,quality,description,directUrl,roots,itemDetails,items,mediaSources,streamsFrom,streamsFor};
