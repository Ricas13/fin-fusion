...
async function indexServer(serverId,{pageSize=500}={}){
  ...
  try{
    while(true){
      ...
      const payload=await registry.request(serverId,`/Items?${qs.toString()}`,{timeoutMs:30000});
      const items=valueItems(payload);
      if(!items.length)break;
      await transaction(async client=>{
        for(const item of items){
          const imdb=normalizeImdb(item.ProviderIds?.Imdb||item.ProviderIds?.IMDB||item.providerIds?.Imdb);
...
module.exports={normalizeImdb,eligibleServers,indexServer,indexAll,lookup,states};
