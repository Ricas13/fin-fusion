...
async function data(req){
  const [user,communication,events,prefs,email,delivery,adminDiscordRedirect]=await Promise.all([
    ...
    query(`SELECT event_type,display_name,description,email_enabled,telegram_enabled,discord_enabled,whatsapp_enabled FROM notification_preferences WHERE event_scope IN ('admin','both') ORDER BY event_type`),
  ...
  const selected={};for(const row of prefs.rows){if(!selected[row.event_type])selected[row.event_type]={};selected[row.event_type][row.channel]=Boolean(row.enabled);}
...
function channelCard(req,d,channel){
  ...
  if(channel==='discord'){
    ...
    return `<div class="serverCard"><div class="serverTop"><div><strong>Discord</strong><div class="subText">${personal?`Connected${c.discord_handle?` as ${esc(c.discord_handle)}`:''}`:'Not connected'}</div></div>${statusPill(ready?'Ready':!global?'Unavailable globally':'Connect account',ready?'good':'warn')}</div><div class="card-body"><div class="buttonRow">${personal?`<form method="post" action="/admin/profile/notifications/discord/unlink">${token(req)}<button class="button secondary btn-sm">Disconnect</button></form>${global?`<form method="post" action="/admin/profile/notifications/test/discord">${token(req)}<button class="button secondary btn-sm">Send test</button></form>`:''}`:global?`<form method="post" action="/admin/profile/notifications/discord/start">${token(req)}<button class="button btn-sm">Connect Discord</button></form>`:'<a class="button secondary btn-sm" href="/admin/notifications/preferences">Configure globally</a>'}</div>${global?`<details class="detailsBox" style="margin-top:10px"><summary>Discord OAuth setup reference</summary><div class="subText">Admin redirect URI</div><code>${esc(d.adminDiscordRedirect)}</code></details>`:''}</div></div>`;
  }
  const global=Boolean(delivery.whatsappConfigured),personal=Boolean(c.whatsapp_opt_in&&c.phone_e164),ready=global&&personal;
  return `<div class="serverCard"><div class="serverTop"><div><strong>WhatsApp</strong><div class="subText">${esc(c.phone_e164||'No number saved')}</div></div>${statusPill(ready?'Ready':!global?'Unavailable globally':personal?'Ready':'Opt in',ready?'good':'warn')}</div><div class="card-body"><form method="post" action="/admin/profile/notifications/whatsapp">${token(req)}<input class="input" name="phone" value="${esc(c.phone_e164||'')}" placeholder="+447700900123"><label class="checkRow"><input type="checkbox" name="enabled" ${c.whatsapp_opt_in?'checked':''}> Use WhatsApp for selected events</label><div class="buttonRow"><button class="button secondary btn-sm">Save</button>${ready?`<button class="button secondary btn-sm" formaction="/admin/profile/notifications/test/whatsapp">Send test</button>`:''}${!global?'<a class="button secondary btn-sm" href="/admin/notifications/preferences">Configure globally</a>':''}</div></form></div></div>`;
}
...
function createAdminPersonalNotificationPreferencesRouter(){
  ...
  // Compatibility for an older Notifications-tab currency form. Currency now belongs to Profile.
