...
function layout(options = {}) {
    ...
    const mapped = ['automation-jobs','policy-drift'].includes(requested) ? 'provisioning'
        : requested === 'notification-events' ? 'notifications'
    ...
    const notificationEvents = `<a class="adminSubTab ${requested==='notification-events'?'active':''}" href="/admin/notifications/preferences">Event Delivery</a>`;
    ...
    if (!html.includes('href="/admin/provisioning/drift"')) {
        html = html.replace(/(<a class="adminSubTab[^"]*" href="\/admin\/notifications">Notifications<\/a>)/, `${driftLink}$1${notificationEvents}${automationLink}`);
    } else {
        if (!html.includes('href="/admin/notifications/preferences"')) html = html.replace(/(<a class="adminSubTab[^"]*" href="\/admin\/notifications">Notifications<\/a>)/, `$1${notificationEvents}`);
        if (!html.includes('href="/admin/automation"')) html = html.replace(/(<a class="adminSubTab[^"]*" href="\/admin\/notifications(?:\/preferences)?">[^<]+<\/a>)/, `$1${automationLink}`);
    }

    const commerceLink = `<a class="adminSubTab ${requested==='commerce-overview'?'active':''}" href="/admin/commerce">Overview</a>`;
    if (!html.includes('href="/admin/commerce"')) html = html.replace(/(<a class="adminSubTab[^"]*" href="\/admin\/plans">Plans<\/a>)/, `${commerceLink}$1`);
    ...
    if (!html.includes('href="/admin/settings/resellers"')) html = html.replace(/(<a class="adminSubTab[^"]*" href="\/admin\/settings\/branding">Branding<\/a>)/, `$1${resellerSettings}${configTransfer}${configHealth}`);
    ...
    if (!html.includes('href="/admin/search"')) html = html.replace('<a class="headerButton hideMobile" href="/"', `${quickLinks}<a class="headerButton hideMobile" href="/"`);
    return html;
}
