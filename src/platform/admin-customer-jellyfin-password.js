...
async function rows(search=''){
    const value=String(search||'').trim().slice(0,120),needle=`%${value}%`;
...
function createAdminCustomerJellyfinPasswordRouter(){
    ...
    router.post('/admin/customer-jellyfin-password/:customerId/:accountId',async(req,res)=>{
        ...
        if(password.length<12||password.length>200)return res.redirect('/admin/customer-jellyfin-password?error='+encodeURIComponent('Jellyfin password must be between 12 and 200 characters.'));
        if(password!==confirm)return res.redirect('/admin/customer-jellyfin-password?error='+encodeURIComponent('Jellyfin passwords do not match.'));
        try{
            ...
            return res.redirect('/admin/customer-jellyfin-password?message='+encodeURIComponent('Jellyfin password changed.'));
        }catch(error){return res.redirect('/admin/customer-jellyfin-password?error='+encodeURIComponent(error.message||'Jellyfin password could not be changed.'));}
    });
    return router;
}
