...
async function page(plan, req, error = '', body = null) {
  ...
  ${v.stremio ? `<h3>Stremio household</h3><div class="securityNote standalone"><strong>1 household slot (IPv4 + IPv6 /64)</strong><div class="subText">Unlimited Stremio playback inside the household slot: one IPv4 address and one IPv6 /64 prefix are allowed before another same-family network is rejected.</div></div><div class="formGroup narrow"><label>Stremio network lease</label><div class="inputUnit"><input class="input" type="number" min="15" max="1440" name="stremioHouseholdLeaseMinutes" value="${esc(v.stremioHouseholdLeaseMinutes)}"><span>minutes</span></div><div class="inlineHelp">This lease refreshes on playback and can be reset from the customer or admin Stremio controls.</div></div>` : ''}
