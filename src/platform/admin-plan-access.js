...
async function page(plan, req, error = '', body = null) {
  ...
  <section class="section"><div class="sectionHead"><div><h2>Access & playback</h2><div class="muted">Concurrent streams are always independent. The legacy household lease can optionally run alongside them.</div></div><span class="muted">${affected} live entitlement${affected === 1 ? '' : 's'}</span></div>
...
function parse(plan, body) {
  ...
  return {
    ...
    stremio,
    accessModel,
    streams: jellyfin ? int(body.streams, 1, 50, 'Concurrent streams') : (Number.isInteger(Number(plan.streams)) ? Number(plan.streams) : 1),
