...
async function completePasswordChange(raw){
  return transaction(async client=>{
    ...
    const user=(await client.query(`SELECT password_hash FROM app_users WHERE id=$1 AND role='customer' FOR UPDATE`,[token.user_id])).rows[0];if(!user)return null;
    if(passwordDigest(user.password_hash)!==String(token.metadata?.basePasswordDigest||'')){await client.query(`UPDATE account_tokens SET consumed_at=NOW() WHERE id=$1`,[token.id]);return null;}
    const passwordHash=String(token.metadata?.passwordHash||'');if(!passwordHash.startsWith('$2'))return null;
    ...
    await client.query(`INSERT INTO audit_log(actor_user_id,action,entity_type,entity_id,metadata) VALUES($1::uuid,'customer.password.change','app_user',$1::text,$2::jsonb)`,[token.user_id,JSON.stringify({confirmation:'verified_email',revokedSessions:revoked,sessionVersion:Number(updated.rows[0].session_version)})]);
...
async function approveOldEmail(raw){
  ...
  await runtimeSettings.ensureLoaded();const site=runtimeSettings.siteName(),url=(await baseUrl())+`/account/confirm-portal-email-new?token=${encodeURIComponent(result.nextRaw)}`;
...
async function completeNewEmail(raw){
  const result=await transaction(async client=>{
    ...
    const revoked=await revokeAllSessions(client,token.user_id);
    await client.query(`UPDATE account_tokens SET consumed_at=NOW() WHERE id=$1`,[token.id]);
    await invalidate(client,token.user_id,[EMAIL_OLD_TOKEN,'email_change']);
  ...
  if(result){const site=runtimeSettings.siteName(),securityUrl=(await baseUrl())+'/account/security';sendSecurityMail({to:result.oldEmail,subject:`${site} email address changed`,title:'Portal email address changed',text:`Your portal email address was changed to ${emailChange.maskEmail(result.email)} after approval from the previous address and verification of the new address. All portal sessions were signed out. If this was not you, contact support immediately.`,actionLabel:'Review account security',actionUrl:securityUrl,dedupeKey:null}).catch(error=>console.warn('Old-email completion notice failed:',error.message));}
  return result;
}
