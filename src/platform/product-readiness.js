...
async function evaluatePlan(plan,ctx=null){
  ...
  const sourceState=await planSourceState(plan?.id||plan?.plan_id),managedReady=managedPlanReady(resolved),externalReady=sourceState.ready>0;
...
module.exports={serviceType,catalogueState,stremioContext,context,evaluate,planSourceState,managedPlanReady,evaluatePlan,planByCode,assertSellablePlan,assertSellableCode,deliveryLabel};
