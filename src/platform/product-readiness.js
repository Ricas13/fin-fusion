...
async function assertSellablePlan(plan,ctx=null){if(!plan)throw new Error('This plan is not available for new sale.');const readiness=await evaluatePlan(plan,ctx);if(!readiness.sellable){const error=new Error(`This plan cannot be sold right now: ${readiness.label}.`);error.code=`PLAN_${String(readiness.key||'UNAVAILABLE').toUpperCase()}`;error.readiness=readiness;throw error;}return readiness;}
