...
async function insightData(customerId,rawRange){
  ...
  const [summaryResult,previousResult,topResult,deviceResult,timelineResult,heatResult,recentResult]=await Promise.all([
    ...
    optionalInsightQuery('heatmap',()=>query(`SELECT EXTRACT(ISODOW FROM ph.started_at)::int AS "day",EXTRACT(HOUR FROM ph.started_at)::int AS "hour",COALESCE(SUM(${duration}),0) seconds FROM playback_history ph WHERE ${predicate} GROUP BY 1,2 ORDER BY 1,2`,params)),
