...
function syncSummaryMessage(prefix, result) {
  const ignored = result.ignored ? `, ${result.ignored} already without a managed request account` : '';
  return `${prefix}: ${result.created} created, ${result.linked} active/linked, ${result.suspended} removed/no access, ${result.failed} failed${ignored}.`;
}
...
async function page(req) {
  ...
  const lifecycleCard = `<section class="card"><div class="card-header"><div><h2 class="card-title">Request access lifecycle</h2><div class="muted">CAPTAiNFiN owns request identity, plan quotas and plan-selected Jellyseerr policy; Jellyseerr remains the content-request frontend.</div></div></div><div class="card-body"><div class="compact-item"><div><div class="compact-title">One request identity per customer</div><div class="compact-meta">A customer can move between Jellyfin servers without creating another request account.</div></div><span class="pill good">Centralised</span></div><div class="compact-item"><div><div class="compact-title">Plan request policy</div><div class="compact-meta">Quota, request access and optional customer-safe Jellyseerr permissions come from the current CAPTAiNFiN plan.</div></div><span class="pill good">Plan controlled</span></div><div class="compact-item"><div><div class="compact-title">Subscription lifecycle</div><div class="compact-meta">Only customers with current request entitlement keep a managed Seerr account. When eligibility ends, the exact linked account is deleted; a future entitlement creates a fresh local account.</div></div><span class="pill good">Automatic</span></div></div></section>`;
  const metrics = `<div class="metrics" style="margin-top:16px"><div class="metric"><div class="metricLabel">Active request access</div><div class="metricValue smallish">${active}</div></div><div class="metric"><div class="metricLabel">No request account</div><div class="metricValue smallish">${summary.suspended || 0}</div></div><div class="metric"><div class="metricLabel">Need request password</div><div class="metricValue smallish">${passwordsNeeded}</div></div><div class="metric"><div class="metricLabel">Failed</div><div class="metricValue smallish">${counts.failed || 0}</div></div></div>`;

  const rows = candidates.map(row => {
    const loginEmail = row.external_email || row.email || 'Assigned at first sync', state = row.status || 'pending';
    let access;
    if (!row.entitlement_active) access = '<span class="pill warn">No request access</span>';
...
function createAdminRequestUsersRouter() {
  ...
  router.post('/admin/request-users/:customerId/sync', writeLimit, async (req, res) => {
    ...
    try {
      ...
      const message = result.status === 'synced' ? 'Request user synced.' : result.status === 'suspended' ? 'No valid request entitlement; linked Seerr account removed.' : result.status === 'ignored' ? 'No valid request entitlement and no managed Seerr account to remove.' : `Request user ${result.status}.`;
