...
async function page(req) {
    ...
    const passwordsNeeded = candidates.filter(row => row.password_reset_required).length;
    const configCard = `<section class="card">
        ...
        <div class="card-body">
            ...
            <div class="compact-item"><div><div class="compact-title">Sync model</div><div class="compact-meta">CAPTaINFiN deduplicates users across all Jellyfin servers. Existing request accounts are linked by saved external ID or email; users without an email receive a private non-deliverable request login address. Expired/removed users are never auto-deleted, preserving request history.</div></div><span class="pill good">Non-destructive</span></div>
        </div>
    </section>`;
...
function createAdminRequestUsersRouter() {
    ...
    router.post('/admin/request-users/sync-all', async (req, res) => {
        ...
        try {
            ...
            const message = `Request users synced: ${result.created} created, ${result.linked} linked, ${result.failed} failed.`;
