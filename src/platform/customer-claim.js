...
function publicForm(req, claim, values = {}, error = null) {
    ...
    const emailField = claim.email_lock
        ...
        : `<div class="field"><label>Email${verificationRequired ? '' : ' <span class="help">(optional)</span>'}</label><input class="input" type="email" name="email" maxlength="254" autocomplete="email" ${verificationRequired ? 'required' : ''} value="${esc(values.email || claim.customer_email || '')}"><div class="help">${verificationRequired ? 'Required because this portal verifies customer email addresses before sign-in.' : 'Optional. You can sign in with the portal username even without an email.'}</div></div>`;
