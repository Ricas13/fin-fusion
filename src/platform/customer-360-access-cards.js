...
function accessOverview(detail,token,accessDetail){
  ...
  const warning=failed?`<div class="notice error"><strong>Provisioning failed / Needs attention.</strong> ${esc(provisioning.last_error||'Jellyfin access could not be created.')} ${freePlan?'This Free Server entitlement remains allocated while you repair or manually assign access.':'The entitlement remains active while you repair access.'}</div>`:'';
...
function technicalControls(detail,token,accessDetail){
  ...
  return `<section class="section accessControlsSection"><div class="sectionHead"><div><h2>Access controls</h2><div class="muted">Plan defaults stay intact unless you explicitly override a customer.</div></div>${pill(overrides?`${overrides} override${overrides===1?'':'s'}`:'Plan defaults',overrides?'accent':'good')}</div><form method="post" action="/admin/users/${encodeURIComponent(customerId)}/policy-overrides">${csrfHidden(token)}<div class="accessControlGrid">${cards}</div><div class="accessSaveRow"><button class="button primary" type="submit">Save access changes</button><span class="muted">Inherit removes the customer override and follows the plan again.</span></div></form><form class="plainForm" method="post" action="/admin/users/${encodeURIComponent(customerId)}/policy-overrides/reset-all">${csrfHidden(token)}<button class="button secondary" type="submit">Reset access controls to plan</button></form></section>`;
}

function libraryChoice(row){
  const hasOverride=row.override!==null&&row.override!==undefined;
  return `<div class="accessLibraryRow"><div><strong>${esc(row.name)}</strong><span>Plan ${row.plan?'enabled':'disabled'} · Effective ${row.effective?'enabled':'disabled'}</span></div><input type="hidden" name="libraryName" value="${esc(row.name)}"><select class="input compact" name="libraryValue" aria-label="${esc(row.name)} library override"><option value="" ${!hasOverride?'selected':''}>Inherit</option><option value="true" ${hasOverride&&row.override===true?'selected':''}>Allow</option><option value="false" ${hasOverride&&row.override===false?'selected':''}>Deny</option></select></div>`;
}
