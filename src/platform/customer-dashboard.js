...
function createCustomerDashboardRouter(){
  ...
  r.get('/account',requireCustomer,async(req,res,next)=>{
    try{
      ...
      const paymentFlags={stripeEnabled:stripe.enabled(),paypalEnabled:paypal.enabled(),coingateEnabled:coingate.enabled()};
      if(!currentPlan){
        const openCheckout=await checkoutIntents.getOpenForOwner('customer',req.session.customerId).catch(()=>null);
        return res.render('customer/onboarding',{portal,plans,...paymentFlags,currency,openCheckout,csrfToken:csrf.token(req),siteName:runtimeSettings.siteName(),message:req.query.message||restoreMessage||null,error:req.query.error||restored.error||null});
      }
      ...
      if(delivery==='stremio'){
        return res.render('customer/stremio-dashboard',{portal,plans,currentPlan,...paymentFlags,currency,csrfToken:csrf.token(req),siteName:runtimeSettings.siteName(),message:req.query.message||restoreMessage||null,error:req.query.error||restored.error||null,permanentAccess:isPermanent});
      }
