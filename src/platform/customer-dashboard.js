...
function createCustomerDashboardRouter(){
  ...
  r.get('/account',requireCustomer,async(req,res,next)=>{
    try{
      ...
      const navOptions=customerNav.optionsFromPortal(portalRaw);
      const portal=canonicalizePortalSubscriptions(await tagMediaServerAccounts(customerId,await hideInternalAccounts(customerId,portalRaw)),accessRows),paymentFlags={stripeEnabled:stripe.enabled(),paypalEnabled:paypal.enabled(),plisioEnabled:plisio.enabled()},openCheckout=await checkoutIntents.getOpenForOwner('customer',customerId).catch(()=>null);
