...
function sidebarPage(current,[key,label,url]){
  const parentSelected=current.sidebarKey===key;
  const exactSelected=current.key===key;
  const children=nav.childPages(key);
  const childMarkup=children.length?`<div class="adminTabChildren" aria-label="${esc(label)} tools">${children.map(([childKey,childLabel,childUrl])=>{const selected=current.key===childKey;return `<a class="adminSubTab ${selected?'active':''}" href="${esc(childUrl)}" title="Open ${esc(childLabel)}" ${selected?'aria-current="page"':''}>${esc(childLabel)}</a>`}).join('')}</div>`:'';
  return `<div class="adminTabGroup"><a class="adminTab ${parentSelected?'active':''}" href="${esc(url)}" title="Open ${esc(label)}" ${exactSelected?'aria-current="page"':''}>${esc(label)}</a>${childMarkup}</div>`;
}
...
function header(active,site){
  ...
  const sections=nav.groups.map(group=>{
    ...
    const pages=group.pages.map(page=>sidebarPage(current,page)).join('');
...
// Compatibility export retained for older callers. Payment destinations are
// now nested beneath Payments & Billing in the canonical sidebar.
function paymentTabsFor(){return'';}
...
function layout(options={}){
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark"><title>${esc(options.title)} · ${esc(site)}</title><link rel="icon" href="${esc(favicon)}"><style>${critical}</style><link rel="stylesheet" href="/css/admin-original-base.css"><link rel="stylesheet" href="/css/admin-original-components.css"><link rel="stylesheet" href="/css/customer-360.css"><link rel="stylesheet" href="/css/admin-server-library-dashboard.css"><link rel="stylesheet" href="/css/admin-form-feedback.css"><link rel="stylesheet" href="/css/admin-section-graphics.css"><link rel="stylesheet" href="/css/admin-visual-refinement.css"><link rel="stylesheet" href="/css/operator-experience.css"><link rel="stylesheet" href="/css/admin-capability.css"></head><body><a class="skipLink" href="#admin-main">Skip to main content</a><div class="appShell">${header(options.active,site)}<main class="mainPane" id="admin-main" tabindex="-1" aria-labelledby="adminPageTitle"><header class="topBar"><div class="topBreadcrumb" aria-label="Current location"><span>${esc(current.group.label)}</span><strong>${esc(current.page[1])}</strong></div>${quickFind}${topActions}</header><div class="content"><div class="pageHeader"><div><h1 id="adminPageTitle">${esc(options.title)}</h1><div class="pageSubtitle">${esc(options.subtitle||'')}</div></div></div>${options.body||''}</div></main></div><script src="/js/admin-form-feedback.js" defer></script><script src="/js/operator-experience.js" defer></script></body></html>`;
}

module.exports={esc,layout,header,sidebarPage,paymentTabsFor};
