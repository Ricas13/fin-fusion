...
function commerceCard(data, req) {
  ...
  const mappings = priceRow ? `<form class="planConfigBody" method="post" action="/admin/plans/${esc(p.id)}/editor-payments">${token(req)}<div class="planPaymentOptions">${paymentChoice(state, currency, 'stripe', 'payment', 'Stripe · one-time', true)}${paymentChoice(state, currency, 'stripe', 'subscription', 'Stripe · recurring', true)}${paymentChoice(state, currency, 'paypal', 'payment', 'PayPal · one-time', false)}${paymentChoice(state, currency, 'paypal', 'subscription', 'PayPal · recurring', true)}</div><div class="buttonRow"><button class="button" type="submit">Verify & save payment options</button></div></form>` : '<div class="planConfigBody"><div class="empty">Save the commercial schedule first, then configure payment options.</div></div>';
