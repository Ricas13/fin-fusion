...
function manualEditor(p,usage,req){
  const trial=isTrial(p),stremio=isStremio(p),label=trial?'Maximum simultaneous trials':stremio?'Stremio household capacity':'Maximum concurrent plan slots';
  const help=trial?'This remains a hard trial concurrency cap. Trial activation also requires free Premium fleet stream capacity.':stremio?'Manual household-unit inventory. Each household included in a purchase consumes one unit: a 3-household subscription consumes 3. Set 0 to close new Stremio acquisition without removing existing access.':'Set 0 to close new acquisition while keeping the plan configured.';
...
async function page(req){
  ...
  const usage=await capacity.usage(p.id),fleet=usage.model==='fleet_streams',stremioHouseholds=usage.model==='manual_households';
  const metrics=fleet
    ...
    : stremioHouseholds
      ? `<div class="metrics"><div class="metric"><div class="metricLabel">Household capacity</div><div class="metricValue">${usage.householdLimit==null?'—':esc(usage.householdLimit)}</div></div><div class="metric"><div class="metricLabel">Sold / held households</div><div class="metricValue">${esc(Number(usage.householdUsed||0)+Number(usage.householdReserved||0))}</div></div><div class="metric"><div class="metricLabel">Available base-plan places</div><div class="metricValue ${usage.soldOut?'statusBad':'statusGood'}">${usage.remaining==null?'Not limited':esc(usage.remaining)}</div><div class="subText">${esc(usage.label||'Available')} · ${esc(usage.requiredHouseholds)} household${Number(usage.requiredHouseholds)===1?'':'s'} per base-plan purchase</div></div></div>`
  ...
  const description=derived?'Jellyfin availability is shared across the eligible server fleet and is no longer configured independently on each paid/free plan.':stremioHouseholds?'Stremio availability stays manual, but the inventory is measured in household units so multi-household purchases consume the correct amount.':'Control the manual acquisition cap without changing existing customer entitlements.';
  ...
  const safety=ui.confirmationPanel({tone:'info',title:'Existing customer access is preserved',body:'Availability controls new acquisition only. Existing subscriptions keep the access they already bought.',items:[derived?'Changing server stream capacity changes the shared storefront availability for every plan in this fleet.':stremioHouseholds?'A customer buying multiple Stremio households consumes that many units from this manual household capacity.':'Set 0 to stop new acquisition without disabling the plan.',isTrial(p)?'Trials are limited by both this manual cap and remaining Premium stream capacity.':stremioHouseholds?'Set 0 to stop new Stremio purchases while existing household entitlements continue normally.':'Raising the limit reopens acquisition when the product is otherwise ready.']});
  const body=`${ui.noticesFromRequest(req)}${metrics}<section class="section">${sectionHead}${controls}${safety}</section>`;
  return layout({siteName:runtimeSettings.siteName(),active:'plans',title:`${p.name} · Availability`,subtitle:derived?'Shared fleet stream capacity and live scarcity':stremioHouseholds?'Manual household capacity and live scarcity':'Manual acquisition capacity',body});
}
