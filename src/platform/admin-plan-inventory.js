...
function manualEditor(p,usage,req){
  const trial=isTrial(p),stremio=isStremio(p),label=trial?'Maximum simultaneous trial customers':stremio?'Customer availability units':'Maximum concurrent customers';
  const help=trial?'This remains a hard trial customer cap. Trial activation also requires capacity in the shared Premium fleet.':stremio?'For a normal 1-household plan, one unit equals one customer place. Larger household variants consume proportionally more units. Set 0 to close new Stremio acquisition without removing existing access.':'Set 0 to close new acquisition while keeping existing customer access unchanged.';
...
async function page(req){
  ...
  const [usage,customers]=await Promise.all([capacity.usage(p.id),liveCustomers(p.id)]),fleet=usage.model==='fleet_streams',stremioHouseholds=usage.model==='manual_households';
  const metrics=fleet
    ? `<div class="metrics"><div class="metric"><div class="metricLabel">Customers on this plan</div><div class="metricValue">${esc(customers)}</div></div><div class="metric"><div class="metricLabel">Customer capacity</div><div class="metricValue">${usage.limit==null?'—':esc(usage.limit)}</div><div class="subText">Current shared-fleet equivalent for this plan</div></div><div class="metric"><div class="metricLabel">New places available</div><div class="metricValue ${usage.soldOut?'statusBad':'statusGood'}">${esc(usage.remaining)}</div><div class="subText">${esc(usage.label||'Available')}</div></div></div>`
    : stremioHouseholds
  ...
  const description=derived?'Jellyfin availability is shared across the eligible server fleet and presented here as customer places.':stremioHouseholds?'Stremio availability is presented as customer places; larger household variants consume more of the same inventory automatically.':'Control the customer acquisition cap without changing existing customer entitlements.';
  const sectionHead=ui.sectionHeader({title:'Storefront availability',description});
  const controls=derived
    ? `<div class="formPanel"><strong>${esc(usage.pool==='free'?'Free':'Premium')} fleet controls this plan.</strong><p class="muted">CAPTAiNFiN accounts for access already sold or held across the shared ${esc(usage.pool)} fleet, then shows how many additional customers this plan can accept. Each customer's stream allowance is still enforced automatically in the background.</p><div class="buttonRow"><a class="button" href="/admin/servers">Manage server capacity</a><a class="button secondary" href="/admin/plans">Back to plans</a></div></div>`
    : manualEditor(p,usage,req);
  const safety=ui.confirmationPanel({tone:'info',title:'Existing customer access is preserved',body:'Availability controls new acquisition only. Existing subscriptions keep the access they already bought.',items:[derived?'Changing server capacity changes the shared customer availability for every plan in this fleet.':stremioHouseholds?'A customer buying a larger Stremio household variant consumes proportionally more of the shared plan inventory.':'Set 0 to stop new acquisition without disabling the plan.',isTrial(p)?'Trials are limited by both this manual customer cap and remaining Premium fleet capacity.':stremioHouseholds?'Set 0 to stop new Stremio purchases while existing customer entitlements continue normally.':'Raising the limit reopens acquisition when the product is otherwise ready.']});
  const body=`${ui.noticesFromRequest(req)}${metrics}<section class="section">${sectionHead}${controls}${safety}</section>`;
  return layout({siteName:runtimeSettings.siteName(),active:'plans',title:`${p.name} · Availability`,subtitle:derived?'Shared fleet capacity and live customer availability':stremioHouseholds?'Customer availability and live scarcity':'Customer acquisition capacity',body});
}
function createAdminPlanInventoryRouter(){
  ...
  r.post('/admin/plans/:id/inventory',inventoryWriteLimit,async(req,res)=>{
    ...
    try{
      ...
      if(capacity.capacityModel(p)==='fleet_streams'&&!isTrial(p))throw new Error('Paid and Free Jellyfin availability is controlled by shared server capacity.');
