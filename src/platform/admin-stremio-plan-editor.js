...
function page(data,req,{input=null,error=''}={}){
  ...
  ${requestPlanPolicy.planCard(req,p,{variant:'stremio'})}
...
async function save(data,input,_scope,actorUserId){
  const impact=householdImpact(data.plan,input);
  return transaction(async client=>{
    const updatedSubscriptions=await updateTrackingSnapshots(client,data.plan,input,impact,'all_current');
    ...
    await client.query(`INSERT INTO audit_log(actor_user_id,action,entity_type,entity_id,metadata) VALUES($1,'admin.plan.stremio_editor.update','plan',$2,$3::jsonb)`,[actorUserId,data.plan.id,JSON.stringify({householdImpact:impact,impactScope:'all_current',updatedSubscriptions,unlimitedStreams:true,unlimitedDevices:true})]);
...
function createAdminStremioPlanEditorRouter(){
  ...
  router.post('/admin/plans/:id/stremio-editor',writeLimit,async(req,res,next)=>{
    ...
    try{
      ...
      const result=await save(data,input,'all_current',req.session.authUserId);
      const requestJob=await queuePlanRequestReconciliation(data.plan.id,req.session.authUserId);
      const accessSuffix=result.updatedSubscriptions?` ${result.updatedSubscriptions} active subscription${result.updatedSubscriptions===1?' was':'s were'} updated to the new household policy.`:'';
      const requestSuffix=requestJob?` ${Number(requestJob.total_items||0)} current member${Number(requestJob.total_items||0)===1?'':'s'} queued for request-policy sync.`:'';
      return res.redirect(`/admin/plans/${encodeURIComponent(data.plan.id)}/edit?message=${encodeURIComponent(`Stremio plan saved.${accessSuffix}${requestSuffix}`)}`);
