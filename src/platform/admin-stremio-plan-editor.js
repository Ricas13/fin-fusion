...
function sourceSummary(sources){
  ...
  if(!selectedSources.length)return `<div class="stremioSourceSummary"><strong>Default eligible source pool</strong><div class="muted">No plan-specific source override is set. Manage Stremio sources globally, then return here to review the plan.</div></div>`;
  return `<div class="stremioSourceSummary"><strong>${selectedSources.length} plan-specific source${selectedSources.length===1?'':'s'}</strong><div class="chipRow">${selectedSources.map(source=>`<span class="pill ${source.enabled&&source.auth_state==='connected'?'good':'warn'}">${esc(source.name)}</span>`).join('')}</div></div>`;
}
...
function page(data,req,{input=null,error='',impact=null}={}){
  ...
  <form method="post" action="/admin/plans/${esc(p.id)}/stremio-editor" class="stremioPlanForm"><input type="hidden" name="_csrf" value="${esc(csrf.token(req))}"><input type="hidden" name="__submitted" value="1">
    <div class="stremioPlanGrid">
      ...
      <section class="section stremioCard"><div class="sectionHead"><div><h3>Stremio sources</h3><div class="muted">Sources are selected by product; internal fulfilment terminology stays hidden.</div></div></div>${sourceSummary(data.sources)}<div class="buttonRow"><a class="button secondary" href="/admin/servers/stremio">Manage sources</a></div></section>
