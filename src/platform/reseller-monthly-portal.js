...
function subscriptionActive(row){ return Boolean(row&&row.status==='active'&&new Date(row.current_period_end).getTime()>Date.now()); }
...
function range(req){
    ...
    const preset=PERIODS.get(key); const end=new Date(); const start=new Date(end.getTime()-preset.days*86400000);
    return {key,label:preset.label,start,end,days:preset.days};
}
...
async function analytics(resellerId,rng){
    const [sales,daily,newCustomers,playback,live,top]=await Promise.all([
        ...
        query(`SELECT date_trunc('day',created_at)::date AS sale_day,currency,SUM(amount_minor)::bigint amount_minor FROM reseller_sales WHERE reseller_id=$1 AND created_at>=$2 AND created_at<$3 GROUP BY 1,2 ORDER BY 1,2`,[resellerId,rng.start,rng.end]),
    ...
    const series=[];
    for(let i=Math.min(rng.days,45)-1;i>=0;i--){ const d=new Date(rng.end.getTime()-i*86400000); series.push({key:d.toISOString().slice(0,10),value:0}); }
    const byDay=new Map(series.map(x=>[x.key,x]));
    for(const row of daily.rows){
        if(String(row.currency).trim()!==String(primary.currency).trim()) continue;
        const point=byDay.get(new Date(row.sale_day).toISOString().slice(0,10));
        if(point) point.value+=Number(row.amount_minor||0);
    }
    return {totals,primary,newCustomers:Number(newCustomers.rows[0]?.n||0),playback:playback.rows[0]||{},liveStreams:Number(live.rows[0]?.streams||0),top:top.rows,series};
}

function chart(series,currency){
    const max=Math.max(1,...series.map(x=>Number(x.value||0)));
    return `<div class="resellerChart">${series.map(x=>`<div class="barWrap" title="${esc(x.key)} · ${esc(money(x.value,currency))}"><div class="bar" style="height:${Math.max(3,Math.round(Number(x.value||0)/max*100))}%"></div></div>`).join('')}</div>`;
}
...
body{background:#0c1117}.wrap{max-width:1500px;margin:auto;padding:0 22px 50px}.top{height:58px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #222933;margin-bottom:22px}.brand{display:flex;gap:10px;align-items:center;font-weight:800}.brand img{width:30px;height:30px;border-radius:7px;object-fit:cover}.heading{display:flex;justify-content:space-between;gap:18px;align-items:end;margin:8px 0 18px}.heading h1{font-size:24px;margin:0 0 4px}.ranges{display:flex;gap:6px;flex-wrap:wrap}.ranges a{padding:7px 10px;border:1px solid #29323d;border-radius:7px;color:#93a0b0;text-decoration:none}.ranges a.active{color:#fff;border-color:#20a9d6;background:rgba(32,169,214,.12)}.resellerChart{height:180px;display:flex;gap:3px;align-items:end;border-bottom:1px solid #29323d;padding:16px 4px 0}.barWrap{height:100%;flex:1;display:flex;align-items:end}.bar{width:100%;background:linear-gradient(180deg,#34bfe9,#197a9c);border-radius:4px 4px 0 0;min-height:3px}.meter{height:8px;border-radius:20px;background:#222a34;overflow:hidden;margin-top:8px}.meter span{display:block;height:100%;background:#20a9d6}.salesGrid{display:grid;grid-template-columns:minmax(0,2fr) minmax(280px,1fr);gap:16px}.tierCards{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px}.tierCard{border:1px solid #28313c;border-radius:12px;padding:17px;background:#111820}.tierPrice{font-size:27px;font-weight:850;color:#fff;margin:9px 0}.providerButtons,.customerActions{display:flex;gap:7px;flex-wrap:wrap}.customerActions form{margin:0}.ownerTag{color:#39bce7;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.08em}.credential{padding:16px;border:1px solid #2c3947;background:#0d141c;border-radius:9px;font-family:ui-monospace,monospace;font-size:17px;word-break:break-all}@media(max-width:850px){.salesGrid{grid-template-columns:1fr}.heading{display:block}.ranges{margin-top:12px}.wrap{padding:0 12px 35px}}
</style></head><body><div class="wrap"><header class="top"><div class="brand"><img src="${esc(branding.assetUrl('logo'))}" alt=""><span>${esc(site())} · Reseller</span></div><div class="buttonRow"><a class="button secondary btn-sm" href="/reseller/credit-history">Legacy credits</a><a class="button secondary btn-sm" href="/logout">Sign out</a></div></header><div class="heading"><div><h1>${esc(title)}</h1><div class="muted">${esc(subtitle)}</div></div></div>${body}</div></body></html>`;
}
...
function tierChoices(req,tiers,current){
    ...
    const hasLive=subscriptionActive(current);
    return `<div class="tierCards">${tiers.map(t=>{
        ...
        const selected=hasLive&&String(current.tier_id)===String(t.id);
        const buttons=!hasLive?`<div class="providerButtons">${providers.some(p=>p.provider==='stripe')?`<form method="post" action="/reseller/billing/stripe">${token(req)}<input type="hidden" name="tierId" value="${esc(t.id)}"><button class="button">Subscribe with Stripe</button></form>`:''}${providers.some(p=>p.provider==='paypal')?`<form method="post" action="/reseller/billing/paypal">${token(req)}<input type="hidden" name="tierId" value="${esc(t.id)}"><button class="button secondary">Subscribe with PayPal</button></form>`:''}${!providers.length?'<span class="muted">Contact admin to subscribe</span>':''}</div>`:'<div class="muted">Tier changes are controlled by the administrator while a paid subscription is active.</div>';
        return `<article class="tierCard"><div class="serverTop"><strong>${esc(t.name)}</strong>${selected?'<span class="pill good">Current</span>':''}</div><div class="tierPrice">${esc(money(t.monthly_price_minor,t.currency))}<span class="muted" style="font-size:12px"> / month</span></div><p class="muted">${esc(t.description||`${t.seat_limit} active Jellyfin accounts.`)}</p><p><strong>${esc(t.seat_limit)}</strong> active accounts <span class="muted">including your own account</span></p>${buttons}</article>`;
    }).join('')}</div>`;
}

function planOptions(plans){ return plans.map(p=>`<option value="${esc(p.code)}">${esc(p.name)} · ${esc(p.duration_days)} days</option>`).join(''); }
...
async function dashboard(req){
    ...
    const entitled=subscriptionActive(subscription);
    const limit=entitled?Number(subscription.seat_limit||0):0;
    const pct=limit?Math.min(100,Math.round(seats/limit*100)):0;
    const owner=customers.find(c=>c.is_reseller_owner);
    const primary=stats.primary;
    const ranges=[...PERIODS].map(([key,p])=>`<a class="${rng.key===key?'active':''}" href="/reseller?range=${key}">${p.label}</a>`).join('');
    const bill=subscription?money(subscription.monthly_price_minor,subscription.currency):'—';
    const body=`${notice(req)}<div class="heading"><div><div class="muted">Selected reporting period</div><strong>${esc(rng.label)}</strong></div><div class="ranges">${ranges}</div></div>
    <div class="metrics"><div class="metric"><div class="metricLabel">Downstream revenue</div><div class="metricValue">${esc(money(primary.amount_minor,primary.currency))}</div><div class="subText">${esc(primary.sales||0)} recorded sales</div></div><div class="metric"><div class="metricLabel">Active accounts</div><div class="metricValue">${seats}${limit?` / ${limit}`:''}</div><div class="meter"><span style="width:${pct}%"></span></div></div><div class="metric"><div class="metricLabel">New customers</div><div class="metricValue">${esc(stats.newCustomers)}</div><div class="subText">${esc(rng.label)}</div></div><div class="metric"><div class="metricLabel">Watch time</div><div class="metricValue">${(Number(stats.playback.seconds||0)/3600).toFixed(1)}h</div><div class="subText">${esc(stats.playback.sessions||0)} sessions</div></div><div class="metric"><div class="metricLabel">Streams now</div><div class="metricValue">${esc(stats.liveStreams)}</div></div><div class="metric"><div class="metricLabel">Your CAPTaINFiN bill</div><div class="metricValue">${esc(bill)}</div><div class="subText">${entitled?'Active':esc(subscription?.status||'Not subscribed')}${subscription?` · paid to ${esc(date(subscription.current_period_end))}`:''}</div></div></div>
    ${!entitled?'<div class="notice error"><strong>Your reseller subscription is not active.</strong> Customer activation and renewal are locked. A previously-paid estate is suspended until monthly payment is restored.</div>':''}
    <div class="salesGrid"><section class="section"><div class="sectionHead"><h2>Revenue</h2><span class="muted">Your manually recorded downstream sales</span></div>${chart(stats.series,primary.currency)}</section><section class="section"><div class="sectionHead"><h2>Top customers</h2></div>${stats.top.length?stats.top.map(c=>`<div class="serverTop" style="padding:8px 0;border-bottom:1px solid #222933"><span>${esc(c.display_name||'Customer')}</span><strong>${esc(money(c.revenue_minor,c.currency||primary.currency))}</strong></div>`).join(''):'<div class="empty">No sales in this period.</div>'}</section></div>
    <section class="section"><div class="sectionHead"><h2>Monthly reseller subscription</h2><span class="muted">Parent entitlement for your whole estate</span></div>${subscription&&!entitled?`<div class="notice">Previous subscription: ${esc(subscription.tier_name)} · ${esc(subscription.status)} · paid through ${esc(date(subscription.current_period_end))}. Choose a monthly tier below to reactivate.</div>`:''}${tierChoices(req,tiers,subscription)}${entitled&&subscription.source!=='manual'&&!subscription.cancel_at_period_end?`<form class="formPanel" method="post" action="/reseller/billing/cancel">${token(req)}<button class="button secondary">Cancel renewal at period end</button><div class="inlineHelp">Access remains active until the paid-through date, then the whole estate is suspended.</div></form>`:''}</section>
    <section class="section"><div class="sectionHead"><h2>Your own Jellyfin account</h2><span class="muted">Counts as one active seat</span></div>${owner?`<div class="serverCard"><strong>${esc(owner.jellyfin_username||owner.display_name||reseller.username)}</strong><div class="subText">${esc(owner.plan_name||'No active plan')} · ${esc(owner.server_name||'Provisioning pending')}</div></div>`:entitled&&plans.length?`<form class="formPanel" method="post" action="/reseller/owner/create">${token(req)}<div class="formGroup"><label>Customer access plan</label><select class="input" name="planCode">${planOptions(plans)}</select></div><button class="button">Create my Jellyfin account</button></form>`:'<div class="empty">Activate a monthly reseller subscription first.</div>'}</section>
    <section class="section"><div class="sectionHead"><h2>Add customer</h2><span class="muted">You make the sale; CAPTaINFiN handles Jellyfin</span></div>${entitled&&plans.length&&seats<limit?`<form class="formPanel" method="post" action="/reseller/customer/create">${token(req)}<div class="formGrid"><div class="formGroup"><label>Jellyfin username</label><input class="input" name="username" required pattern="[A-Za-z0-9._-]{3,40}"></div><div class="formGroup"><label>Customer plan</label><select class="input" name="planCode">${planOptions(plans)}</select></div><div class="formGroup"><label>Amount you charged</label><input class="input" type="number" name="amount" step="0.01" min="0" value="0.00"></div><div class="formGroup"><label>Currency</label><input class="input" name="currency" maxlength="3" value="${esc(subscription?.currency||'GBP')}"></div><div class="formGroup"><label>Payment method</label><select class="input" name="paymentMethod"><option>Cash</option><option>Bank transfer</option><option>PayPal</option><option>Stripe</option><option>Other</option></select></div><div class="formGroup"><label>Note</label><input class="input" name="note" maxlength="500"></div></div><button class="button">Create customer & provision Jellyfin</button></form>`:!entitled?'<div class="empty">Your monthly reseller subscription must be active first.</div>':`<div class="notice error">Your ${esc(subscription?.tier_name||'reseller')} tier is full (${seats}/${limit}). Upgrade before adding another active account.</div>`}</section>
    <section class="section"><div class="sectionHead"><h2>Customers</h2><span class="muted">${customers.length} managed records</span></div>${customers.length?`<div class="tableWrap"><table class="dataTable responsiveTable"><thead><tr><th>Customer</th><th>Plan</th><th>Server</th><th>Access until</th><th>Streams</th><th>Status</th><th></th></tr></thead><tbody>${customers.map(c=>`<tr><td data-label="Customer"><strong>${esc(c.display_name||c.jellyfin_username||'Customer')}</strong>${c.is_reseller_owner?'<div class="ownerTag">Your account</div>':`<div class="subText">${esc(c.jellyfin_username||'Provisioning pending')}</div>`}</td><td data-label="Plan">${esc(c.plan_name||'—')}</td><td data-label="Server">${esc(c.server_name||'—')}</td><td data-label="Access until">${esc(date(c.current_period_end))}</td><td data-label="Streams">${esc(c.active_streams||0)}</td><td data-label="Status"><span class="pill ${c.access_paused_at?'bad':c.sub_status?'good':''}">${c.access_paused_at?'Suspended':esc(c.sub_status||'No plan')}</span></td><td data-label=""><div class="customerActions">${!c.is_reseller_owner?`<a class="button secondary btn-sm" href="/reseller/customer/${encodeURIComponent(c.id)}/renew">Renew</a>`:''}<a class="button secondary btn-sm" href="/reseller/client/${encodeURIComponent(c.id)}/credentials">Credentials</a>${!c.is_reseller_owner?`<form method="post" action="/reseller/customer/${encodeURIComponent(c.id)}/toggle">${token(req)}<input type="hidden" name="suspended" value="${c.access_paused_at?'false':'true'}"><button class="button ${c.access_paused_at?'secondary':'btn-danger'} btn-sm">${c.access_paused_at?'Resume':'Suspend'}</button></form>`:''}</div></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">No customers yet.</div>'}</section>`;
    return shell({title:'Reseller dashboard',subtitle:'Revenue, customers, streaming and your recurring monthly subscription',body});
}
...
function createResellerMonthlyPortalRouter(){
    ...
    r.post('/reseller/billing/stripe',async(req,res)=>{if(!csrf.verify(req))return res.status(403).send('Invalid security token');try{const reseller=await resolveReseller(req.session.authUserId);await canStartNewSubscription(reseller.id);const checkout=await resellerBilling.createStripeCheckout({resellerId:reseller.id,tierId:req.body.tierId,successUrl:absoluteUrl(req,'/reseller?message=Payment%20received.%20Your%20monthly%20subscription%20will%20activate%20when%20Stripe%20confirms%20it.'),cancelUrl:absoluteUrl(req,'/reseller?error=Checkout%20cancelled')});return res.redirect(303,checkout.url);}catch(e){return redirect(res,'error',e.message);}});
    r.post('/reseller/billing/paypal',async(req,res)=>{if(!csrf.verify(req))return res.status(403).send('Invalid security token');try{const reseller=await resolveReseller(req.session.authUserId);await canStartNewSubscription(reseller.id);const checkout=await resellerBilling.createPayPalCheckout({resellerId:reseller.id,tierId:req.body.tierId,returnUrl:absoluteUrl(req,'/reseller/billing/paypal/return'),cancelUrl:absoluteUrl(req,'/reseller?error=PayPal%20checkout%20cancelled')});req.session.pendingResellerPayPal=checkout.id;return res.redirect(303,checkout.url);}catch(e){return redirect(res,'error',e.message);}});
...
module.exports={createResellerMonthlyPortalRouter,resolveReseller,analytics,range,downstreamPlans,subscriptionActive};
