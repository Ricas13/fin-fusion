...
function link(label,url,internal){return url?`<a class="helpLink" href="${esc(url)}" ${internal?'':'rel="noopener noreferrer"'}><strong>${esc(label)}</strong><span>Open</span></a>`:''}
function render(site,policy){
    ...
    const links=[link('Your account guide','/account/docs',true),link('Help & guides',policy.docsUrl),link('Support',policy.supportUrl),link('Service status',policy.statusUrl),link('Terms of service',policy.termsUrl),link('Privacy policy',policy.privacyUrl),link('Refund policy',policy.refundPolicyUrl)].filter(Boolean).join('');
