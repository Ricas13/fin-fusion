  ...
  const result = await query(`
    WITH buckets AS (
      ...
      SELECT date_trunc('${grain.trunc}',started_at) AS bucket,COUNT(*)::int AS plays,
             COALESCE(SUM(EXTRACT(EPOCH FROM(GREATEST(started_at,LEAST(COALESCE(ended_at,last_seen_at,$2::timestamptz),$2::timestamptz))-started_at)),0)::bigint AS watch_seconds
