...
const providerPricing=require('../payments/provider-plan-pricing');
...
const CHECKOUT_SAFE=[
 'Invalid or expired security token','Plan is required',
 /^This plan is not configured for .+ in .+$/,/^That payment type is not available for this plan in .+$/,
 /^(Stripe|PayPal|Plisio) checkout is temporarily unavailable until its payment callback is configured\.$/,
 ...
 /^You already have active .+ access\. Wait until the current paid term ends, or manage that access before purchasing another overlapping term\.$/,
 'That is already your current plan, currency and access allowance.',
 /^The selected access option is not configured for (Stripe|PayPal) recurring billing in .+\.$/,
 'That discount code is not valid or has expired','That discount code does not apply to this plan','That discount code has reached its redemption limit',
 "That discount code's currency does not match this plan",
 'You have already used that discount code','You have already used or reserved that discount code',
 'Checkout intent was not found for this discount reservation','Discount reservation customer does not match the checkout intent',
 'Discounts can only be reserved for an open checkout intent','Checkout intent has already expired',
...
function priceLabel(plan){return moneyFormat.formatMinor(plan.price_minor,plan.currency||'GBP');}
...
async function existingRecurringReplacementOption(req,provider,planCode,currency,quantity,requestedMode){
  ...
  if(!targetResult.rowCount)return null;
  const target=targetResult.rows[0],current=await planChange.currentRecurring(req.session.customerId,target);
  if(!current||current.source!==provider||String(current.plan_id)!==String(target.id))return null;
  const mapping=await planChange.replacementMapping(current,target,provider,currency,quantity);if(!mapping)return null;
...
function isProviderRecurring(row){return(row.source==='stripe'&&/^sub_/i.test(String(row.provider_subscription_id||'')))||(row.source==='paypal'&&/^I-/i.test(String(row.provider_subscription_id||'')));}
function overlappingPaidRows(rows,target){return(rows||[]).filter(row=>serviceScope.overlaps(row,target)&&!serviceScope.isFreeTier(row));}
async function overlappingRecurring(customerId,target){const rows=await livePrimaryRows(customerId);return overlappingPaidRows(rows,target).filter(isProviderRecurring);}
async function begin(req,res,provider){
 ...
 const liveRows=await livePrimaryRows(req.session.customerId),paidOverlaps=overlappingPaidRows(liveRows,choice.plan);
 if(wantsCredit){const conflict=paidOverlaps[0];if(conflict)throw new Error(`Mixed service-credit checkout is for activating a new ${serviceScope.label(choice.plan)} service. End the current overlapping service before applying credit to a new provider checkout.`);}
 if(choice.mode==='subscription'&&!wantsCredit){const recurring=paidOverlaps.filter(isProviderRecurring);if(recurring.length){const changed=await planChange.requestChange({customerId:req.session.customerId,targetPlanCode:choice.planCode,targetCurrency:choice.currency,targetAccessQuantity:choice.accessQuantity,targetVariantKind:choice.accessVariantKind,timing:req.body.changeTiming||'auto',actorUserId:req.session.customerUserId});if(changed.handled)return{internal:true,message:changed.message};throw new Error(`You already have recurring ${serviceScope.label(choice.plan)} access. Manage or cancel that service before starting another recurring subscription.`);}if(paidOverlaps.length)throw new Error(`You already have active ${serviceScope.label(choice.plan)} access. Wait until the current paid term ends, or manage that access before purchasing another overlapping term.`);}
 if(choice.mode==='payment'&&paidOverlaps.length)throw new Error(`You already have active ${serviceScope.label(choice.plan)} access. Wait until the current paid term ends, or manage that access before purchasing another overlapping term.`);
 ...
 try{
  ...
  if(providerDue===0&&choice.mode==='payment'){
   await zeroValueCheckout.activateFullyDiscountedPayment({customerId:req.session.customerId,intentId:intent.id,nonce:intent.nonce,provider});
   return{internal:true,message:'Your promo covered the full price. Your access has been activated without sending you to a payment provider.'};
  }
  if(provider==='stripe'){
  ...
  if(provider==='plisio'){
   const checkout=await plisio.createCheckout({intentId:intent.id,resolvedPlan:choice.plan,finalAmountMinor:providerDue,callbackUrl:await operations.absoluteUrl(req,'/webhooks/plisio'),successUrl:await stateUrl(req,'/account/plisio/return',intent),cancelUrl:await stateUrl(req,'/account/checkout/cancel',intent)});
   await intents.attachProviderCheckout(intent.id,checkout.id);return checkout;
  }
  const checkout=await paypal.createCheckout({customerId:req.session.customerId,planCode:choice.planCode,checkoutMode:choice.mode,discountCode:req.body.discountCode||null,returnUrl:await stateUrl(req,'/account/paypal/return',intent),cancelUrl:await stateUrl(req,'/account/checkout/cancel',intent),idempotencyKey:intent.id,checkoutIntentId:intent.id,commercialSnapshot:intent.commercial_snapshot,resolvedPlan:choice.plan,finalAmountMinor:providerDue});
  await intents.attachProviderCheckout(intent.id,checkout.id);return checkout;
...
function createFlexibleCheckoutRouter(){const router=express.Router();router.use(async(_req,_res,next)=>{try{await providerSettings.ensureLoaded();return next();}catch(error){return next(error);}});router.get('/account/checkout/readiness',requireCustomer,async(_req,res)=>{try{return res.json(await checkoutReadiness());}catch(_){return res.status(503).json({stripe:false,paypal:false,plisio:false});}});router.post('/account/checkout/stripe',requireCustomer,checkoutStartLimit,async(req,res)=>{try{return await finish(req,res,'stripe');}catch(error){return checkoutErrorRedirect(res,error,'Stripe checkout start failed','Stripe checkout could not be started.');}});router.post('/account/checkout/paypal',requireCustomer,checkoutStartLimit,async(req,res)=>{try{return await finish(req,res,'paypal');}catch(error){return checkoutErrorRedirect(res,error,'PayPal checkout start failed','PayPal checkout could not be started.');}});router.post('/account/checkout/plisio',requireCustomer,checkoutStartLimit,async(req,res)=>{try{return await finish(req,res,'plisio');}catch(error){return checkoutErrorRedirect(res,error,'Plisio checkout start failed','Plisio checkout could not be started.');}});router.get('/account/checkout/cancel',requireCustomer,async(req,res)=>{try{await intents.consume({intentId:String(req.query.checkout_intent||''),nonce:String(req.query.checkout_state||''),state:'cancelled',scope:'customer',ownerId:req.session.customerId});return res.redirect('/account?error='+encodeURIComponent('Checkout cancelled.'));}catch(error){return checkoutErrorRedirect(res,error,'Checkout cancel failed','Checkout could not be cancelled.');}});router.post('/account/checkout/cancel-open',requireCustomer,async(req,res)=>{if(!csrf.verify(req))return res.redirect('/account?error='+encodeURIComponent('Invalid or expired security token'));try{const cancelled=await intents.cancelForOwner('customer',req.session.customerId);return res.redirect('/account?message='+encodeURIComponent(cancelled?'Your in-progress checkout was cancelled. You can start again below.':'No in-progress checkout was found.'));}catch(error){return checkoutErrorRedirect(res,error,'Checkout cancel-open failed','Your in-progress checkout could not be cancelled.');}});return router;}
module.exports={createFlexibleCheckoutRouter,providerLabel,priceLabel,requireCustomer,stateUrl,stripeSuccessUrl,commercialSnapshot,requestedCurrency,requestedAccessQuantity,choiceQuantity,checkoutReadiness,assertProviderCheckoutReady,existingRecurringReplacementOption,livePrimaryRows,overlappingRecurring,overlappingPaidRows,isProviderRecurring,CHECKOUT_SAFE};