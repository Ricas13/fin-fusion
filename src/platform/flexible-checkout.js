...
async function resumeExistingCheckout(customerId,provider,choice){
 const existing=await intents.getOutstandingForOwner('customer',customerId);
 ...
 if(sameTarget&&existing.provider_checkout_id){
  ...
  if(provider==='paypal'&&choice.mode==='subscription'){
   resumed=await paypal.resumeCheckout(existing.provider_checkout_id);
  }else if(provider==='stripe'){
   resumed=await stripe.resumeCheckout(existing.provider_checkout_id);
  }

  if(resumed)return resumed;

  const stillOutstanding=await intents.getOutstandingForOwner('customer',customerId);
  if(!stillOutstanding)return null;
 }

 throw new Error(`An existing ${providerLabel(existing.provider)} checkout is still awaiting completion. Continue that checkout before starting another payment method.`);
}
