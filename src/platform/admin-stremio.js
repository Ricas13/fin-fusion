...
function runtimeStatus(){const flag=String(process.env.STREMIO_RUNTIME_ENABLED||'').toLowerCase()==='true',ready=foundation.runtimeReady();return{flag,ready,keyConfigured:Boolean(process.env.STREMIO_JELLYFIN_TOKEN_KEY)};}

async function data(){
    const [plans,servers,entitlements,indexStates]=await Promise.all([
        query(`SELECT id,code,name,service_type,streams,active,visible,price_minor,currency,archived_at
    ...
    return {plans:plans.rows,servers:servers.rows,entitlements:entitlements.rows[0]||{total:0,active:0,pending:0,suspended:0,revoked:0},indexStates,runtime:runtimeStatus()};
}
...
module.exports={createAdminStremioRouter,page,data,runtimeStatus};
