...
function target(message='',error=''){const q=new URLSearchParams();if(message)q.set('message',message);if(error)q.set('error',error);return `/admin/servers/stremio/managed${q.toString()?`?${q}`:''}`;}
...
function createAdminStremioRouter(){
  ...
  router.get('/admin/stremio-sources',gate,noStore,(_req,res)=>res.redirect(302,'/admin/servers/stremio/managed'));
  router.use('/admin/settings/stremio',gate,noStore);
  router.get('/admin/settings/stremio',(_req,res)=>res.redirect(302,'/admin/servers/stremio/managed'));
  ...
  router.post('/admin/settings/stremio/servers/:id',mutationLimit,(req,res)=>res.redirect(303,target('Managed Jellyfin Stremio eligibility is now controlled from Manage Stremio.')));
  return router;
}
