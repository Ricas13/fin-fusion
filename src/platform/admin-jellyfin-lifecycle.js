...
<section class="section"><div class="sectionHead"><div><h2>Free Server lifecycle automation</h2><div class="muted">This page controls the Free inactivity policy mode. The recurring Customer inactivity job is scheduled separately under Automation. Thresholds belong to each Free-class media server under Servers → Advanced settings.</div></div><span class="statusPill ${cfg.enabled?(cfg.dryRun?'statusWarn':'statusGood'):'statusInfo'}">${cfg.enabled?(cfg.dryRun?'Dry run':'Enforcing'):'Paused'}</span></div>
