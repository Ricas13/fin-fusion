...
function deliveryPill(status) {
    ...
    if (status === 'dead') return pill('Dead', 'bad');
...
function emailHealthCard(req, settings, recent, counts) {
    ...
    } else if (settings.configured && dead > 0) {
        statusLabel = 'Needs attention'; statusKind = 'warn'; workingLabel = `${dead} message${dead === 1 ? '' : 's'} gave up retrying`; workingKind = 'warn';
        fixHint = 'These addresses failed repeatedly and stopped auto-retrying. Fix the address or SMTP issue, then retry the affected messages below.';
