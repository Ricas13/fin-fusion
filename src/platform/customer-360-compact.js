...
function portalCard(detail,token){
  ...
  const body=`${stateRow('Username',esc(c.login_username||'Not enrolled'))}${stateRow('Email',esc(c.login_email||c.email||'—'))}${stateRow('Account',enrolled?pill(active?'Active':'Disabled',active?'good':'warn'):pill('Not enrolled','warn'))}${stateRow('Verification',pill(verified?'Verified':'Not verified',verified?'good':'warn'):pill('Not verified','warn'))}${enrolled?'':portalEnrolForm(c,token)}`;
...
function plansCard(detail,token,plans){
  ...
  if(current.length){
    ...
    actions.push(bulkForm(token,id,'plan_change','Change plan'));
    actions.push(actionLink(id,'extend','Extend',{params:{subscriptionId:primary.id}}));
    actions.push(actionLink(id,'expiry','Edit expiry',{params:{subscriptionId:primary.id}}));
...
async function render(detail,token,options={}){
  ...
  const core=`<div class="customer360Core" data-customer360-core><div class="opGrid">${cards.join('')}</div>${planSettings}${activityDisclosure(detail)}${paymentsDisclosure(extra.payments,extra.paymentIncidents)}${logsDisclosure(detail)}</div>`;
