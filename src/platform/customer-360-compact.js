...
function paymentsDisclosure(rows){const body=rows.length?`<div class="compactTable"><div class="compactTableHead"><span>Date</span><span>Provider</span><span>Type</span><span>Amount</span><span>Status</span></div>${rows.map(row=>{const minor=Number(row.gross_amount_minor||0),amount=`${esc(String(row.currency||'').toUpperCase())} ${(minor/100).toFixed(2)}`;return `<div class="compactTableRow"><span>${esc(dt(row.occurred_at))}</span><span>${esc(row.provider||'—')}</span><span>${esc(String(row.transaction_type||'payment').replaceAll('_',' '))}</span><span>${amount}</span><span>${esc(row.transaction_status||'—')}</span></div>`;}).join('')}</div>`:'<div class="opEmpty">No payment history recorded.</div>';return disclosure('Payments',`${rows.length} payment / refund events`,body);}
