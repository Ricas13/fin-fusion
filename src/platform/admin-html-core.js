...
const critical=`*{box-sizing:border-box}:root{--bg:#090d12;--sidebar:#0d1218;--panel:#121820;--border:#222b36;--text:#dfe6ed;--muted:#8390a0;--accent:#20a9d6;--sidebar-w:248px}html,body{margin:0;min-height:100%;background:var(--bg);color:var(--text)}body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:14px}.appShell{min-height:100vh;background:var(--bg)}.adminHeader{position:fixed;inset:0 auto 0 0;width:var(--sidebar-w);z-index:110;background:var(--sidebar);border-right:1px solid var(--border);display:flex;flex-direction:column}.headerMain{padding:18px 17px 15px}.brandBlock{display:flex;align-items:center;gap:11px;text-decoration:none}.brandLogo{width:34px;height:34px;border-radius:9px;object-fit:cover}.brandText{font-size:15px;font-weight:800;color:#f4f7fa}.brandSub{font-size:10px;color:#647184}.adminTabsWrap{flex:1;overflow:auto;padding:8px 10px 138px}.adminTabs{display:grid;gap:18px}.navSection{display:grid;gap:5px}.navSectionLabel{display:flex;align-items:center;gap:8px;padding:0 8px;color:#677487;font-size:10px;font-weight:800;letter-spacing:.09em;text-transform:uppercase}.navIcon{width:15px;height:15px;display:grid;place-items:center}.navIcon svg{width:15px;height:15px}.navSectionPages{display:grid;gap:2px}.adminTab{display:flex;align-items:center;min-height:34px;padding:7px 10px 7px 31px;border-radius:8px;color:#9aa6b5;text-decoration:none;font-weight:600;font-size:12px}.adminTab.active{color:#fff;background:rgba(32,169,214,.12);box-shadow:inset 3px 0 0 var(--accent)}.headerActions{position:absolute;left:10px;right:10px;bottom:10px;display:grid;gap:4px;padding-top:9px;border-top:1px solid var(--border)}.headerButton{display:flex;padding:7px 10px;color:#8e9aaa;text-decoration:none;font-size:11px}.mainPane{min-height:100vh;margin-left:var(--sidebar-w);background:var(--bg)}.topBar{position:sticky;top:0;z-index:95;height:58px;padding:0 30px;display:flex;align-items:center;justify-content:space-between;background:rgba(13,18,24,.96);border-bottom:1px solid var(--border)}.content{padding:30px 32px 56px}.pageHeader{margin-bottom:22px}.pageHeader h1{margin:0 0 6px;font-size:27px;line-height:1.15;color:#f4f7fa}.muted{color:var(--muted)}@media(max-width:860px){:root{--sidebar-w:0px}.adminHeader{position:static;width:100%;border-right:0;border-bottom:1px solid var(--border)}.adminTabsWrap{padding:6px 10px 10px;overflow-x:auto}.adminTabs{display:flex;min-width:max-content;gap:12px}.navSection{display:flex;align-items:center}.navSectionLabel{display:none}.navSectionPages{display:flex}.adminTab{padding:7px 10px}.headerActions{display:none}.mainPane{margin-left:0}.topBar{height:50px;padding:0 15px}.content{padding:20px 15px 40px}}`;
...
function header(active,site){
  ...
  const sections=nav.groups.map(group=>{
    ...
    const pages=group.pages.map(([key,label,url])=>`<a class="adminTab ${current.key===key?'active':''}" href="${esc(url)}">${esc(label)}</a>`).join('');
    return `<section class="navSection ${activeGroup?'active':''}"><div class="navSectionLabel">${icon(group.key)}<span>${esc(group.label)}</span></div><div class="navSectionPages">${pages}</div></section>`;
  }).join('');
  return `<header class="adminHeader"><div class="headerMain"><a class="brandBlock" href="/admin"><img class="brandLogo" src="${esc(branding.assetUrl('logo'))}" alt=""><div><div class="brandText">${esc(site)}</div><div class="brandSub">Control centre</div></div></a></div><div class="adminTabsWrap"><nav class="adminTabs" aria-label="Administration">${sections}</nav></div><div class="headerActions"><a class="headerButton hideMobile" href="/" target="_blank" rel="noopener noreferrer">Open storefront</a><a class="headerButton" href="/admin/security">Security</a><a class="headerButton danger" href="/logout">Sign out</a></div></header>`;
}

function layout(options={}){
  const site=options.siteName||'CAPTaINFiN';
  const current=activePage(options.active);
  const docsUrl=String(process.env.PUBLIC_DOCS_URL||'').trim();
  const docsAction=docsUrl?`<a class="topHelpLink" href="${esc(docsUrl)}" target="_blank" rel="noopener noreferrer">Help & guides</a>`:'';
