...
const SECTIONS=new Set(['general','integrations','security','advanced']);
...
async function load() {
    ...
    const [rows, plans, stripe, paypal, requests, email, notifications, resellerDefaults, automation, abuse, pending, support] = await Promise.all([
    ...
    return {
        ...
        integrations: { stripe, paypal, requests, email, notifications },
        abuse,
        pending,
        support,
        resellerDefaults: {
...
function createAdminOriginalSettingsRouter() {
    ...
    router.get('/admin/settings',async(req,res,next)=>{try{if(String(req.query.section||'')==='commerce')return res.redirect('/admin/commerce');res.setHeader('Cache-Control','no-store, private, max-age=0');return res.send(page(req,await load()));}catch(error){return next(error);}});
...
module.exports={createAdminOriginalSettingsRouter,load,page,registrationLabel,sectionPage,sectionTitle};
