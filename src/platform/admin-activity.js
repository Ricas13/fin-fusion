...
async function dashboardData(cfg){
  const[summaryResult,streamsResult,serversResult,significantEventsResult,eventsResult,historyResult]=await Promise.all([
    query(`SELECT
      ...
      (SELECT COUNT(*)::int FROM stream_policy_events WHERE created_at>=NOW()-INTERVAL '24 hours' AND decision='skipped_safety' AND reason IN('incomplete_server_snapshot','revalidation_failed')) AS safety_issues_24h,
