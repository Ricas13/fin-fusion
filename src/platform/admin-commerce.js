...
async function commerceData(){
  const [direct,reseller,states,events,activations,accessPolicy,freePlans,riskPolicy,paymentIncidents,admins]=await Promise.all([
    ...
    query(`SELECT COALESCE(rs.currency_snapshot,rt.currency) currency,
      SUM(COALESCE(rs.monthly_price_minor_snapshot,rt.monthly_price_minor))::bigint amount_minor,
      COUNT(*)::int subscriptions
      FROM reseller_subscriptions rs JOIN reseller_tiers rt ON rt.id=rs.tier_id
      WHERE rs.status='active' AND rs.current_period_end>NOW() AND rs.source IN('stripe','paypal')
      GROUP BY 1 ORDER BY 1`),
    query(`SELECT 'direct' scope,status,COUNT(*)::int n FROM subscriptions WHERE source IN('stripe','paypal') AND provider_subscription_id IS NOT NULL GROUP BY status
      UNION ALL
      SELECT 'reseller',status,COUNT(*)::int FROM reseller_subscriptions WHERE source IN('stripe','paypal') GROUP BY status
    ...
    query(`SELECT COUNT(*) FILTER(WHERE created_at>=NOW()-INTERVAL '30 days' AND(event_type ILIKE '%refund%' OR event_type ILIKE '%refunded%'))::int refunds,
      ...
      FROM payment_events`),
    query(`SELECT COUNT(*) FILTER(WHERE source IN('stripe','paypal') AND created_at>=NOW()-INTERVAL '30 days')::int direct_activations,
      (SELECT COUNT(*)::int FROM reseller_subscriptions WHERE source IN('stripe','paypal') AND created_at>=NOW()-INTERVAL '30 days') reseller_activations,
      COUNT(*) FILTER(WHERE source IN('stripe','paypal') AND status IN('cancelled','expired') AND updated_at>=NOW()-INTERVAL '30 days')::int direct_churn,
      (SELECT COUNT(*)::int FROM reseller_subscriptions WHERE source IN('stripe','paypal') AND status IN('cancelled','expired') AND updated_at>=NOW()-INTERVAL '30 days') reseller_churn
      FROM subscriptions`),
