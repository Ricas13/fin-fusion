...
async function commerceData(){
  const [direct,states,events,activations,affiliateBalances,affiliateActivity,affiliateCount,accessPolicy,freePlans,riskPolicy,paymentIncidents,admins]=await Promise.all([
  ...
  return{direct:direct.rows,states:states.rows,events:events.rows[0]||{},activations:activations.rows[0]||{},affiliateBalances:affiliateBalances.rows,affiliateActivity:affiliateActivity.rows,affiliateCount:Number(affiliateCount.rows[0]?.n||0),accessPolicy,freePlans:freePlans.rows,riskPolicy,paymentIncidents:currentIncidents,admins:admins.rows};
}
...
function incidentTable(req,rows,admins){
  if(!rows.length)return'<div class="empty">No normalized customer payment incidents recorded yet.</div>';
