...
async function commerceData(){
  const [states,events,activations,upcomingExpiries,affiliateBalances,affiliateActivity,affiliateCount,paymentIncidents,admins]=await Promise.all([
  ...
  return{states:states.rows,events:events.rows[0]||{},activations:activations.rows[0]||{},upcomingExpiries:upcomingExpiries.rows,affiliateBalances:affiliateBalances.rows,affiliateActivity:affiliateActivity.rows,affiliateCount:Number(affiliateCount.rows[0]?.n||0),paymentIncidents:currentIncidents,admins:admins.rows};
}
