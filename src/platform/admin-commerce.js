...
async function commerceData(){
  const [direct,states,events,activations,upcomingExpiries,affiliateBalances,affiliateActivity,affiliateCount,paymentIncidents,admins]=await Promise.all([
  ...
  return{direct:direct.rows,states:states.rows,events:events.rows[0]||{},activations:activations.rows[0]||{},upcomingExpiries:upcomingExpiries.rows,affiliateBalances:affiliateBalances.rows,affiliateActivity:affiliateActivity.rows,affiliateCount:Number(affiliateCount.rows[0]?.n||0),paymentIncidents:currentIncidents,admins:admins.rows};
}
...
function commerceGraphics(d,{directCount,availableCredit,pendingCredit,earned30,redeemed30}){
  ...
  const expiryCount=(d.upcomingExpiries||[]).length;
  return `${graphics.hero({title:'Commerce health',subtitle:'Recurring revenue, payment incident pressure, affiliate service credit and customer lifecycle movement.',tone:openIncidents?'warn':'good',stats:[
    ...
    graphics.stat({label:'New subscribers',value:graphics.number(a.activations||0),meta:'activated in the last 30 days',tone:'good'}),
    graphics.stat({label:'Expiring soon',value:graphics.number(expiryCount),meta:'next 30 days without safe auto-renewal',tone:expiryCount?'warn':'good'}),
