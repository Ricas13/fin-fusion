...
function row(req,server){
  ...
  const readiness=!playable?pill('Public URL required','bad'):healthy?pill('Ready','good'):pill(server.health_status||'unknown','warn');
  return `<tr><td><strong>${esc(server.name)}</strong><div class="subText">${esc(server.server_class)}${server.public_url?` · ${esc(server.public_url)}`:''}</div></td><td>${status}<div class="subText">${Number(server.managed_stremio_accounts||0)} active hidden account${Number(server.managed_stremio_accounts||0)===1?'':'s'}</div></td><td>${readiness}</td><td><form class="managedSourceForm" method="post" action="/admin/servers/stremio/managed/${esc(server.id)}">${token(req)}<label class="checkRow"><input type="checkbox" name="enabled" value="1" ${server.stremio_enabled?'checked':''}><span>Use for Stremio</span></label><label>Source priority <input class="input compact" type="number" min="1" max="10000" name="priority" value="${Number(server.stremio_priority||100)}"></label><div class="buttonRow"><button class="button secondary btn-sm">Save</button><a class="button secondary btn-sm" href="/admin/servers/${esc(server.id)}/edit">API / server settings</a></div></form></td></tr>`;
}

async function page(req){
  await runtimeSettings.ensureLoaded();
  const servers=await managed.list(),enabled=servers.filter(s=>s.stremio_enabled).length;
  const body=`${notice(req)}<div class="statusBanner"><strong>Managed Stremio sources stay private.</strong> Customers never see these server names or whether a result is managed. CAPTAiNFiN uses each server's write-only Jellyfin API credential only on the backend; customer playback will use restricted hidden Jellyfin identities instead of exposing the administrator API key.</div><section class="section"><div class="sectionHead"><div><h2>Your Jellyfin servers</h2><div class="muted">Enable the servers that should supply the first Stremio results. Lower priority numbers are returned first. ${enabled} currently enabled.</div></div><a class="button secondary" href="/admin/servers/stremio">External sources</a></div>${servers.length?`<div class="tableWrap"><table class="dataTable"><thead><tr><th>Server</th><th>Stremio</th><th>Readiness</th><th>Configuration</th></tr></thead><tbody>${servers.map(s=>row(req,s)).join('')}</tbody></table></div>`:'<div class="empty">No Jellyfin servers are configured.</div>'}</section><div class="securityNote standalone"><strong>Credential boundary:</strong> the server API key is never embedded in a Stremio result or media URL. It is used only by CAPTAiNFiN to provision and manage restricted hidden playback identities.</div><style>.managedSourceForm{display:grid;gap:8px;min-width:280px}.managedSourceForm label{display:grid;gap:4px}.managedSourceForm .compact{max-width:130px}.managedSourceForm .checkRow{display:flex}</style>`;
...
function createAdminStremioManagedSourcesRouter(){
  ...
  router.post('/admin/servers/stremio/managed/:serverId',mutationLimit,async(req,res)=>{
    ...
    try{
      ...
      return res.redirect('/admin/servers/stremio/managed?message='+encodeURIComponent(enabled?'Managed Stremio source enabled.':'Managed Stremio source disabled.'));
    }catch(error){return res.redirect('/admin/servers/stremio/managed?error='+encodeURIComponent(error.message));}
  });
  return router;
}

module.exports={createAdminStremioManagedSourcesRouter,page};
