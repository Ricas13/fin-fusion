...
function activeStreams(data){const rows=data.playback.rows,weighted=rows.length?rows.reduce((s,r)=>s+Number(r.avg_concurrent||0),0)/rows.length:0,totalStarts=sum(rows,'session_starts');return`${metric(fmt1(weighted),`average concurrent streams · ${number(totalStarts)} starts`)}${areaLine(rows,'avg_concurrent','cyan',fmt1,'Average concurrent streams')}${foot(`Playback overlap by ${data.playback.grain}; long streams contribute proportionally to actual load.`)}`;}
...
function players(data){const rows=data.players.rows,totalHours=rows.reduce((s,r)=>s+Number(r.seconds||0),0)/3600;return`${metric(`${fmt1(totalHours)}h`,'watch time across top players')}${playerBars(rows)}${foot('Clients are normalized into stable player families so browser/device naming does not fragment the chart.')}`;}
