...
function svgTooltip(anchorX,title,items=[]){
  const lines=(items||[]).map(value=>String(value??'')).filter(Boolean),width=236,lineHeight=15,height=34+lines.length*lineHeight;
  const preferred=anchorX>W/2?anchorX-width-10:anchorX+10,x=Math.max(L+2,Math.min(W-R-width-2,preferred)),y=T+5;
  return `<g class="growthSvgTooltip" transform="translate(${x.toFixed(1)} ${y.toFixed(1)})" aria-hidden="true"><rect width="${width}" height="${height}" rx="7"/><text class="growthTooltipTitle" x="10" y="17">${esc(title)}</text>${lines.map((line,i)=>`<text class="growthTooltipLine" x="10" y="${36+i*lineHeight}">${esc(line)}</text>`).join('')}</g>`;
}
function hoverColumn(count,index,centerX,step,title,items=[]){
  const left=count<=1?L:(index===0?L:centerX-step/2),right=count<=1?W-R:(index===count-1?W-R:centerX+step/2),summary=[title,...items].join(' · ');
  return `<g class="growthInteractive"><rect class="growthHit" x="${left.toFixed(1)}" y="${T}" width="${Math.max(1,right-left).toFixed(1)}" height="${PH}"><title>${esc(summary)}</title></rect>${svgTooltip(centerX,title,items)}</g>`;
}

function areaLine(rows,key,tone,formatter=compact,label='Trend',detailBuilder=null){
  ...
  const hovers=pts.map(([x],i)=>{const details=detailBuilder?detailBuilder(rows[i],values[i]):[`${label}: ${formatter(values[i])}`];return hoverColumn(rows.length,i,x,step,rows[i].label,details);}).join('');
  return chartFrame(`${grid(min,max,formatter)}<polygon class="growthArea tone-${tone}" points="${area}"/><polyline class="growthLine tone-${tone}" points="${line}"/>${dots}${hovers}${labels(rows)}`,label);
}
...
function movementBars(rows){
  ...
  const hovers=rows.map((row,i)=>{const fresh=Number(row.new_customers||0),react=Number(row.reactivations||0),churn=Number(row.churned||0),center=L+i*band+band/2;return hoverColumn(rows.length,i,center,band,row.label,[`New: ${number(fresh)}`,`Reactivated: ${number(react)}`,`Churned: ${number(churn)}`,`Net: ${(fresh+react-churn)>=0?'+':''}${number(fresh+react-churn)}`]);}).join('');
  ...
  return chartFrame(`<line class="growthGrid growthZero" x1="${L}" x2="${W-R}" y1="${zero}" y2="${zero}"/>${bars}${hovers}${axisLabels}`,'New customers, reactivations and churn');
}
function signedBars(rows,key){
  ...
  const hovers=rows.map((row,i)=>{const value=values[i],center=L+i*band+band/2;return hoverColumn(rows.length,i,center,band,row.label,[`Net growth: ${value>=0?'+':''}${number(value)}`]);}).join('');
  ...
  return chartFrame(`<line class="growthGrid growthZero" x1="${L}" x2="${W-R}" y1="${zero}" y2="${zero}"/>${bars}${hovers}${axisLabels}`,'Net subscriber growth');
}
function stackedArea(rows,series){
  ...
  const hovers=rows.map((row,i)=>hoverColumn(rows.length,i,x(i),step,row.label,[...series.map(s=>`${s.label}: ${number(row[s.key]||0)}`),`Total: ${number(totals[i])}`])).join('');
...
function percentStacked(rows,series){
  ...
  const hovers=rows.map((row,i)=>hoverColumn(rows.length,i,x(i),step,row.label,series.map(s=>`${s.label}: ${pct(row[s.key]||0)}`))).join('');
  return chartFrame(`${grid(0,100,v=>`${Math.round(v)}%`)}${layers}${hovers}${labels(rows)}`,'Playback method share');
}
function playerBars(rows){if(!rows.length)return empty('No managed playback history in this period.');const max=Math.max(1,...rows.map(r=>Number(r.seconds||0)));return`<div class="playerBars">${rows.map((row,index)=>`<div class="playerBar" title="${esc(`${row.name} · ${fmt1(Number(row.seconds||0)/3600)}h watch time · ${pct(row.share)} · ${number(row.sessions)} plays`)}"><span>${esc(row.name)}</span><div class="playerTrack"><i class="tone-player-${index}" style="width:${Math.max(2,Number(row.seconds||0)/max*100).toFixed(1)}%"></i></div><strong>${esc(pct(row.share))}</strong><small>${esc(number(row.sessions))} plays</small></div>`).join('')}</div>`;}
...
function churnRate(data){const rows=data.growth.rows,current=lastNonNull(rows,'churn_rate'),first=rows.find(r=>r.churn_rate!=null)?.churn_rate??null,delta=current!=null&&first!=null?current-first:null;return`${metric(current==null?'—':pct(current),'churn / active customers at bucket start',delta,{inverse:true})}${areaLine(rows.map(r=>({...r,churn_rate:r.churn_rate??0})),'churn_rate','yellow',pct,'Churn rate',(row,value)=>[`Churn rate: ${pct(value)}`,`Opening customers churned: ${number(row.opening_churned||0)}`,`All churn transitions: ${number(row.churned||0)}`])}${foot('Opening-customer churn only: customers acquired during a bucket are not added to its denominator.')}`;}
function mrr(data){const rows=data.mrr.rows,current=data.mrr.current,first=rows[0]?.mrr_minor||0,delta=deltaPercent(current,first);return`${metric(money(current,data.mrr.currency),`recurring MRR · ${data.mrr.currency}`,delta)}${areaLine(rows,'mrr_minor','green',v=>money(v,data.mrr.currency),'Monthly recurring revenue',(row,value)=>[`MRR: ${money(value,data.mrr.currency)}`])}${foot('Provider-linked recurring Stripe/PayPal records only; prepaid cash is not disguised as MRR.')}`;}
function activeStreams(data){const rows=data.playback.rows,totalSeconds=sum(rows,'bucket_seconds'),weighted=totalSeconds?rows.reduce((s,r)=>s+Number(r.avg_concurrent||0)*Number(r.bucket_seconds||0),0)/totalSeconds:0,totalStarts=sum(rows,'session_starts');return`${metric(fmt1(weighted),`average concurrent streams · ${number(totalStarts)} starts`)}${areaLine(rows,'avg_concurrent','cyan',fmt1,'Average concurrent streams',(row,value)=>[`Average concurrent: ${fmt1(value)}`,`Stream starts: ${number(row.session_starts||0)}`])}${foot(`Playback overlap by ${data.playback.grain}; zero-playback time is included in the selected-period average.`)}`;}
function playMethods(data){const rows=data.playback.rows,unknown=sum(rows,'unknown_seconds'),series=[{key:'directplay_pct',label:'Direct Play'},{key:'directstream_pct',label:'Direct Stream'},{key:'transcode_pct',label:'Transcode'}],legendItems=[['Direct Play','green'],['Direct Stream','blue'],['Transcode','purple']];if(unknown>0){series.push({key:'unknown_pct',label:'Unknown'});legendItems.push(['Unknown','dim']);}const seconds={direct:sum(rows,'directplay_seconds'),stream:sum(rows,'directstream_seconds'),transcode:sum(rows,'transcode_seconds')},total=seconds.direct+seconds.stream+seconds.transcode+unknown,transPct=total?seconds.transcode/total*100:0;return`${metric(pct(transPct),'transcoded watch time')}${legend(legendItems)}${percentStacked(rows,series)}${foot(`Based on watch-time overlap, not just how a session happened to start.${unknown>0?' Unclassified watch time is shown as Unknown.':''}`)}`;}
function players(data){const rows=data.players.rows,totalHours=Number(data.players.totalSeconds||0)/3600;return`${metric(`${fmt1(totalHours)}h`,'total managed watch time')}${playerBars(rows)}${foot('Player watch time is clipped to the selected reporting window; shares use all managed player watch time as the denominator.')}`;}
