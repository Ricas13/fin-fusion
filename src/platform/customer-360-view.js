...
function manualServerAssignmentForm(token,customerId,assignment){
  if(!assignment?.entitlement)return'';
  if((assignment.activeAccounts||[]).length)return'';
  const servers=assignment.servers||[];
  if(!servers.length)return `<div class="operatorCallout warn"><strong>No eligible Jellyfin server is available for manual assignment.</strong> Capacity is not the blocker here: check whether an eligible server is enabled, accepting users, healthy enough for placement and compatible with this plan.</div>`;
  const options=servers.map(server=>`<option value="${escapeHtml(server.id)}">${escapeHtml(server.name)} · ${escapeHtml(server.health_status||'unknown')} · ${escapeHtml(assignmentCapacityLabel(server))}</option>`).join('');
  const overfilled=servers.some(server=>server.full);
  return `<section class="profileCard" style="margin:0 0 14px"><div class="profileCardHead"><div><h2>Assign Jellyfin server</h2><div class="subText">Administrator placement ignores the configured user ceiling.</div></div>${overfilled?'<span class="pill warn">Capacity override available</span>':'<span class="pill good">Manual placement</span>'}</div><div class="profileCardBody"><div class="operatorCallout warn"><strong>Admin assignment can exceed capacity by any amount.</strong> A configured limit such as 50 users continues to control public signup and automatic placement only. Selecting a 50/50, 100/50 or 1000/50 server here does not change its configured limit and does not create public Free Server availability.</div><form class="formPanel compactAction" method="post" action="/admin/users/${encodeURIComponent(customerId)}/assign-server" data-native-submit="true">${csrfHidden(token)}<div class="formGroup"><label>Target Jellyfin server</label><select class="input" name="serverId" required><option value="">Choose server…</option>${options}</select></div><button class="button primary" type="submit">Assign Jellyfin server</button></form></div></section>`;
}
function accessWorkspaceSection(detail,token,accessDetail){
  ...
  const serverActions=includesJellyfin?(canReenable?`${reenableJellyfinForm(token,customerId)}${reconcileForm(token,customerId)}`:accounts.length?`${bulkPreviewForm(token,customerId,'migrate_server','Move server')}${serverPlacementResetForm(token,customerId)}`:reconcileForm(token,customerId,'Try automatic provisioning')):'';
  let expiryActions='';
...
function body(detail,tab,token,accessDetail,options={}){
  ...
  // Put the actions an administrator is most likely to need before the large
  // policy/provisioning diagnostics. This also makes a failed fresh signup
  // recoverable without scrolling to the bottom of Customer 360.
  return jellyfinPasswordSupport(safe)+accessWorkspaceSection(safe,token,accessDetail)+html+foldedAccessSections(safe,token,options);
}
