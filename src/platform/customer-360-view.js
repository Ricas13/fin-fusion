...
function banAction(detail,token,banned){
  const id=detail.customer.id;
  if(banned)return `<details class="customerBanAction"><summary class="mockTopButton danger">Remove ban…</summary><form method="post" action="/admin/users/${encodeURIComponent(id)}/access-ban/revoke" data-native-submit="true">${csrfHidden(token)}<strong>Remove administrative ban</strong><small>Releases the ban across matching customer identities and email registration blocks. Portal logins stay disabled until you enable them explicitly.</small><label>Reason<input class="input" name="reason" minlength="5" maxlength="500" required placeholder="Why is this ban being removed?"></label><label>Type UNBAN<input class="input" name="confirmation" autocomplete="off" required></label><button class="button danger sm" type="submit">Remove ban</button></form></details>`;
  return `<details class="customerBanAction"><summary class="mockTopButton danger">Ban customer…</summary><form method="post" action="/admin/users/${encodeURIComponent(id)}/access-ban" data-native-submit="true">${csrfHidden(token)}<strong>Ban customer</strong><small>Blocks service access, disables matching customer portal logins, revokes active sessions/onboarding links, and blocks matching email addresses from registering. Billing and payment history are preserved.</small><label>Reason<input class="input" name="reason" minlength="3" maxlength="500" required placeholder="Why is this customer being banned?"></label><label>Type BAN<input class="input" name="confirmation" autocomplete="off" required></label><button class="button danger sm" type="submit">Ban customer</button></form></details>`;
}
...
function mockHero(detail,token,permanent){
  ...
  const customerStatus=banned?'Banned':isPermanent?'Permanent User':sub?.is_free_tier?'Free User':paid?'Regular User':'No active plan';
  const statusTone=banned?'bad':isPermanent?'warn':paid||sub?.is_free_tier?'good':'';
  const statusSub=banned?'Administrative ban · service and registration blocked':isPermanent?'Admin-managed permanent access':paid?'Managed by automation':sub?.is_free_tier?'Managed by Free Server rules':'No automatic entitlement';
  ...
  const portalDisabled=banned&&c.app_user_id&&c.login_active===false?'<span class="pill warn">Portal disabled</span>':'';
  return `<style>
  .customerMockHero{display:grid;grid-template-columns:minmax(390px,1.2fr) minmax(520px,1.8fr);gap:22px;align-items:end;margin:4px 0 10px}.customerMockIdentity{display:grid;grid-template-columns:72px minmax(0,1fr);gap:18px;align-items:center;min-width:0}.customerMockAvatar{width:72px;height:72px;border-radius:50%;display:grid;place-items:center;font-size:1.7rem;font-weight:700;background:linear-gradient(145deg,rgba(130,89,167,.72),rgba(70,45,88,.78));border:1px solid rgba(255,255,255,.08)}.customerMockName{display:flex;align-items:center;gap:8px}.customerMockName h2{margin:0;font-size:1.35rem;line-height:1}.customerMockEmail{font-size:.82rem;color:#aebdcc;margin-top:6px}.customerMockMeta{display:flex;gap:7px;flex-wrap:wrap;font-size:.65rem;color:var(--muted,#8796a5);margin-top:8px}.customerMockPills{display:flex;gap:6px;margin-top:8px}.customerMockRight{display:grid;gap:8px}.customerMockTopActions{display:flex;justify-content:flex-end;gap:7px;align-items:flex-start}.mockTopButton{display:inline-flex;align-items:center;justify-content:center;min-height:32px;padding:6px 12px;border:1px solid #344657;border-radius:7px;background:#111a22;color:#dce7ef;text-decoration:none;font-size:.7rem;font-weight:700}.mockTopButton.primary{border-color:#29aef0}.mockTopButton.danger{border-color:rgba(255,110,120,.6);color:#ff9aa1}.customerBanAction{position:relative}.customerBanAction>summary{list-style:none;cursor:pointer}.customerBanAction>summary::-webkit-details-marker{display:none}.customerBanAction>form{position:absolute;right:0;top:38px;z-index:60;width:min(360px,82vw);display:grid;gap:8px;padding:12px;border:1px solid rgba(255,110,120,.42);border-radius:9px;background:#101920;box-shadow:0 16px 36px rgba(0,0,0,.38)}.customerBanAction form strong{font-size:.8rem}.customerBanAction form small{font-size:.65rem;color:var(--muted,#93a3b2);line-height:1.4}.customerBanAction form label{display:grid;gap:4px;font-size:.64rem;color:var(--muted,#93a3b2)}.customerBanAction form .input{min-height:31px;padding:5px 7px}.customerMockMetrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.customerMockMetric{border:1px solid #263644;border-radius:8px;background:#111b24;padding:10px 12px;min-height:72px;display:grid;align-content:center}.customerMockMetric small{font-size:.62rem;color:#93a3b2}.customerMockMetric strong{font-size:.88rem;margin-top:5px}.customerMockMetric span{font-size:.62rem;color:#899aa9;margin-top:3px}.customerMockStatusRow{display:flex;align-items:center;gap:6px;flex-wrap:wrap}.mockInlineDanger{border:0;background:transparent;color:#ff7c84;font-size:.59rem;padding:0;text-decoration:underline;cursor:pointer}.customerLegacyNav{display:none!important}@media(max-width:1180px){.customerMockHero{grid-template-columns:1fr}.customerMockTopActions{justify-content:flex-start}}@media(max-width:760px){.customerMockIdentity{grid-template-columns:56px 1fr}.customerMockAvatar{width:56px;height:56px}.customerMockMetrics{grid-template-columns:repeat(2,1fr)}.customerBanAction>form{position:fixed;left:12px;right:12px;top:90px;width:auto}}
  </style><section class="customerMockHero"><div class="customerMockIdentity"><div class="customerMockAvatar">${escapeHtml(initials(name))}</div><div><div class="customerMockName"><h2 data-customer-name>${escapeHtml(name)}</h2></div><div class="customerMockEmail">${escapeHtml(email)}</div><div class="customerMockMeta"><span>Customer since ${escapeHtml(customerSince)}</span><span>·</span><span>ID: ${escapeHtml(idShort)}</span><span>·</span><span>${escapeHtml(c.registration_source||'Source unknown')}</span></div><div class="customerMockPills">${identityStatus}${portalDisabled}${c.email_verified_at?'<span class="pill">✓ Verified</span>':''}</div></div></div><div class="customerMockRight"><div class="customerMockTopActions">${edit}${portal}${banAction(detail,token,banned)}<button class="mockTopButton" type="button" title="More customer actions">•••</button></div><div class="customerMockMetrics"><div class="customerMockMetric"><small>Current plan</small><strong>${escapeHtml(sub?.plan_name||'None')}</strong><span>${sub?`${escapeHtml(String(sub.status||'active'))} · ${escapeHtml(sub.streams||0)} streams`:'No active subscription'}</span></div><div class="customerMockMetric"><small>Next renewal</small><strong>${escapeHtml(renewal)}</strong><span>${sub?.cancel_at_period_end?'Renewal stopped':'Current period end'}</span></div><div class="customerMockMetric"><small>Total renewal</small><strong>${escapeHtml(price)}</strong><span>${sub?.source?escapeHtml(String(sub.source)):'No provider payment'}</span></div><div class="customerMockMetric"><small>Customer status</small><div class="customerMockStatusRow"><span class="pill ${statusTone}">${escapeHtml(customerStatus)}</span>${permanentRemove}</div><span>${escapeHtml(statusSub)}</span></div></div></div></section>`;
}
