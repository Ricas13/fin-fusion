...
function reconcileForm(token,customerId,label='Reconcile access',tone='secondary'){return `<form class="plainForm" method="post" action="/admin/users/${encodeURIComponent(customerId)}/reconcile" data-native-submit="true">${csrfHidden(token)}<button class="button ${escapeHtml(tone)}" type="submit">${escapeHtml(label)}</button></form>`;}
...
function accessWorkspaceSection(detail,token,accessDetail){
  ...
  const plan=accessDetail?.currentPlan||detail.primaryEntitlement||activeSubscription(detail),sub=activeSubscription(detail),permanent=isPermanent(plan),includesJellyfin=serviceIncludesJellyfin(plan,detail),accounts=(detail.accounts||[]).filter(a=>!a.disabled&&String(a.account_purpose||'jellyfin')!=='stremio_internal');
  ...
  const serverCurrent=accounts.length?accounts.map(a=>a.server_name||'Assigned server').join(', '):'Not assigned';
  ...
  const lastRecon=accounts.map(a=>a.recon_status).filter(Boolean)[0]||'Not reconciled yet',lastError=accounts.map(a=>a.recon_last_error).find(Boolean)||'';
  const planActions=bulkPreviewForm(token,customerId,'plan_change','Manual entitlement edit','primary');
  const serverActions=includesJellyfin?(accounts.length?bulkPreviewForm(token,customerId,'migrate_server','Move server'):'')+reconcileForm(token,customerId,accounts.length?'Reset server to plan placement':'Provision / reconcile access'):'';
  let expiryActions='';
  ...
  else expiryActions=`${bulkPreviewForm(token,customerId,'set_expiry','Change expiry')}${permanentForm(token,customerId,false)}`;
...
module.exports={...v2,body,serviceType,customerFacingDetail,jellyfinPasswordSupport,activeSubscription,accessWorkspaceSection};