...
function accessWorkspaceSection(detail,token,accessDetail){
  ...
  const serverActions=includesJellyfin?(accounts.length?`${bulkPreviewForm(token,customerId,'migrate_server','Move server')}${serverPlacementResetForm(token,customerId)}`:reconcileForm(token,customerId,'Provision / reconcile access')):'';
  let expiryActions='';
