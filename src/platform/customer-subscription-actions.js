...
function createCustomerSubscriptionActionsRouter(){const r=express.Router();r.use('/account/subscription',requireCustomer);r.use('/account/plan-change',requireCustomer);
 r.post('/account/subscription/renewal',guard,async(req,res)=>{try{const current=await currentRecurringOwned(req.session.customerId),enable=String(req.body.action||'')==='resume';if(!['stop','resume'].includes(String(req.body.action||'')))throw new Error('Unknown renewal action.');await billingControl.setRenewal(current.subscription_id||current.id,enable,req.session.customerUserId);return redirect(res,'message',enable?'Automatic renewal resumed.':'Automatic renewal stopped. Your paid access remains active until the current period ends.')}catch(error){return redirect(res,'error',error.message||'Renewal could not be changed.')}});
 r.post('/account/plan-change/cancel',guard,async(req,res)=>{try{const cancelled=await planChange.cancelPendingChange(req.session.customerId,req.session.customerUserId);return redirect(res,'message',`Scheduled plan target cancelled${cancelled.effective_at?` before ${new Date(cancelled.effective_at).toLocaleDateString('en-GB')}`:''}.${cancelled.warning||''}`)}catch(error){return redirect(res,'error',error.message||'Plan change could not be cancelled.')}});return r}
