...
// page and become the job's `params`. Export is intentionally not here: it
// is read-only, so it's a direct action on the list page instead of going
// through the mutate-with-confirmation pipeline.
const BULK_ACTIONS=[
    ...
    ['enable','Enable / reactivate',{highImpact:false,fields:[]}],
    ['disable','Disable',{highImpact:true,fields:[]}],
    ['suspend','Suspend',{highImpact:true,fields:[]}],
...
function parseSelectionInput(body){
    ...
    return {
        selectAllMatching:body.selectAllMatching==='1',
        customerIds,
        filters:require('./admin-customers-list').parseFilters(body)
    };
}
...
// Per-action eligibility -- reasons a resolved customer can't safely be
// processed by this action. Ineligible customers are shown on the preview
// page and excluded from the job entirely (never silently attempted).
function unsafeReason(action,row){
    if(['extend_entitlement','set_expiry','library_add','library_remove','library_replace','library_reset','reconcile'].includes(action)&&!row.plan_id){
...
async function selectionSummary(customerIds,action){
    ...
    const result=await query(`
        SELECT c.id,COALESCE(c.display_name,au.username,c.email,'Customer') AS name,
               cur.plan_id,p.name AS plan_name,
    ...
    const safe=[],unsafe=[];
    const planCounts=new Map(),serverCounts=new Map();
    for(const row of rows){
        const reason=unsafeReason(action,row);
        if(reason){unsafe.push({...row,reason})}else{safe.push(row)}
    ...
    return {
        ...
        serverBreakdown:Array.from(serverCounts.entries())
    };
}
...
function fieldForm(action){
    ...
    return meta.fields.map(field=>{
        ...
        if(field==='planId')return `<div class="formGroup"><label>Target plan ID</label><input class="input" name="planId" required placeholder="Use the plan's UUID from Plans"></div>`;
        if(field==='units')return `<div class="formGroup"><label>Extend by (units of the customer's current plan duration)</label><input class="input" type="number" name="units" min="1" max="24" value="1" required></div>`;
        ...
        if(field==='resellerId')return `<div class="formGroup"><label>Target reseller ID</label><input class="input" name="resellerId" required placeholder="Use the reseller's UUID from Resellers"></div>`;
        return '';
    }).join('');
}
...
function previewPage(req,action,selection,summary,idempotencyKey){
    const meta=ACTION_MAP.get(action);
    const {filterHiddenFields}=require('./admin-customers-list');
    const selectionHidden=selection.selectAllMatching
        ?`<input type="hidden" name="selectAllMatching" value="1">${filterHiddenFields(selection.filters)}`
        :selection.customerIds.map(id=>`<input type="hidden" name="customerId" value="${esc(id)}">`).join('');
    ...
    ${meta.highImpact?'<div class="notice error">This is a high-impact action affecting many customers. Review carefully before confirming.</div>':''}
    ...
    <form class="formPanel" method="post" action="/admin/customers/bulk/confirm">
...
function createAdminBulkCustomersRouter(){
    ...
    r.use('/admin/customers/bulk',gate,noStore);
    ...
    r.post('/admin/customers/bulk/preview',async(req,res,next)=>{
        ...
        try{
            ...
            if(!ids.length)return res.redirect('/admin/users?error='+encodeURIComponent('No customers matched your selection.'));
            const summary=await selectionSummary(ids,action);
            const idempotencyKey=bulkJobs.newIdempotencyKey();
            return res.send(previewPage(req,action,selection,summary,idempotencyKey));
    ...
    r.post('/admin/customers/bulk/confirm',async(req,res)=>{
        ...
        try{
            ...
            if(meta.fields.includes('planId')){params.planId=t(req.body.planId,60);const planCheck=await query('SELECT 1 FROM plans WHERE id=$1 AND active=TRUE',[params.planId]);if(!planCheck.rowCount)throw new Error('validation')}
            ...
            if(meta.fields.includes('resellerId')){params.resellerId=t(req.body.resellerId,60);const resellerCheck=await query('SELECT 1 FROM resellers WHERE id=$1',[params.resellerId]);if(!resellerCheck.rowCount)throw new Error('validation')}
