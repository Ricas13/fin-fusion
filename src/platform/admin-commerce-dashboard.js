...
async function buildContext(req) {
    ...
    const [mrrRows, previousMrrRows, events, refundFailureSeries, renewal, funnel, topPlans, paymentStates] = await Promise.all([
        mrrByCurrency(range.end), mrrByCurrency(range.previousEnd), paymentEventsInRange(range), refundAndFailureSeries(range), renewalVsChurn(range), checkoutFunnel(range), topPlansBySubscribers(10), paymentStateBreakdown()
    ]);
    const mrr = primaryMrr(mrrRows, reporting.currency, reporting), previousMrr = primaryMrr(previousMrrRows, reporting.currency, reporting), revenue = summarizeEvents(events, range, reporting);
