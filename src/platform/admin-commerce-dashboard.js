...
registry.register('commerce', 'netRevenue', {title:'Net revenue',subtitle:'Gross provider receipts minus refunds and available processing fees. Fee data for webhook-only periods may be incomplete until Payment History import.',defaultOrder:3,defaultSpan:3,render:async ctx=>widgets.kpiCard({key:'netRevenue',label:'Net revenue',value:money(ctx.data.revenue.netMinor,ctx.data.revenue.primaryCurrency),meta:`${money(ctx.data.revenue.refundMinor,ctx.data.revenue.primaryCurrency)} refunded · ${money(ctx.data.revenue.feeMinor||0,ctx.data.revenue.primaryCurrency)} provider fees`,delta:mrrDelta(ctx.data.revenue.netMinor,ctx.data.revenue.previousNetMinor)})});
