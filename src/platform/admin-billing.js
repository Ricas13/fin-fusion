...
function recurringProblems(data){return data.subscriptions.filter(row=>row.recurring&&(Boolean(row.last_error)||(row.status==='past_due'&&!row.cancel_at_period_end)));}
...
function billingHero(data, stripeStatus, paypalStatus, coverage = { premium: 0, linked: 0, ending: 0, missing: 0 }, providerCounts = { stripe: 0, paypal: 0 }) {
    const problems=recurringProblems(data),pastDue=problems.filter(row=>row.status==='past_due').length;
...
async function page(req, options = {}) {
    ...
    const problemSection=problems.length?`<section class="section" id="billing-problems">${ui.sectionHeader({title:'Fix these subscriptions first',description:'Past-due subscriptions still expected to renew and provider-sync failures. Provider-linked subscriptions intentionally set to end after the current period remain visible in the linked reference list but are not treated as unresolved operator work.'})}${subscriptionTable(req,problems)}</section>`:'';
