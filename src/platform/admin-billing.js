...
function manualResolveForm(req, row) {
    return `<details class="manualResolve"><summary class="button secondary btn-sm">Resolve</summary><form method="post" action="/admin/billing/${encodeURIComponent(row.subscription_id)}/manual-preview" class="manualResolveForm">${csrfInput(req)}<div class="formGroup"><label>Provider</label><select class="input" name="provider" required><option value="paypal">PayPal</option><option value="stripe">Stripe</option></select></div><div class="formGroup"><label>Provider subscription ID</label><input class="input" name="providerSubscriptionId" maxlength="255" placeholder="I-… or sub_…" required></div><button class="button btn-sm" type="submit">Verify subscription</button></form></details>`;
}
...
function manualResolutionSection(req, result) {
    if (!result) return '';
...
function missingProviderLinksSection(req, rows, manualResult = null, manualError = null) {
    ...
    const warning = manualError ? `<div class="operatorCallout bad"><strong>Could not verify provider subscription:</strong> ${esc(manualError)}</div>` : '';
...
function createAdminBillingRouter() {
    ...
    router.post('/admin/billing/:id/manual-preview', async (req, res, next) => {
        if (!csrf.verify(req)) return res.status(403).send('Invalid security token');
        try {
            const result = await manualLink.preview({ subscriptionId:req.params.id, provider:req.body?.provider, providerSubscriptionId:req.body?.providerSubscriptionId });
            return res.send(await page(req, { manualResolution: result }));
        } catch (error) {
            try { return res.status(400).send(await page(req, { manualError: error.message || String(error) })); }
