...
function subscriptionActions(req, row) {
    ...
    if (!billing.validRecurringProviderReference(row)) return '<span class="muted">Repair provider ID before provider actions</span>';
...
function subscriptionRow(req, row) {
    ...
    const invalidReference = row.recurring && !billing.validRecurringProviderReference(row);
    const remote = invalidReference
        ? `<span class="pill bad">Invalid provider ID</span><div class="subText">${esc(row.provider_subscription_id || 'Missing recurring provider ID')}</div>`
        : row.remote_status ? `${esc(row.remote_status)}${row.last_success_at ? `<div class="subText">Synced ${esc(date(row.last_success_at))}</div>` : ''}` : '<span class="muted">Not synced yet</span>';
    const syncState = invalidReference
        ? '<span class="pill bad">Repair required</span><div class="subText">Provider actions are disabled until the recurring ID is repaired.</div>'
...
function recurringProblems(data){return data.subscriptions.filter(row=>row.recurring&&(!billing.validRecurringProviderReference(row)||Boolean(row.last_error)||(row.status==='past_due'&&!row.cancel_at_period_end)));}

function billingHero(data, stripeStatus, paypalStatus, coverage = { premium: 0, linked: 0, ending: 0, missing: 0 }, providerCounts = { stripe: 0, paypal: 0 }) {
    const problems=recurringProblems(data),pastDue=problems.filter(row=>row.status==='past_due').length,invalidRecurring=Number(providerCounts.invalid||0);
    const providerProblem=[stripeStatus,paypalStatus].some(status=>{
        const liveRecurring=Number(providerCounts?.[status.provider]||0);
        return status.enabled?!status.configured:liveRecurring>0;
    });
    const missing=Number(coverage.missing||0),ending=Number(coverage.ending||0);
    const tone=problems.length?'bad':invalidRecurring?'warn':missing?'warn':providerProblem?'warn':'commerce';
    const title=problems.length?`${problems.length} recurring ${problems.length===1?'subscription needs':'subscriptions need'} attention`:invalidRecurring?`${invalidRecurring} recurring provider ${invalidRecurring===1?'reference needs':'references need'} repair`:missing?`${missing} premium ${missing===1?'user is':'users are'} missing a provider subscription`:providerProblem?'A payment provider is unavailable for live recurring billing':'Recurring billing is clear';
    const next=problems[0]?`Open ${problems[0].portal_username||problems[0].display_name||problems[0].email||'the first customer'} and repair or synchronize its provider state before changing renewal.`:invalidRecurring?'Repair the malformed recurring provider references before running provider reconciliation.':missing?'Resolve the missing provider links below before their local paid-through dates expire.':providerProblem?'Restore the affected payment provider in Payments before the next reconciliation run.':'No billing intervention is required; use Sync due for routine reconciliation.';
    return ui.operatorHero({tone,eyebrow:'Billing operations',title,body:'Every provider-managed recurring subscription needs a valid Stripe or PayPal recurring identity. Paid Premium Server entitlements expected to renew must also resolve to the correct provider subscription; fixed paid terms intentionally ending are reference-only.',statusLabel:problems.length?'Action required':invalidRecurring?'Provider identity repair':missing?'Missing billing links':providerProblem?'Setup incomplete':'Billing healthy',next,facts:[{label:'Premium users',value:String(coverage.premium||0),detail:ending?`${ending} ending without renewal`:'active paid Premium Server entitlements'},{label:'Provider linked',value:String(coverage.linked||0),detail:'premium users with recurring billing IDs'},{label:'Missing link',value:String(missing),detail:'visible below until resolved'},{label:'Past due',value:String(pastDue),detail:'customer payment attention'}],actionsHtml:problems[0]?`<a class="button" href="/admin/users/${encodeURIComponent(problems[0].customer_id)}?tab=billing">Open first affected customer</a><a class="button secondary" href="#billing-problems">Billing problems</a>`:missing?'<a class="button" href="#missing-provider-links">Resolve missing links</a><a class="button secondary" href="#billing-discovery">Run discovery</a>':'<a class="button secondary" href="#billing-reconcile">Reconcile due</a><a class="button secondary" href="/admin/payments">Payment providers</a>'});
}
...
function manualResolveForm(req, row) {
    return `<details class="manualResolve"><summary class="button secondary btn-sm">Resolve</summary><form method="post" action="/admin/billing/${encodeURIComponent(row.subscription_id)}/manual-preview#manual-provider-preview" class="manualResolveForm" data-native-submit="true">${csrfInput(req)}<div class="formGroup"><label>Provider</label><select class="input" name="provider" required><option value="paypal">PayPal</option><option value="stripe">Stripe</option></select></div><div class="formGroup"><label>Provider subscription ID</label><input class="input" name="providerSubscriptionId" maxlength="255" placeholder="I-… or sub_…" required></div><button class="button btn-sm" type="submit">Verify subscription</button><div class="subText">CAPTAiNFiN will reload this section with the provider result before anything is linked.</div></form></details>`;
}
...
function endingPaidTermRow(row) {
    const identity = row.portal_username || row.display_name || row.email || 'Customer';
    return `<tr><td data-label="Customer"><a href="/admin/users/${encodeURIComponent(row.customer_id)}?tab=billing"><strong>${esc(identity)}</strong></a><div class="subText">${esc(row.email||'')}</div></td><td data-label="Plan"><strong>${esc(row.plan_name||row.plan_code||'Premium')}</strong><div class="subText">${esc(row.plan_code||'')}</div></td><td data-label="Local source">${pill(row.source||'local')}</td><td data-label="Paid through">${esc(date(row.current_period_end))}</td><td data-label="State">${pill('Ending','good')}<div class="subText">Expires automatically after the paid-through date</div></td><td data-label="Action" class="right">${pill('No action required','good')}</td></tr>`;
}
function endingPaidTermsSection(rows) {
    if (!rows.length) return '';
    const table = `<div class="tableWrap"><table class="dataTable responsiveTable endingTermTable"><thead><tr><th>Customer</th><th>Plan</th><th>Local source</th><th>Paid through</th><th>State</th><th class="right">Action</th></tr></thead><tbody>${rows.map(endingPaidTermRow).join('')}</tbody></table></div>`;
    return `<section class="section" id="ending-paid-terms">${ui.sectionHeader({title:`Paid terms ending without renewal (${rows.length})`,description:'These customers have valid paid access until the shown date, but renewal is intentionally stopped. They do not need a provider subscription link.'})}<div class="operatorCallout good"><strong>No action required.</strong> Access remains active through the paid-through date and expires automatically afterwards.</div>${table}</section>`;
}
...
function discoveryStatePill(state) {
    ...
    if (state === 'ending') return pill('Ending', 'good');
...
async function page(req, options = {}) {
    ...
    const problemSection=problems.length?`<section class="section" id="billing-problems">${ui.sectionHeader({title:'Fix these subscriptions first',description:'Malformed recurring provider IDs, past-due subscriptions still expected to renew and provider-sync failures. Provider-linked subscriptions intentionally set to end after the current period remain visible in the linked reference list but are not treated as unresolved operator work.'})}${subscriptionTable(req,problems)}</section>`:'';
...
module.exports = { createAdminBillingRouter, page, billingHero, recurringProblems, subscriptionTable, subscriptionRow, discoverySection, missingProviderLinksSection, endingPaidTermsSection, expiryUrgency, manualResolutionSection };
