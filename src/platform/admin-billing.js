...
async function page(req, options = {}) {
    ...
    const missingRows=premiumRows.filter(row=>!discovery.localRecurring(row)).sort((a,b)=>new Date(a.current_period_end||'9999-12-31')-new Date(b.current_period_end||'9999-12-31'));
    ...
    const problemSection=problems.length?`<section class="section" id="billing-problems">${ui.sectionHeader({title:'Fix these subscriptions first',description:'Past-due subscriptions still expected to renew and provider-sync failures. Subscriptions intentionally set to end after the current period remain visible in the linked list but are not treated as unresolved operator work.'})}${subscriptionTable(req,problems)}</section>`:'';
