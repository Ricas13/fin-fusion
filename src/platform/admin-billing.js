...
function billingHero(data, stripeStatus, paypalStatus, coverage = { premium: 0, linked: 0, ending: 0, missing: 0 }) {
    const problems=recurringProblems(data),pastDue=problems.filter(row=>row.status==='past_due').length,providerProblem=(stripeStatus.enabled&&!stripeStatus.configured)||(paypalStatus.enabled&&!paypalStatus.configured),missing=Number(coverage.missing||0),ending=Number(coverage.ending||0);
