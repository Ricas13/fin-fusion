...
async function timeline({type='',q='',limit=250}={}){
 ...
 const [audit,auth,payments,provisioning,email]=await Promise.all([
  ...
  query(`SELECT COALESCE(completed_at,started_at,created_at) AS at,'provisioning' kind,action title,status subject,customer_id::text subject_id,detail FROM provisioning_runs ORDER BY COALESCE(completed_at,started_at,created_at) DESC LIMIT $1`,[n]),
