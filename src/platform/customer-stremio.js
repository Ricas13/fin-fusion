...
function requestRedirect(req,kind,message){return req.body?.returnTo==='access'?`/account/access?${kind}=${encodeURIComponent(message)}#stremio-access`:homeRedirect(kind,message);}
...
async function currentInstallation(req,customerId){
...
function createCustomerStremioRouter(){
  ...
  // Compatibility URL: Stremio setup remains available from Account Home.
  r.get('/account/stremio',(req,res)=>res.redirect(302,'/account#stremio-access'));
  r.get('/account/stremio/installation.json',async(req,res)=>{try{res.setHeader('Cache-Control','no-store, private, max-age=0');res.setHeader('Pragma','no-cache');return res.json(await currentInstallation(req,req.session.customerId));}catch(error){console.warn('Customer Stremio installation lookup failed:',{customerId:req.session.customerId,error:error.message});return res.status(503).json({manifestUrl:null,installUrl:null,error:'Stremio installation link is temporarily unavailable.'});}});
  r.post('/account/stremio/install',mutateLimit,async(req,res)=>{if(!csrf.verify(req))return res.status(403).send('Invalid security token');try{const{provisioned}=await issueCustomerInstallation(req.session.customerId,{actorUserId:req.session.customerUserId});return res.redirect(requestRedirect(req,provisioned?'message':'error',provisioned?'Your new Stremio installation link is ready. This is a secret bearer link: anyone who has it can use your Stremio access, so treat it like a password and do not share it. Any previous installation link has been replaced.':'Your new Stremio installation link is ready, but automatic access setup is still finishing. Treat the link like a password and do not share it. If playback does not work within a few minutes, retry Stremio setup.'));}catch(error){return res.redirect(requestRedirect(req,'error',error.message||'Stremio installation link could not be created.'));}});
