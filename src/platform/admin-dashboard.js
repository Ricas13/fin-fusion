...
async function dashboardPage(req,res){
  ...
  try{
    ...
    return res.send(layout({siteName:process.env.SITE_NAME||'CAPTaINFiN',active:'dashboard',title:'Admin Dashboard',subtitle:'Customers, resellers, subscriptions and Jellyfin operations',body:renderDashboard(stats),action:primaryAction(stats)}));
  }catch(error){
    console.error('Admin dashboard failed:',error.message);
    return res.status(500).render('auth/message',{siteName:process.env.SITE_NAME||'CAPTaINFiN',title:'Dashboard unavailable',message:'The administration dashboard could not be loaded safely.',link:'/admin/setup',linkText:'Open Setup'});
  }
}
module.exports={dashboardPage,dashboardData,primaryAction};
