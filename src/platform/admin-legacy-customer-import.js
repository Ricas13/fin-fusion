...
function metrics(counts) {
  ...
  return `<div class="metrics">
    ...
    <div class="metric"><div class="metricLabel">Safe to restore</div><div class="metricValue">${esc(counts.ready)}</div><div class="subText">${esc(counts.current)} now · ${esc(counts.future)} future</div></div>
...
function candidateRows(result) {
  ...
  return `<div class="tableWrap"><table class="dataTable responsiveTable legacyMigrationTable"><thead><tr><th>Customer</th><th>Legacy plan</th><th>Processor</th><th>Paid</th><th>Term</th><th>Outcome</th></tr></thead><tbody>${important.slice(0,250).map(row => `<tr><td><strong>${esc(row.email)}</strong><div class="subText">${row.customer ? 'Existing CAPTAiNFiN customer' : row.createCustomer ? (row.linkUserId ? 'Will attach existing portal login' : 'Will create imported customer record') : ''}</div></td><td>${esc(row.plan?.name || '—')}${row.planMatch ? `<div class="subText">→ ${esc(row.planMatch.name)}${row.streamOverride ? ` · keep ${esc(row.plan.streams)} streams` : ''}</div>` : ''}</td><td>${esc(provider(row.provider))}<div class="subText"><code>${esc(row.transactionId || '—')}</code></div></td><td>${row.money ? esc(money(row.money.minor,row.money.currency)) : '—'}</td><td>${esc(when(row.start))}<div class="subText">to ${esc(when(row.end))}</div></td><td><span class="pill ${stateClass(row.state)}">${esc(stateLabel(row.state))}</span><div class="subText">${esc(row.reason || '')}</div></td></tr>`).join('')}</tbody></table></div>${important.length>250?`<div class="muted">Showing the first 250 of ${esc(important.length)} non-expired rows.</div>`:''}`;
}
