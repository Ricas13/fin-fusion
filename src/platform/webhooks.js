...
function createWebhookRouter(){
 ...
 router.post('/webhooks/paypal',express.raw({type:'application/json',limit:'1mb'}),async(req,res)=>{try{await providerSettings.ensureLoaded();if(!paypal.enabled())return res.status(404).end();const event=parse(req.body);if(PAYPAL_RISK.has(event.event_type))await paypal.processWebhook(req.body,req.headers);else if(await resellerBilling.isPayPalResellerEvent(req.body))await resellerBilling.processPayPalWebhook(req.body,req.headers);else await paypal.processWebhook(req.body,req.headers);return res.json({received:true})}catch(error){console.error('PayPal webhook error:',error.message);return res.status(400).send('Webhook rejected')}});
 return router;
}
module.exports={createWebhookRouter,STRIPE_RISK,PAYPAL_RISK};
