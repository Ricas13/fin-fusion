...
const COINGATE_RISK=new Set(['order.refunded','order.partially_refunded']);
...
function createWebhookRouter(){
 ...
 // Compatibility only: previously-created CoinGate orders may still send a late
 // callback after Plisio becomes the active crypto gateway. No new customer
 // checkout route advertises or starts CoinGate orders.
 router.post('/webhooks/coingate',express.raw({type:'*/*',limit:'1mb'}),requestMaintenanceGuard,async(req,res)=>{try{await providerSettings.ensureLoaded();if(!coingate.enabled())return res.status(404).end();await coingate.processWebhook(req.body,req.get('content-type')||'');return res.status(204).end()}catch(error){console.error('CoinGate webhook error:',error.message);return res.status(400).send('Webhook rejected')}});
 return router;
}
module.exports={createWebhookRouter,STRIPE_RISK,PAYPAL_RISK,COINGATE_RISK,PLISIO_RISK,paypalCheckoutCompletion,completeCheckoutOrIncident,intentIdentity};
