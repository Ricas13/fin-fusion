...
function intentIdentity(intent){if(!intent)return{scope:'unresolved',customerId:null,resellerId:null};if(intent.scope==='customer')return{scope:'direct',customerId:intent.customer_id,resellerId:null};if(intent.scope==='reseller')return{scope:'reseller',customerId:null,resellerId:intent.reseller_id};return{scope:'unresolved',customerId:null,resellerId:null}}
async function completeCheckoutOrIncident({provider,eventId,eventType,providerId,state='completed'}){try{const completed=await checkoutIntents.completeVerifiedProvider(provider,providerId,state);if(state==='completed'&&!completed)throw new Error('Provider confirmed checkout but no matching local checkout intent exists.');return completed}catch(error){if(state==='completed'){const intent=await checkoutIntents.findProviderIntent(provider,providerId).catch(()=>null);await incidents.record({provider,eventId:String(eventId||providerId),caseId:providerId,kind:'checkout_completion',status:'open',identity:intentIdentity(intent),metadata:{eventType,checkoutIntentId:intent?.id||null,checkoutState:intent?.state||null,error:String(error.message||error).slice(0,1000)}}).catch(recordError=>console.error('Checkout completion incident could not be recorded:',recordError.message));}throw error}}
function createWebhookRouter(){
 const router=express.Router();
 router.post('/webhooks/stripe',express.raw({type:'application/json',limit:'1mb'}),requestMaintenanceGuard,async(req,res)=>{try{await providerSettings.ensureLoaded();if(!stripe.enabled())return res.status(404).end();const signature=req.get('stripe-signature');if(!signature)return res.status(400).send('Missing Stripe signature');const event=parse(req.body);if(STRIPE_RISK.has(event.type))await stripe.processWebhook(req.body,signature);else if(await resellerBilling.isStripeResellerEvent(req.body))await resellerBilling.processStripeWebhook(req.body,signature);else await stripe.processWebhook(req.body,signature);const object=event?.data?.object||{};if(event.type==='checkout.session.completed'&&object.id)await completeCheckoutOrIncident({provider:'stripe',eventId:event.id,eventType:event.type,providerId:object.id,state:'completed'});if(event.type==='checkout.session.expired'&&object.id)await completeCheckoutOrIncident({provider:'stripe',eventId:event.id,eventType:event.type,providerId:object.id,state:'cancelled'});return res.json({received:true})}catch(error){console.error('Stripe webhook error:',error.message);return res.status(400).send('Webhook rejected')}});
 router.post('/webhooks/paypal',express.raw({type:'application/json',limit:'1mb'}),requestMaintenanceGuard,async(req,res)=>{try{await providerSettings.ensureLoaded();if(!paypal.enabled())return res.status(404).end();const event=parse(req.body);if(PAYPAL_RISK.has(event.event_type))await paypal.processWebhook(req.body,req.headers);else if(await resellerBilling.isPayPalResellerEvent(req.body))await resellerBilling.processPayPalWebhook(req.body,req.headers);else await paypal.processWebhook(req.body,req.headers);const providerId=paypalCheckoutCompletion(event);if(providerId)await completeCheckoutOrIncident({provider:'paypal',eventId:event.id,eventType:event.event_type,providerId,state:'completed'});return res.json({received:true})}catch(error){console.error('PayPal webhook error:',error.message);return res.status(400).send('Webhook rejected')}});
 return router;
}
module.exports={createWebhookRouter,STRIPE_RISK,PAYPAL_RISK,paypalCheckoutCompletion,completeCheckoutOrIncident,intentIdentity};
