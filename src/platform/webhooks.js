...
async function checkoutFinalizationReady(provider,eventId,processingResult){if(!processingResult?.duplicate)return true;return lifecycle.paymentEventProcessed(provider,eventId)}
...
function createWebhookRouter(){
 ...
 router.post('/webhooks/stripe',express.raw({type:'application/json',limit:'1mb'}),requestMaintenanceGuard,async(req,res)=>{try{await providerSettings.ensureLoaded();if(!stripe.enabled())return res.status(404).end();const signature=req.get('stripe-signature');if(!signature)return res.status(400).send('Missing Stripe signature');const event=parse(req.body),processingResult=await stripe.processWebhook(req.body,signature),mayFinalize=await checkoutFinalizationReady('stripe',event.id,processingResult),object=event?.data?.object||{};if(mayFinalize&&event.type==='checkout.session.completed'&&object.id){const subscriptionId=typeof object.subscription==='string'?object.subscription:object.subscription?.id||null;await completeCheckoutOrIncident({provider:'stripe',eventId:event.id,eventType:event.type,providerId:object.id,state:'completed',providerSubscriptionId:subscriptionId})}if(mayFinalize&&event.type==='checkout.session.expired'&&object.id)await completeCheckoutOrIncident({provider:'stripe',eventId:event.id,eventType:event.type,providerId:object.id,state:'cancelled'});return res.json({received:true})}catch(error){console.error('Stripe webhook error:',error.message);return res.status(400).send('Webhook rejected')}});
 router.post('/webhooks/paypal',express.raw({type:'application/json',limit:'1mb'}),requestMaintenanceGuard,async(req,res)=>{try{await providerSettings.ensureLoaded();if(!paypal.enabled())return res.status(404).end();const event=parse(req.body),processingResult=await paypal.processWebhook(req.body,req.headers),mayFinalize=await checkoutFinalizationReady('paypal',event.id,processingResult),providerId=paypalCheckoutCompletion(event);if(mayFinalize&&providerId)await completeCheckoutOrIncident({provider:'paypal',eventId:event.id,eventType:event.event_type,providerId,state:'completed',providerSubscriptionId:providerId});return res.json({received:true})}catch(error){console.error('PayPal webhook error:',error.message);return res.status(400).send('Webhook rejected')}});
...
module.exports={createWebhookRouter,STRIPE_RISK,PAYPAL_RISK,PLISIO_RISK,paypalCheckoutCompletion,checkoutFinalizationReady,completeCheckoutOrIncident,intentIdentity};
