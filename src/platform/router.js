...
function createRouter() {
    ...
    router.get('/account/register', (req, res) => {
        res.render('customer/register', {
            error: null,
            registrationOpen: process.env.PUBLIC_REGISTRATION !== 'false',
    ...
    router.post('/account/register', async (req, res) => {
        try {
            const created = await customers.registerCustomer(req.body);
    ...
    router.post('/account/logout', requireCustomer, (req, res) => {
    ...
    router.post('/account/trial/start', requireCustomer, async (req, res) => {
        try {
            await lifecycle.startFreeTrial(req.session.customerId);
            return res.redirect('/account?message=' + encodeURIComponent('Your 24-hour trial is active.'));
        } catch (error) {
    ...
    router.post('/account/stripe/portal', requireCustomer, async (req, res) => {
        try {
            const portal = await stripe.createCustomerPortal({
                customerId: req.session.customerId,
                returnUrl: absoluteUrl(req, '/account')
            });
