...
function checklistItem(item) {
    return `<a class="setupStep ${item.configured ? 'complete' : ''}" href="${esc(item.href)}"><span class="setupStepMark" aria-hidden="true">${item.configured ? '✓' : '○'}</span><span class="setupStepBody"><strong>${esc(item.label)}</strong><small>${esc(item.detail)}</small></span>${ui.statusBadge(item.configured ? 'Ready' : 'Optional', item.configured ? 'good' : '')}</a>`;
}
function moduleRow(module) {
    return `<tr><td><strong>${esc(module.name)}</strong></td><td>${ui.statusBadge(stateLabel(module.state), stateKind(module.state))}</td><td class="muted">${esc(module.detail || '')}</td></tr>`;
}
function healthRow(issue){return `<tr><td>${ui.statusBadge(issue.severity, issue.severity==='critical'?'bad':issue.severity==='warning'?'warn':'')}</td><td>${esc(issue.area)}</td><td><strong>${esc(issue.name)}</strong></td><td>${esc(issue.message)}</td><td>${issue.href?`<a class="button secondary btn-sm" href="${esc(issue.href)}">Fix / review</a>`:''}</td></tr>`;}
...
function page(data, health) {
    ...
    const nextCard=next
        ...
        : `<div class="setupNext complete"><div><span class="eyebrow">Setup status</span><strong>Everything on the checklist is configured</strong><small>You can still change any capability later from its normal admin page.</small></div>${ui.statusBadge('Ready','good')}</div>`;
    const healthSection=health.issues.length
        ? `<section class="card setupHealth"><div class="card-header"><div><h2 class="card-title">Configuration & dependency health</h2><div class="muted">These are live issues worth reviewing; routine setup choices stay separate below.</div></div><div class="buttonRow">${ui.statusBadge(`${critical} critical`,critical?'bad':'good')}${ui.statusBadge(`${warnings} warning${warnings===1?'':'s'}`,warnings?'warn':'')}<a class="button secondary" href="/admin/attention">Needs Attention</a></div></div><div class="tableWrap"><table class="dataTable responsiveTable"><thead><tr><th>Severity</th><th>Area</th><th>Object</th><th>Issue / impact</th><th></th></tr></thead><tbody>${health.issues.map(healthRow).join('')}</tbody></table></div></section>`
        : `<div class="setupHealthy">${ui.emptyState({title:'No configuration or live dependency issues detected',body:'Setup can stay incremental; nothing needs attention right now.',tone:'success'})}</div>`;
    const body = `<section class="setupHero"><div class="setupHeroTop"><div>${ui.statusBadge('GETTING STARTED','good')}<h2>Configure CAPTAiNFiN at your own pace</h2><p>${esc(data.configuredCount)} of ${esc(data.totalCount)} optional capabilities are configured. The admin portal remains usable while the rest are left off.</p></div><strong class="setupPercent">${esc(percent)}%</strong></div><div class="setupProgress" role="progressbar" aria-label="Optional setup progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${esc(percent)}"><span style="width:${esc(percent)}%"></span></div>${nextCard}</section>
        ...
        ${healthSection}
        <details class="card setupAdvanced"><summary><span><strong>Readiness & advanced setup</strong><small>Feature states, portability and clean-install guarantees</small></span>${ui.statusBadge('Details')}</summary><div class="setupAdvancedBody">
