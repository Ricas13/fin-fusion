...
function page(data, health) {
    ...
    const body = `<section class="card"><div class="card-header"><div><h2 class="card-title">Setup checklist</h2><div class="muted">${esc(data.configuredCount)} / ${esc(data.totalCount)} configured · every integration is optional</div></div></div><div class="card-body">${data.checklist.map(checklistItem).join('')}</div></section>
        ...
        <section class="card" style="margin-top:16px"><div class="card-header"><div><h2 class="card-title">Configuration & dependency health</h2><div class="muted">Provider mappings, worker heartbeat, request sync, fleet placement, backup and notification dependencies are checked together. Ongoing operational failures also appear in Needs Attention.</div></div><div class="buttonRow"><span class="pill ${critical?'bad':'good'}">${critical} critical</span><span class="pill ${warnings?'warn':''}">${warnings} warning${warnings===1?'':'s'}</span><a class="button secondary" href="/admin/attention">Needs Attention</a></div></div>${health.issues.length?`<div class="tableWrap"><table class="dataTable responsiveTable"><thead><tr><th>Severity</th><th>Area</th><th>Object</th><th>Issue / impact</th><th></th></tr></thead><tbody>${health.issues.map(healthRow).join('')}</tbody></table></div>`:'<div class="card-body"><div class="statusBanner"><strong>No configuration or live dependency issues detected.</strong></div></div>'}</section>
