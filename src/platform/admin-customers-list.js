...
function parseFilters(q){
    ...
    // Presets are shorthand for the same canonical filters used by list/export/bulk.
...
function expiryText(x){
    if(!x.plan_id)return '—';
    if(!x.has_current_entitlement){
        if(x.subscription_starts_at&&new Date(x.subscription_starts_at)>new Date())return `Starts ${relativeTime(x.subscription_starts_at)}`;
        return x.current_period_end?`Expired ${relativeTime(x.current_period_end)}`:'Expired';
    }
...
function rowState(x){
    ...
    if(current){
        ...
        if(x.is_free_tier)return{access:'Ready',tone:'good',reason:'Free access · activity based',action:'Open'};
        if(x.subscription_status==='trialing')return{access:'Ready',tone:'good',reason:'Trial access active',action:'Open'};
        return{access:'Ready',tone:'good',reason:'Entitlement active',action:'Open'};
    }
    if(x.plan_id){
        const stale=jfCount>0;
        return{access:'Expired',tone:stale?'bad':'warn',reason:stale?'No current entitlement · Jellyfin still present':'No current entitlement',action:stale?'Remove access':'Open'};
    }
    return{access:'No entitlement',tone:'',reason:'No plan history',action:'Open'};
}
function jellyfinCell(x,state){
    const current=x.has_current_entitlement===true;
    const required=['jellyfin','bundle',null,undefined].includes(x.service_type);
    const count=Number(x.customer_account_count||0);
    if(current&&!required)return `${pill('Not required')}<div class="subText">Stremio-only plan</div>`;
    if(count>0){const label=current?'Ready':'Still present',tone=current?'good':'bad';return `${pill(label,tone)}${x.jellyfin_username?`<div class="subText">${esc(x.jellyfin_username)}</div>`:''}`;}
    if(current&&required)return `${pill(state.access==='Provisioning'?'Provisioning':'Missing',state.access==='Provisioning'?'warn':'bad')}<div class="subText">No Jellyfin account</div>`;
...
function row(x){
    ...
    const portalNote=x.login_active===false?'<div class="subText">Portal sign-in disabled</div>':'';
    return `<tr data-customer-row>
        ...
        <td data-label="Renews / expires">${esc(expiryText(x))}</td>
        <td data-label="Last active">${esc(relativeTime(x.last_activity_at))}</td>
        <td data-label="Action"><a class="button secondary btn-sm" href="/admin/users/${esc(x.id)}">${esc(state.action)}</a></td>
    </tr>`;
}
...
async function customerOverview(){
    const [summary,plans]=await Promise.all([
        query(`SELECT
            ...
            (SELECT COUNT(*)::int FROM effective_customer_entitlements e WHERE e.status='past_due') past_due,
            ...
            (SELECT COUNT(*)::int FROM effective_customer_entitlements e WHERE COALESCE(NULLIF(e.service_type_snapshot,''),e.service_type,'jellyfin') IN ('jellyfin','bundle')) jellyfin_required,
            (SELECT COUNT(*)::int FROM effective_customer_entitlements e WHERE COALESCE(NULLIF(e.service_type_snapshot,''),e.service_type,'jellyfin') IN ('jellyfin','bundle') AND EXISTS(SELECT 1 FROM jellyfin_accounts ja WHERE ja.customer_id=e.customer_id AND ja.account_purpose='jellyfin')) jellyfin_ready,
            (SELECT COUNT(*)::int FROM effective_customer_entitlements e JOIN customer_provisioning_state cps ON cps.customer_id=e.customer_id WHERE COALESCE(NULLIF(e.service_type_snapshot,''),e.service_type,'jellyfin') IN ('jellyfin','bundle') AND cps.status IN('blocked','failed') AND NOT EXISTS(SELECT 1 FROM jellyfin_accounts ja WHERE ja.customer_id=e.customer_id AND ja.account_purpose='jellyfin')) provisioning_attention,
        ...
        query(`SELECT COALESCE(p.name,'No active plan') name,COUNT(*)::int count FROM effective_customer_entitlements e LEFT JOIN plans p ON p.id=e.plan_id GROUP BY 1 ORDER BY count DESC,name LIMIT 6`).catch(()=>({rows:[]}))
    ]);
    return{summary:summary.rows[0]||{},plans:plans.rows};
}
function customerOverviewHtml(data){
    const s=data.summary,total=Number(s.total||0),ready=Number(s.jellyfin_ready||0),required=Number(s.jellyfin_required||0),active=Number(s.active_access||0),attention=Number(s.provisioning_attention||0)+Number(s.past_due||0);
    return `${graphics.hero({title:'Customer health',subtitle:'Customer growth, current access, service readiness and support needs in one view.',tone:attention?'warn':'good',stats:[graphics.stat({label:'Customers',value:graphics.number(total),meta:`${graphics.number(s.new_30d)} joined in 30 days`,tone:'blue',href:'/admin/users?sort=recent'}),graphics.stat({label:'Current access',value:graphics.number(active),meta:`${graphics.number(s.portal_enabled)} customer sign-ins enabled`,tone:'good'}),graphics.stat({label:'Recently active',value:graphics.number(s.active_30d),meta:'played in the last 30 days',tone:'violet'}),graphics.stat({label:'Needs attention',value:graphics.number(attention),meta:'past due or missing Jellyfin access',tone:attention?'warn':'good',href:'/admin/users?preset=attention'})],meters:[graphics.meter({label:'Jellyfin readiness',value:ready,max:Math.max(required,1),tone:ready>=required?'good':'blue',meta:`${graphics.number(ready)} of ${graphics.number(required)} customers who require Jellyfin are ready`})],actions:'<a class="button secondary" href="/admin/users/new">Add customer</a><a class="button secondary" href="/admin/jellyfin-import">Import Jellyfin users</a>'})}${graphics.insightGrid([{title:'Plan mix',subtitle:'Customers with current effective access',value:graphics.number(active),body:graphics.bars(data.plans),tone:'blue',href:'/admin/plans',linkLabel:'Open plans'},{title:'Activation',subtitle:'Customers who still need to verify their account',value:graphics.number(s.unverified),body:graphics.meter({label:'Verified or ready',value:Math.max(0,total-Number(s.unverified||0)),max:Math.max(total,1),tone:Number(s.unverified||0)?'warn':'good'}),tone:Number(s.unverified||0)?'warn':'good'},{title:'Support pressure',subtitle:'Current customer issues needing review',value:graphics.number(attention),body:graphics.bars([{name:'Past due',count:s.past_due||0},{name:'Missing Jellyfin access',count:s.provisioning_attention||0}]),tone:attention?'warn':'good',href:'/admin/users?preset=attention',linkLabel:'Review attention'}])}`;
}
...
async function listPage(req){
    ...
    const headers=`<th><input type="checkbox" id="checkAllPage" aria-label="Select all customers on this page"></th>${sortHeader(filters,sortState,'Customer','name')}${sortHeader(filters,sortState,'Plan','plan')}${sortHeader(filters,sortState,'Access','access')}<th>Jellyfin</th>${sortHeader(filters,sortState,'Server','server')}${sortHeader(filters,sortState,'Renews / expires','expiring')}${sortHeader(filters,sortState,'Last active','recent')}<th>Action</th>`;
