...
function coverageHtml(rows) {
    if (!rows.length) return `<div class="operatorCallout warn"><strong>No stored provider transaction history is available yet.</strong></div>`;
    return `<div class="operatorCallout"><strong>Stored coverage:</strong> ${rows.map(row => `${esc(providerLabel(row.provider))}: ${esc(dateTime(row.first_at))} → ${esc(dateTime(row.last_at))} · ${esc(row.transactions)} records`).join(' &nbsp; · &nbsp; ')}.</div>`;
}
