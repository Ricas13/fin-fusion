...
async function integrationState(){await Promise.all([runtimeSettings.ensureLoaded(),providerSettings.ensureLoaded(),requestServiceSettings.ensureLoaded()]);const [stripe,paypal,requests,email,notifications]=await Promise.all([providerSettings.status('stripe'),providerSettings.status('paypal'),requestServiceSettings.status(),emailSettings.status().catch(()=>({configured:false,enabled:false})),notificationSettings.status().catch(()=>({telegramEnabled:false,telegramConfigured:false,discordEnabled:false,discordConfigured:false,whatsappEnabled:false,whatsappConfigured:false}))]);return{stripe,paypal,requests,email,notifications};}
