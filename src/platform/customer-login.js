...
function safeNext(value){
 ...
 if(!raw||raw.includes('\\')||/[\u0000-\u001f\u007f]/.test(raw))return'/account';
 try{const url=new URL(raw,'https://customer-portal.invalid');if(url.origin!=='https://customer-portal.invalid')return'/account';const next=`${url.pathname}${url.search}${url.hash}`;return next.startsWith('/')&&!next.startsWith('//')?next:'/account';}catch(_){return'/account';}
}
...
async function identityLoginRateLimit(req,res,next){
 ...
 try{
  // This middleware is mounted directly on the password POST, after the global
  // Turnstile gate and before password verification. Persisted bucket keys are
  // HMACed by customer-rate-limit.js; raw email/usernames never enter the DB.
...
async function twoFactorLoginRateLimit(req,res,next){
 if(req.method!=='POST')return next();
 const userId=String(req.session?.pendingCustomerAuth?.account?.userId||'').trim();
 if(!userId)return next();
 try{
  const result=await customerRateLimit.consume(`customer-login-2fa:${userId}`,{limit:12,windowMs:10*60*1000});
  if(!result.allowed){
   res.setHeader('Retry-After',String(Math.max(1,Math.ceil((result.resetAt.getTime()-Date.now())/1000))));
   return res.status(429).send('Too many security-code attempts. Try again later.');
  }
  return next();
 }catch(error){
  console.error('Customer two-factor login limiter unavailable:',error.message);
  return res.status(503).send('Authentication temporarily unavailable.');
 }
}
