...
function createCustomerLoginRouter(){const r=express.Router();r.get('/account/login',async(req,res,next)=>{try{await runtimeSettings.ensureLoaded();return res.render('customer/login',{error:null,message:req.query.session==='expired'?'Your session expired. Sign in again.':null,next:safeNext(req.query.next),siteName:runtimeSettings.siteName()})}catch(e){next(e)}});return r}
