...
function twoFactorPasswordPage(req,error=null){const site=runtimeSettings.siteName();return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark"><title>Confirm 2FA setup · ${esc(site)}</title><link rel="stylesheet" href="/css/customer-portal.css"><style>main{max-width:620px;margin:40px auto;padding:22px}.panel{padding:20px}.field{margin:14px 0}</style></head><body><main><p><a href="/account/security">← Account security</a></p><section class="panel"><h1>Confirm two-factor setup</h1><p>Enter your current portal password before adding a new authenticator. This prevents a stolen browser session from installing an attacker-controlled second factor.</p>${error?`<div class="notice error">${esc(error)}</div>`:''}<form method="post" action="/account/security/2fa/start"><input type="hidden" name="_csrf" value="${esc(csrf.token(req))}"><div class="field"><label>Current portal password</label><input class="input" type="password" name="currentPassword" autocomplete="current-password" required autofocus></div><button class="button primary">Continue to authenticator setup</button></form></section></main></body></html>`;}
...
function createCustomerLoginRouter(){const r=express.Router();
 ...
 return r}
module.exports={createCustomerLoginRouter,safeNext,processProfile,confirmationPage,twoFactorPasswordPage,establish,normalizeLoginIdentity,identityLoginRateLimit};
