...
function orderTable(orders){if(!orders.length)return'<div class="empty">No provider orders yet.</div>';return `<div class="table"><div class="tableHead" style="grid-template-columns:140px minmax(190px,1.4fr) minmax(180px,1fr) 110px 120px"><span>Date</span><span>Customer</span><span>Plan</span><span>Provider</span><span>Status</span></div>${orders.map(row=>{const customer=row.display_name||row.customer_username||row.customer_email||row.customer_id;return `<a class="tableRow" href="/admin/users/${esc(row.customer_id)}?tab=billing" style="grid-template-columns:140px minmax(190px,1.4fr) minmax(180px,1fr) 110px 120px;text-decoration:none;color:inherit"><span>${esc(when(row.created_at))}</span><span><strong>${esc(customer)}</strong><div class="subText">Open customer billing →</div></span><span>${esc(row.plan_name||row.plan_code||'Plan')}</span><span><span class="pill ${row.source==='stripe'?'accent':''}">${esc(row.source==='stripe'?'Stripe':'PayPal')}</span></span><span><span class="pill ${statusKind(row.status)}">${esc(row.status)}</span></span></a>`}).join('')}</div>`;}
...
async function page(){await runtimeSettings.ensureLoaded();const orders=await rows(),recent=orders.slice(0,25),body=`${ordersHero(orders)}<section class="section">${ui.sectionHeader({title:'Recent purchases',description:'Newest Stripe and PayPal purchases. Select a transaction to continue in that customer’s Billing journey.'})}${orderTable(recent)}${orders.length>recent.length?ui.detailDisclosure({title:`Full purchase history (${orders.length})`,summary:'Older provider purchases · open only when tracing a historical transaction',bodyHtml:orderTable(orders)}):''}</section>`;return layout({siteName:runtimeSettings.siteName(),active:'orders',title:'Orders',subtitle:'Trace purchases to customers; handle recurring billing problems in Billing',body});}
function createAdminOrdersRouter(){const router=express.Router();router.use('/admin/orders',gate,noStore);router.get('/admin/orders',async(_req,res,next)=>{try{return res.send(await page())}catch(error){next(error)}});return router;}
module.exports={createAdminOrdersRouter,page,rows,orderTable,ordersHero};
