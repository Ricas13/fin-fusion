...
async function setupReadiness(){
  ...
  const[counts,directMappings,settings,activeDiscounts,stripeStatus,paypalStatus,requestStatus,emailStatus,automation]=await Promise.all([
    query(`SELECT
    ...
    query(`SELECT pp.provider,COUNT(*)::int mappings FROM plan_provider_prices pp JOIN plans p ON p.id=pp.plan_id WHERE pp.active=TRUE AND p.active=TRUE AND p.audience='direct' GROUP BY pp.provider`),
  ...
  const c=counts.rows[0]||{},direct=Object.fromEntries(directMappings.rows.map(r=>[r.provider,Number(r.mappings||0)])),settingMap=Object.fromEntries(settings.rows.map(r=>[r.setting_key,r.setting_value||{}]));
  const stripeConfigured=Boolean(stripeStatus.configured),paypalConfigured=Boolean(paypalStatus.configured),directStripe=stripeConfigured&&Number(direct.stripe||0)>0,directPayPal=paypalConfigured&&Number(direct.paypal||0)>0;
  const emailReady=Boolean(emailStatus.configured),telegramReady=Boolean(String(process.env.TELEGRAM_BOT_TOKEN||'').trim()&&String(process.env.TELEGRAM_CHAT_ID||'').trim()),notificationsReady=emailReady||telegramReady,storefrontEnabled=runtimeSettings.storefrontEnabled(),registrationEnabled=runtimeSettings.publicRegistrationOpen(),affiliatesEnabled=settingMap.affiliate_program?.enabled===true,requestReady=Boolean(requestStatus.configured),auto=automation.rows[0]||{},configuredCustomerPlans=Number(c.configured_customer_plans||0),freeTiers=Number(c.free_tiers||0),affiliateCount=Number(c.affiliates||0);
  const checklist=[
    ...
    {key:'affiliates',label:'Configure affiliate programme',configured:affiliatesEnabled,detail:affiliatesEnabled?`${affiliateCount} active affiliate account${affiliateCount===1?'':'s'}`:'Optional · enable only when you are ready to run referrals and commissions',href:'/admin/referrals'},
    {key:'requests',label:'Configure request service',configured:requestReady,detail:requestReady?`Request service ready${requestStatus.baseUrl?` · ${requestStatus.baseUrl}`:''}`:'Optional · connect the request service before enabling customer requests',href:'/admin/request-plan-policy'},
  ...
  const modules=[
    ...
    {name:'Registration',state:registrationEnabled?'enabled':'disabled',detail:registrationEnabled?'Public registration open':'Admin/import onboarding only'},
