...
async function setupReadiness(){
  ...
  const checklist=[
    ...
    {key:'plans',label:'Configure customer plans',configured:configuredCustomerPlans>0,detail:configuredCustomerPlans>0?`${configuredCustomerPlans} customer plan${configuredCustomerPlans===1?'':'s'} configured${freeTiers?` · Free Access also present`:''}`:freeTiers?'Permanent Free Access exists but the customer catalogue still needs configuration':'No customer plans yet',href:'/admin/plans'},
    {key:'direct-payments',label:'Configure customer commerce',configured:directStripe||directPayPal,detail:directStripe||directPayPal?[directStripe?'Stripe':null,directPayPal?'PayPal':null].filter(Boolean).join(' + ')+' mapped to customer plans':'Optional · provider credentials plus a plan payment mapping',href:'/admin/payments'},
    ...
    {key:'requests',label:'Configure request service',configured:requestReady,detail:requestReady?`Central request service ready · ${requestStatus.baseUrl}`:'Optional · configure Seerr/Overseerr in Automation → Request Users',href:'/admin/request-users'},
