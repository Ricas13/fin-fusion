...
function overviewInput(body,plan){
  ...
  const features=[body.feature1,body.feature2,body.feature3,body.feature4].map(v=>t(v,90)).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).slice(0,4);return{name,description,audience:plan.audience,billing,duration:n(body.durationDays,1,3650,30),serverClass:['premium','free','custom'].includes(body.serverClass)?body.serverClass:'premium',visible:plan.is_free_tier?true:b(body.visible),active:plan.is_free_tier?true:b(body.active),sort:plan.is_free_tier?0:Number(plan.sort_order||100),features};
}
