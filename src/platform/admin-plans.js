...
function overviewInput(body,plan){
  ...
  if(plan.is_free_tier&&billing==='trial')throw new Error('The permanent Free Access plan cannot be converted into a trial.');
...
async function overviewPage(req,plan){
  ...
  const audienceNote=plan.audience==='direct'
    ? `<div class="securityNote standalone"><strong>Direct customer plan</strong><div class="subText">Resellers now use separate monthly Reseller Plans with their own Jellyfin policy and managed-user allowance.</div></div>`
  ...
  const catalogueControls=free
    ? `<div class="securityNote standalone"><strong>Permanent Free Access</strong><div class="subText">This plan is pinned on the homepage and must remain active and visible. Use Availability to set 0 when no free places should be claimable.</div></div>`
...
async function archiveConfirmPage(req,plan){
  if(plan.is_free_tier)throw new Error('The permanent Free Access plan cannot be archived. Set Availability to 0 when no free spots should be claimable.');
...
async function assertNotAutomaticFreeTarget(plan){
  if(plan.is_free_tier)throw new Error('The permanent Free Access plan cannot be disabled or archived. Set Availability to 0 instead.');
...
function createAdminPlansRouter(){
  ...
  r.post('/admin/plans/:id',async(req,res)=>{
    ...
    try{
      ...
      await transaction(async client=>{
        ...
        await client.query(`UPDATE plans SET name=$2,description=$3,audience=$4,billing_interval=$5,duration_days=$6,server_class=$7,visible=$8,active=$9,sort_order=$10,updated_at=NOW() WHERE id=$1`,[plan.id,p.name,p.description,p.audience,p.billing,p.duration,p.serverClass,p.visible,p.active,p.sort]);
  ...
  r.post('/admin/plans/:id/archive',async(req,res)=>{
    ...
    try{
      const plan=await planById(req.params.id);if(!plan)throw new Error('Plan not found.');
      if(plan.is_free_tier)throw new Error('The permanent Free Access plan cannot be archived. Set Availability to 0 instead.');
