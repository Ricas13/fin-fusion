...
async function page(req){
    ...
    const body=`<form class="formPanel" method="get" action="/admin/search"><div class="formGroup"><label>Search everything</label><div class="buttonRow"><input class="input" style="min-width:min(680px,72vw)" name="q" value="${esc(q)}" placeholder="Name, email, Jellyfin user, plan, server, UUID or provider ID" autofocus><button class="button">Search</button></div><div class="inlineHelp">Searches customers, Jellyfin identities, plans, servers and Stripe/PayPal identifiers.</div></div></form>${q.length<2?'<div class="empty">Enter at least two characters.</div>':`<div class="statusBanner"><strong>${count}</strong> matching record${count===1?'':'s'} across all indexed admin areas.</div>
