...
function createAdminOperationsRouter(){
  ...
  r.get('/admin/operations',(req,res)=>res.redirect(302,forwardQuery(req,'/admin/servers/operations')));
  r.post('/admin/operations',async(req,res)=>{if(!csrf.verify(req))return res.status(403).send('Invalid security token');try{await operations.save(req.body,req.session.authUserId);return res.redirect('/admin/settings?section=general&message='+encodeURIComponent('Legacy operational settings saved. Platform URL/locale are now under General, security limits under Security, and placement under Servers → Fleet operations.'));}catch(error){return res.redirect('/admin/settings?section=general&error='+encodeURIComponent(error.message));}});
  ...
  r.post('/admin/operations/placement-preview',(req,res)=>{if(!csrf.verify(req))return res.status(403).send('Invalid security token');return res.redirect(303,'/admin/servers/operations?message='+encodeURIComponent('Placement preview moved to Fleet operations. Run the dry run there with the current plan/count.'));});
  return r;
}
