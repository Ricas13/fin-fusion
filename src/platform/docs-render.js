...
const SEARCH_SCRIPT=`document.addEventListener('DOMContentLoaded',function(){var input=document.querySelector('[data-doc-search]');if(!input)return;input.addEventListener('input',function(){var q=input.value.trim().toLowerCase();document.querySelectorAll('[data-doc-nav-section]').forEach(function(section){var anyVisible=false;section.querySelectorAll('[data-doc-nav-link]').forEach(function(link){var match=!q||link.textContent.toLowerCase().indexOf(q)>-1;link.hidden=!match;if(match)anyVisible=true;});section.hidden=!anyVisible;});});});`;

function renderDocsPage({site,basePath,backHref,backLabel,brandLabel,sections,sectionSlug,pageSlug}){
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark"><title>${esc(page.title)} · ${esc(page.section)} · ${esc(site)}</title><style>${SHELL_STYLE}</style></head><body><div class="docShell"><nav class="docSidebar"><a class="docBrand" href="${esc(basePath)}"><div class="docBrandSite">${esc(site)}</div><div class="docBrandLabel">${esc(brandLabel)}</div></a><input class="docSearch" type="search" placeholder="Search guide…" data-doc-search aria-label="Search guide"><div data-doc-nav>${sidebar}</div></nav><main class="docMain"><div class="docCrumb"><a href="${esc(basePath)}">${esc(brandLabel)}</a> / ${esc(page.section)}</div><h1>${esc(page.title)}</h1><div class="docBody">${bodyHtml}</div>${footer}<a class="docBack" href="${esc(backHref)}">← ${esc(backLabel)}</a></main></div><script>${SEARCH_SCRIPT}</script></body></html>`;
}

function renderDocsIndex({site,basePath,backHref,backLabel,brandLabel,description,sections}){
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark"><title>${esc(brandLabel)} · ${esc(site)}</title><style>${SHELL_STYLE}.docIndexGrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px;margin-top:32px}.docIndexCard{border:1px solid var(--border);border-radius:12px;padding:18px 20px;background:var(--panel)}.docIndexCard h3{margin:0 0 10px;font-size:14.5px;color:#eef2f6}.docIndexCard ul{margin:0;padding:0;list-style:none}.docIndexCard li{margin-bottom:6px}.docIndexCard a{font-size:13.5px;text-decoration:none}.docIndexCard a:hover{text-decoration:underline}</style></head><body><div class="docShell"><nav class="docSidebar"><a class="docBrand" href="${esc(basePath)}"><div class="docBrandSite">${esc(site)}</div><div class="docBrandLabel">${esc(brandLabel)}</div></a><input class="docSearch" type="search" placeholder="Search guide…" data-doc-search aria-label="Search guide"><div data-doc-nav>${sidebar}</div></nav><main class="docMain"><h1>${esc(brandLabel)}</h1><div class="docBody"><p>${esc(description)}</p></div>${first?`<p><a href="${esc(first.href)}">Start with “${esc(first.title)}” →</a></p>`:''}<div class="docIndexGrid">${cards}</div><a class="docBack" href="${esc(backHref)}">← ${esc(backLabel)}</a></main></div><script>${SEARCH_SCRIPT}</script></body></html>`;
}
