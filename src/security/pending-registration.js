...
async function begin({email,username,password,referralCode=null,ttlMinutes=60}){
    ...
    const row=await transaction(async client=>{
        ...
        const created=await client.query(`INSERT INTO pending_registrations(email,username,password_hash,referral_code,token_hash,expires_at) VALUES($1,$2,$3,$4,$5,$6) RETURNING id,email,username,expires_at,created_at`,[email,username,passwordHash,ref,hash,expiresAt]);
        await client.query(`INSERT INTO audit_log(action,entity_type,entity_id,metadata) VALUES('customer.registration.pending','pending_registration',$1,$2::jsonb)`,[created.rows[0].id,JSON.stringify({email,username,expiresAt,referral:Boolean(ref)})]);
...
async function consume(rawToken){
    const hash=tokenHash(rawToken),created=await transaction(async client=>{
        ...
        const pending=found.rows[0],exists=await client.query(`SELECT 1 FROM app_users WHERE lower(COALESCE(email,''))=lower($1) OR lower(username)=lower($2) LIMIT 1`,[pending.email,pending.username]);
        ...
        await client.query(`INSERT INTO audit_log(actor_user_id,action,entity_type,entity_id,metadata) VALUES($1,'customer.registration.verified','customer',$2,$3::jsonb)`,[user.id,customer.id,JSON.stringify({pendingRegistrationId:pending.id,emailVerified:true})]);
...
module.exports={begin,consume,cleanupExpired,recent,stats,cleanEmail,cleanUsername,validatePassword,tokenHash};
