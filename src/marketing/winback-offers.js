...
async function suppress(id,reason){await query(`UPDATE winback_offers SET status='suppressed',suppression_reason=$2,processing_started_at=NULL,updated_at=NOW() WHERE id=$1`,[id,String(reason).slice(0,160)]);}
async function retryLater(id,error){await query(`UPDATE winback_offers SET status='pending',processing_started_at=NULL,last_error=$2,next_attempt_at=NOW()+make_interval(mins=>$3),updated_at=NOW() WHERE id=$1`,[id,String(error?.message||error).slice(0,1500),RETRY_MINUTES]);}
...
async function processOne(offer){
  try{
    ...
    return true;
  }catch(error){await retryLater(offer.id,error);return false;}
}

async function expireSent(){const result=await query(`UPDATE winback_offers SET status='expired',updated_at=NOW() WHERE status='sent' AND expires_at<=NOW() RETURNING id`);return result.rowCount;}
async function run({limit=100}={}){
  ...
  for(let i=0;i<Math.max(1,Math.min(500,Number(limit)||100));i+=1){const offer=await claimOne();if(!offer)break;processed+=1;const ok=await processOne(offer);if(!ok){failed+=1;continue;}const state=(await query('SELECT status FROM winback_offers WHERE id=$1',[offer.id])).rows[0]?.status;if(state==='sent')sent+=1;else suppressed+=1;}
...
async function markRedeemedTx(client,{customerId,discountCodeId,subscriptionId}){
  ...
  const offer=(await client.query(`SELECT * FROM winback_offers WHERE customer_id=$1 AND status='sent' AND reserved_discount_code_id=$2 AND expires_at>NOW() ORDER BY sent_at DESC LIMIT 1 FOR UPDATE`,[customerId,discountCodeId])).rows[0];
