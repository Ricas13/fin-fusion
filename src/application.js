...
function securityHeaders(req, res, next) {
    ...
    if (!['GET','HEAD','OPTIONS'].includes(req.method)) {
        ...
        if (origin && forwardedHost) {
            try { if (new URL(origin).host !== forwardedHost) return res.status(403).send('Origin mismatch'); }
            catch (_) { return res.status(403).send('Invalid origin'); }
        }
    }
    return next();
}
...
async function staffLoginRateLimit(req, res, next) {
    ...
    try {
        ...
        if (IS_PRODUCTION) return res.status(503).send('Login temporarily unavailable. Try again shortly.');
        return next();
    }
}
...
async function customerAuthRateLimit(req, res, next) {
    ...
    try {
        const result = await customerRateLimit.consume(`${reset ? 'customer-reset' : 'customer-login'}:${req.ip || req.socket?.remoteAddress || 'unknown'}`, {
            limit: reset ? 5 : 10,
            windowMs: 15 * 60 * 1000
        });
        if (!result.allowed) {
        ...
        if (IS_PRODUCTION) return res.status(503).send('Authentication temporarily unavailable.');
        return next();
    }
}
...
function mountPlatform(app) {
    ...
    // Payment providers require the original raw request bytes. Mount this
    // before every general-purpose body parser and before sessions.
    ...
    const legacyResellerPortal = require('./platform/reseller-portal');

    app.use(createBrandingRouter());
    app.use(createInviteOnboardingRouter());
    ...
    app.use(createResellerMonthlyPortalRouter());
    // Credit history/export are read-only compatibility tools while legacy
    // credit balances are being retired. They no longer own /reseller.
    app.get('/reseller/export', legacyResellerPortal.gate, legacyResellerPortal.noStore, legacyResellerPortal.exportClientsCsv);
    app.get('/reseller/credit-history', legacyResellerPortal.gate, legacyResellerPortal.noStore, legacyResellerPortal.creditHistoryPage);
    ...
    app.use(createAdminResellerTiersRouter());
    app.use(createAdminResellerSummaryRouter());
    app.use(createAdminResellersRouter());
    app.use(createAdminInvitationsRouter());
    app.use(createAdminJellyfinImportRouter());
    ...
    app.use(createAdminCatalogShellRouter());
    ...
    app.use(createAdminShellRouter());
...
function start() {
    ...
    const prune = setInterval(() => {
        Promise.all([pruneLoginRateLimits(), customerRateLimit.cleanup()]).catch(error => console.warn('Rate-limit cleanup failed:', error.message));
    }, 60 * 60 * 1000);
    prune.unref?.();
    return { app, server };
}
