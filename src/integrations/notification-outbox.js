...
async function enqueueTelegram({eventType,text,dedupeKey=null,destination=null}){
    ...
    const result=await query(`
        INSERT INTO notification_outbox(channel,message_type,event_type,destination,payload,dedupe_key,status,next_attempt_at)
        VALUES('telegram',$1,$1,$2,$3::jsonb,$4,'pending',NOW())
        ON CONFLICT(dedupe_key) DO NOTHING RETURNING id
    `,[clean(eventType,200),destination||cfg.telegramChatId,JSON.stringify({text:clean(text,3500)}),dedupeKey||null]);
...
async function claim(limit=25){return transaction(async client=>{const rows=await client.query(`
        SELECT id,channel,message_type,event_type,destination,payload,attempts
...
async function deliverDue({limit=25}={}){const rows=await claim(limit);let sent=0,failedCount=0;for(const row of rows){try{if(row.channel==='telegram')await settings.sendTelegram(row.payload?.text||row.message_type,{chatId:row.destination||null});else throw new Error(`Unsupported notification channel ${row.channel}`);await delivered(row.id);sent++}catch(error){await failed(row,error);failedCount++}}return{processed:rows.length,sent,failed:failedCount}}
...
module.exports={enqueueTelegram,deliverDue,recent,retry,claim};
