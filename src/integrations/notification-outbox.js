...
async function enqueue(channel,{eventType,text,dedupeKey=null,destination=null,discordChannel=false,allowEveryone=false}){
    ...
    const payload={text:clean(text,channel==='discord'?1900:4000),...(channel==='discord'&&discordChannel?{discordChannel:true,allowEveryone:Boolean(allowEveryone)}:{})};
...
async function deliverDue({limit=25}={}){const rows=await claim(limit);let sent=0,failedCount=0;for(const row of rows){try{const payload=row.payload_encrypted?decryptPayload(row.payload_encrypted):row.payload||{};if(row.channel==='telegram')await settings.sendTelegram(payload?.text||row.message_type,{chatId:row.destination});else if(row.channel==='discord'&&payload?.discordChannel)await settings.sendDiscordChannel({channelId:row.destination,text:payload?.text||row.message_type,allowEveryone:Boolean(payload?.allowEveryone)});else if(row.channel==='discord')await settings.sendDiscord(payload?.text||row.message_type,{userId:row.destination});else if(row.channel==='whatsapp')await settings.sendWhatsapp(payload?.text||row.message_type,{to:row.destination});else throw new Error(`Unsupported notification channel ${row.channel}`);await delivered(row.id);sent++}catch(error){await failed(row,error);failedCount++}}return{processed:rows.length,sent,failed:failedCount}}
...
module.exports={enqueueTelegram,enqueueDiscord,enqueueDiscordChannel,enqueueWhatsapp,deliverDue,recent,retry,claim,encryptPayload,decryptPayload,STALE_SENDING_MINUTES};
