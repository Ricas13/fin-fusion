...
async function enqueue(channel,{eventType,text,dedupeKey=null,destination=null}){
    ...
    const defaultDestination=channel==='telegram'?cfg.telegramChatId:null;
    if(channel==='whatsapp'&&!destination)return{queued:false,reason:'whatsapp_destination_required'};
    const result=await query(`INSERT INTO notification_outbox(channel,message_type,event_type,destination,payload,payload_encrypted,dedupe_key,status,next_attempt_at) VALUES($1,$2,$2,$3,'{}'::jsonb,$4,$5,'pending',NOW()) ON CONFLICT(dedupe_key) DO NOTHING RETURNING id`,[channel,clean(eventType,200),destination||defaultDestination,encryptPayload({text:clean(text,channel==='discord'?1900:4000)}),dedupeKey||null]);
...
async function deliverDue({limit=25}={}){const rows=await claim(limit);let sent=0,failedCount=0;for(const row of rows){try{const payload=row.payload_encrypted?decryptPayload(row.payload_encrypted):row.payload||{};if(row.channel==='telegram')await settings.sendTelegram(payload?.text||row.message_type,{chatId:row.destination||null});else if(row.channel==='discord')await settings.sendDiscord(payload?.text||row.message_type);else if(row.channel==='whatsapp')await settings.sendWhatsapp(payload?.text||row.message_type,{to:row.destination||null});else throw new Error(`Unsupported notification channel ${row.channel}`);await delivered(row.id);sent++}catch(error){await failed(row,error);failedCount++}}return{processed:rows.length,sent,failed:failedCount}}
...
module.exports={enqueueTelegram,enqueueDiscord,enqueueWhatsapp,deliverDue,recent,retry,claim,encryptPayload,decryptPayload};
