...
(()=>{
  ...
  function form(action,label,buttonClass='secondary'){
    return `<form class="plainForm stremioInlineForm" method="post" action="${esc(action)}"><input type="hidden" name="_csrf" value="${esc(csrfToken)}"><input type="hidden" name="returnTo" value="access"><button class="button ${esc(buttonClass)}" type="submit">${esc(label)}</button></form>`;
  }
  function trialBanner(trial){
    if(!trial?.startsAt||!trial?.endsAt)return'';
    return `<div class="stremioTrialCountdown" data-stremio-trial-start="${esc(trial.startsAt)}" data-stremio-trial-end="${esc(trial.endsAt)}"><div class="stremioTrialCopy"><span class="eyebrow">Trial access</span><strong>Your Stremio trial is active</strong><small data-stremio-trial-range></small></div><div class="stremioTrialRemaining"><span>Time remaining</span><strong data-stremio-trial-remaining>Calculating…</strong></div><div class="stremioTrialProgress" aria-hidden="true"><span data-stremio-trial-progress></span></div></div>`;
  }
  ...
  function render(data){
    ...
    const householdPanel=household
      ? `<div class="panel" style="margin-bottom:12px"><strong>Household access</strong><div class="accessMeta">${esc(household.accessModel||'Unlimited streams · Unlimited devices · 1 household connection')}</div><div class="stremioLeaseLine"><span>Current leased IP</span><strong>${esc(leaseLabel)}</strong><small>${esc(leaseNote)}</small></div>${replacement?`<div class="accessMeta" style="margin-top:8px">${esc(replacement.message||'')}</div>${replacement.allowed?`<div style="margin-top:10px">${form('/account/stremio/reset-household','Use a different household connection','secondary')}</div>`:'<div style="margin-top:10px"><span class="button secondary" aria-disabled="true">Household change on cooldown</span></div>'}`:''}</div>`
