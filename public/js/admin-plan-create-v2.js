...
(function(){
  ...
  const jellyfinBlocks=form.querySelectorAll('[data-jellyfin-policy],[data-jellyfin-only]'),stremioBlocks=form.querySelectorAll('[data-stremio-household]'),lifecycle=form.querySelector('[data-free-lifecycle]');
  function sync(){
    const type=service?.value||'jellyfin',jellyfin=type==='jellyfin'||type==='bundle',stremio=type==='stremio'||type==='bundle',free=Number(price?.value||0)===0,trial=frequency?.value==='trial';
    jellyfinBlocks.forEach(el=>{el.hidden=!jellyfin;});stremioBlocks.forEach(el=>{el.hidden=!stremio;});if(lifecycle)lifecycle.hidden=!(jellyfin&&free&&!trial);
