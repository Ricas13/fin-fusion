...
(() => {
  ...
  function signalMenuMarkup({key,tone,label,headLabel=label,primaryHref,primaryLabel='View all',items}){
    ...
    const summaryTitle=persistent?'Unresolved operational alerts. This badge stays visible until the underlying issue is resolved; Attention items clear when acknowledged.':'';
    const headStatus=persistent?'<small>Persistent until resolved</small>':'';
