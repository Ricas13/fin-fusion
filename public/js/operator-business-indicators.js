...
(() => {
  ...
  function applyMetrics(metrics){const group=ensureMetricNodes();if(!group)return;const streams=group.querySelector('[data-operator-streams]'),revenue=group.querySelector('[data-operator-revenue]');const active=Number(metrics?.streams?.active),total=Number(metrics?.streams?.total);if(streams)streams.textContent=Number.isFinite(active)&&Number.isFinite(total)?`${active}/${total}`:'—/—';if(revenue)revenue.textContent=formatMoney(metrics?.monthlyRevenue?.minor,metrics?.monthlyRevenue?.currency);}
