...
(() => {
  ...
  const operationalLabels={attention:['Needs Attention','Open operational issues','/admin/attention'],servers:['Servers','Offline or degraded fleet signals','/admin/servers'],payments:['Payments','Recent payment incidents','/admin/payments'],finance:['Finance','Expenses approaching renewal or expiry','/admin/finance#expenses']};
  ...
  function applyMetrics(metrics){const group=ensureMetricNodes();if(!group)return;const streams=group.querySelector('[data-operator-streams]'),revenue=group.querySelector('[data-operator-revenue]'),revenuePill=group.querySelector('[data-operator-revenue-pill]');const active=Number(metrics?.streams?.active),total=Number(metrics?.streams?.total);if(streams)streams.textContent=Number.isFinite(active)&&Number.isFinite(total)?`${active} / ${total}`:'— / —';if(revenue){const monthly=formatMoney(metrics?.monthlyRevenue?.minor,metrics?.monthlyRevenue?.currency),yearly=formatMoney(metrics?.yearlyRevenue?.minor,metrics?.yearlyRevenue?.currency);revenue.textContent=`${monthly} / ${yearly}`;}if(revenuePill){const monthCoverage=Number(metrics?.revenueFeeCoverage?.month),yearCoverage=Number(metrics?.revenueFeeCoverage?.year),coverage=Number.isFinite(monthCoverage)&&Number.isFinite(yearCoverage)?` Merchant-fee coverage: ${monthCoverage}% M / ${yearCoverage}% Y.`:'';revenuePill.title=`Customer payments minus known merchant fees and completed refunds or lost chargebacks, this calendar month / calendar year to date.${coverage}`;}}
